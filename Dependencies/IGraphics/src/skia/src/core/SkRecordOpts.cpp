                                               ...
                                               SkPaint* paint)
{
    ...
    if (layerPaint)
    {
        ...
        if (layerPaint->getPathEffect() || layerPaint->getShader() || ! layerPaint->isSrcOver() || layerPaint->getMaskFilter() || layerPaint->getColorFilter() || layerPaint->getImageFilter())
        {
            return false;
        }
...
static bool effectively_srcover (const SkPaint* paint)
{
    ...
    return ! paint->getShader() && ! paint->getColorFilter() && ! paint->getImageFilter() && 0xFF == paint->getAlpha() && paint->getBlendMode() == SkBlendMode::kSrc;
}
...
struct SaveLayerDrawRestoreNooper
{
    ...
    bool onMatch (SkRecord* record, Match* match, int begin, int end)
    {
        ...
        if (match->first<SaveLayer>()->saveLayerFlags & SkCanvasPriv::kDontClipToLayer_SaveLayerFlag)
        {
...
struct SvgOpacityAndFilterLayerMergePass
{
    ...
    bool onMatch (SkRecord* record, Match* match, int begin, int end)
    {
        ...
        if (! fold_opacity_layer_color_to_paint (opacityPaint, true /*isSaveLayer*/, filterLayerPaint))
        {
            return false;
        }
