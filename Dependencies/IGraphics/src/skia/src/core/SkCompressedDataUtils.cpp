        ...
        for (int x = 0; x < numXBlocks; ++x)
        {
            ...
            for (int i = 0; i < 4; ++i, ++baseShift)
            {
                for (int j = 0; j < 4; ++j)
                {
                    ...
                    int pixelIndex = ((low >> (baseShift + (j * 4))) & 0x1) | (low >> (baseShift + (j * 4) + 15) & 0x2);
...
static bool decompress_bc1 (SkISize dimensions, const uint8_t* srcData, bool isOpaque, SkBitmap* dst)
{
    ...
    for (int y = 0; y < numYBlocks; ++y)
    {
        for (int x = 0; x < numXBlocks; ++x)
        {
            ...
            { // signal for a transparent block
                   ...
                   SkBitmap* dst)
{
    ...
    switch (compressionType)
    {
        case Type::kNone:
            return false;
        case Type::kETC2_RGB8_UNORM:
            return decompress_etc1 (dimensions, bytes, dst);
        case Type::kBC1_RGB8_UNORM:
            return decompress_bc1 (dimensions, bytes, true, dst);
        case Type::kBC1_RGBA8_UNORM:
            return decompress_bc1 (dimensions, bytes, false, dst);
    }
...
size_t SkCompressedDataSize (SkImage::CompressionType type, SkISize dimensions, SkTArray<size_t>* individualMipOffsets, bool mipMapped)
{
    ...
    switch (type)
    {
        ...
        case SkImage::CompressionType::kBC1_RGBA8_UNORM:
        {
            ...
            {
                int numBlocks = num_4x4_blocks (dimensions.width()) * num_4x4_blocks (dimensions.height());
