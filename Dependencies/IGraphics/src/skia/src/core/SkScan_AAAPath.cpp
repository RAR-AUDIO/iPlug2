...
public:
    ...
    static bool CanHandleRect (const SkIRect& bounds)
    {
        ...
        return (width <= MaskAdditiveBlitter::kMAX_WIDTH) && (storage <= MaskAdditiveBlitter::kMAX_STORAGE);
    }
...
protected:
    ...
    void advanceRuns()
    {
        ...
        fRuns.fRuns = reinterpret_cast<int16_t*> (reinterpret_cast<uint8_t*> (fRunsBuffer) + fCurrentRun * kRunsSz);
    ...
    SkAlpha snapAlpha (SkAlpha alpha) { return alpha > 247 ? 0xFF : alpha < 8 ? 0x00
                                                                              : alpha; }
...
static bool is_smooth_enough (SkAnalyticEdge* thisEdge, SkAnalyticEdge* nextEdge, int stop_y)
{
    if (thisEdge->fCurveCount < 0)
    {
        ...
        return SkAbs32 (cEdge.fCDx) >> 1 >= SkAbs32 (cEdge.fCDDx) >> ddshift && SkAbs32 (cEdge.fCDy) >> 1 >= SkAbs32 (cEdge.fCDDy) >> ddshift &&
    ...
    else if (thisEdge->fCurveCount > 0)
    {
        ...
        return SkAbs32 (qEdge.fQDx) >> 1 >= SkAbs32 (qEdge.fQDDx) && SkAbs32 (qEdge.fQDy) >> 1 >= SkAbs32 (qEdge.fQDDy) &&
        ...
        if (0 == (dLeft | dRite))
        {
            ...
            if (fullRite >= fullLeft)
            {
                ...
                if (fullBot > fullTop &&
                    ...
                    // (bug chromium:662800)
                    (fullRite > fullLeft || fixed_to_alpha (partialLeft) > 0 || fixed_to_alpha (partialRite) > 0))
                {
            ...
            { // left and rite are within the same pixel
                if (partialTop > 0)
                {
                    blitter->blitAntiH (fullLeft - 1,
                                        fullTop - 1,
        ...
        else
        {
            ...
            if (count > 1)
            {
                ...
                { // There's a partial-row on the top
                    ...
                    SkASSERT ((left & kSnapMask) >= leftBound && (rite & kSnapMask) <= riteBound && (nextLeft & kSnapMask) >= leftBound && (nextRite & kSnapMask) <= riteBound);
...
static bool edges_too_close (SkAnalyticEdge* prev, SkAnalyticEdge* next, SkFixed lowerY)
{
    ...
    return next && prev && next->fUpperY < lowerY && prev->fX + SLACK >= next->fX - SkAbs32 (next->fDX);
                                  ...
                                  SkFixed rightClip)
{
    ...
    SkAlpha fullAlpha = fixed_to_alpha (lowerY - SkIntToFixed (y)) - fixed_to_alpha (leftE->fSavedY - SkIntToFixed (y));
    ...
    blit_trapezoid_row (
        ...
        maskRow,
        isUsingMask,
        noRealBlitter || (fullAlpha == 0xFF && (edges_too_close (leftE->fPrev, leftE, lowerY) || edges_too_close (riteE, riteE->fNext, lowerY))),
        true);
            ...
            if (isRite)
            {
                ...
                else
                {
                    ...
                    blit_trapezoid_row (
                        ...
                        maskRow,
                        isUsingMask,
                        noRealBlitter || (fullAlpha == 0xFF && (edges_too_close (prevRite, left, leftE->fX) || edges_too_close (currE, currE->fNext, nextY))),
                        true);
        ...
        if (in_interval)
        {
            ...
            else
            {
                blit_trapezoid_row (blitter,
                                    ...
                                    maskRow,
                                    isUsingMask,
                                    noRealBlitter || (fullAlpha == 0xFF && edges_too_close (leftE->fPrev, leftE, nextY)),
                                    true);
            }
        }
...
{ // forceRLE implies that SkAAClip is calling us
    ...
    else
    {
        ...
        bool useDeferred =
            count > (SkFixedFloorToInt (tailEdge.fPrev->fLowerY - headEdge.fNext->fUpperY) + 1) * 4;
