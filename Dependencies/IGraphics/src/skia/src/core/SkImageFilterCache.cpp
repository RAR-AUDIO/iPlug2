...
public:
    ...
    struct Value
    {
        Value (const Key& key, const skif::FilterResult<For::kOutput>& image, const SkImageFilter* filter)
    ...
    void set (const Key& key, const SkImageFilter* filter, const skif::FilterResult<For::kOutput>& result) override
    {
        SkAutoMutexExclusive mutex (fMutex);
        if (Value* v = fLookup.find (key))
        {
