...
public:
    ...
    void addAntiRectRun (int x, int y, int width, int height, SkAlpha leftAlpha, SkAlpha rightAlpha)
    {
...
typedef void (*RowProc) (SkAAClip::Builder&, int bottom, const uint8_t* rowA, const SkIRect& rectA, const uint8_t* rowB, const SkIRect& rectB);
...
static void operatorX (SkAAClip::Builder& builder, int lastY, RowIter& iterA, RowIter& iterB, AlphaProc proc, const SkIRect& bounds)
{
...
static void operateY (SkAAClip::Builder& builder, const SkAAClip& A, const SkAAClip& B, SkRegion::Op op)
{
...
bool SkAAClip::op (const SkAAClip& clipAOrig, const SkAAClip& clipBOrig, SkRegion::Op op)
{
    AUTO_AACLIP_VALIDATE (*this);
...
void SkAAClipBlitter::blitAntiH (int x, int y, const SkAlpha aa[], const int16_t runs[])
{
    const uint8_t* row = fAAClip->findRow (y);
    int initialCount;
    row = fAAClip->findX (row, x, &initialCount);
