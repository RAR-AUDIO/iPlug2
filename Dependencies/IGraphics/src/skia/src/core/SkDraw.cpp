    ...
    SkMatrix* matrix = nullptr)
{
    ...
    paint.setShader (SkMakeBitmapShaderForPaint (origPaint, bitmap, SkTileMode::kClamp, SkTileMode::kClamp, matrix, kNever_SkCopyPixelsMode));
    return paint;
}
...
static void bw_pt_rect_hair_proc (const PtProcRec& rec, const SkPoint devPts[], int count, SkBlitter* blitter)
{
...
static SkXRect make_xrect (const SkRect& r)
{
    ...
    return {
        SkScalarToFixed (r.fLeft), SkScalarToFixed (r.fTop), SkScalarToFixed (r.fRight), SkScalarToFixed (r.fBottom)
    };
}
...
bool PtProcRec::init (SkCanvas::PointMode mode, const SkPaint& paint, const SkMatrix* matrix, const SkRasterClip* rc)
{
    ...
    else if (paint.getStrokeCap() != SkPaint::kRound_Cap && matrix->isScaleTranslate() && SkCanvas::kPoints_PointMode == mode)
    {
...
void SkDraw::drawPoints (SkCanvas::PointMode mode, size_t count, const SkPoint pts[], const SkPaint& paint, SkBaseDevice* device) const
{
            ...
            case SkCanvas::kPoints_PointMode:
            {
                ...
                if (newPaint.getStrokeCap() == SkPaint::kRound_Cap)
                {
                    if (device)
                    {
                        ...
                        {
                            SkRect r = SkRect::MakeLTRB (pts[i].fX - radius, pts[i].fY - radius, pts[i].fX + radius, pts[i].fY + radius);
...
static bool easy_rect_join (const SkPaint& paint, const SkMatrix& matrix, SkPoint* strokeSize)
{
                                          ...
                                          SkPoint* strokeSize)
{
    ...
    if (paint.getPathEffect() || paint.getMaskFilter() || ! matrix.rectStaysRect() || SkPaint::kStrokeAndFill_Style == style)
    {
...
static void draw_rect_as_path (const SkDraw& orig, const SkRect& prePaintRect, const SkPaint& paint, const SkMatrixProvider* matrixProvider)
{
    SkDraw draw (orig);
...
void SkDraw::drawRect (const SkRect& prePaintRect, const SkPaint& paint, const SkMatrix* paintMatrix, const SkRect* postPaintRect) const
{
    SkDEBUGCODE (this->validate();)
...
void SkDraw::drawDevMask (const SkMask& srcM, const SkPaint& paint) const
{
    ...
    SkMask dstM;
    if (paint.getMaskFilter() && as_MFB (paint.getMaskFilter())->filterMask (&dstM, srcM, fMatrixProvider->localToDevice(), nullptr))
    {
        mask = &dstM;
    }
...
void SkDraw::drawDevPath (const SkPath& devPath, const SkPaint& paint, bool drawCoverage, SkBlitter* customBlitter, bool doFill) const
{
...
void SkDraw::drawPath (const SkPath& origSrcPath, const SkPaint& origPaint, const SkMatrix* prePathMatrix, bool pathIsMutable, bool drawCoverage, SkBlitter* customBlitter) const
{
    SkDEBUGCODE (this->validate();)
    ...
    if (paint->getPathEffect() || paint->getStyle() != SkPaint::kFill_Style)
    {
        ...
        doFill = paint->getFillPath (*pathPtr, tmpPath, cullRectPtr, ComputeResScaleForStroking (fMatrixProvider->localToDevice()));
        pathPtr = tmpPath;
    }
...
void SkDraw::drawBitmapAsMask (const SkBitmap& bitmap, const SkPaint& paint) const
{
    ...
    { // need to xform the bitmap first
        SkRect r;
        SkMask mask;
...
void SkDraw::drawBitmap (const SkBitmap& bitmap, const SkMatrix& prematrix, const SkRect* dstBounds, const SkPaint& origPaint) const
{
    SkDEBUGCODE (this->validate();)
        ...
        && SkTreatAsSprite (matrix, bitmap.dimensions(), *paint))
    {
        ...
        if (clipHandlesSprite (*fRC, ix, iy, pmap))
        {
            ...
            SkBlitter* blitter = SkBlitter::ChooseSprite (fDst, *paint, pmap, ix, iy, &allocator, fRC->clipShader());
            if (blitter)
            {
...
void SkDraw::drawSprite (const SkBitmap& bitmap, int x, int y, const SkPaint& origPaint) const
{
    ...
    if (nullptr == paint.getColorFilter() && clipHandlesSprite (*fRC, x, y, pmap))
    {
        // blitter will be owned by the allocator.
        SkSTArenaAlloc<kSkBlitterContextSize> allocator;
        SkBlitter* blitter = SkBlitter::ChooseSprite (fDst, paint, pmap, x, y, &allocator, fRC->clipShader());
        if (blitter)
        {
...
bool SkDraw::ComputeMaskBounds (const SkRect& devPathBounds, const SkIRect* clipBounds, const SkMaskFilter* filter, const SkMatrix* filterMatrix, SkIRect* bounds)
{
...
bool SkDraw::DrawToMask (const SkPath& devPath, const SkIRect* clipBounds, const SkMaskFilter* filter, const SkMatrix* filterMatrix, SkMask* mask, SkMask::CreateMode mode, SkStrokeRec::InitStyle style)
{
    if (devPath.isEmpty())
    {
        return false;
    ...
    {
        if (! ComputeMaskBounds (devPath.getBounds(), clipBounds, filter, filterMatrix, &mask->fBounds))
            return false;
    }
