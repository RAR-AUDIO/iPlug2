...
{
    if (config == SkYUVAInfo::PlaneConfig::kUnknown || subsampling == SkYUVAInfo::Subsampling::kUnknown)
    {
        return false;
    }
    return subsampling == SkYUVAInfo::Subsampling::k444 || (config != SkYUVAInfo::PlaneConfig::kYUV && config != SkYUVAInfo::PlaneConfig::kYUVA && config != SkYUVAInfo::PlaneConfig::kUYV && config != SkYUVAInfo::PlaneConfig::kUYVA);
}
                                 ...
                                 SkISize planeDimensions[SkYUVAInfo::kMaxPlanes])
{
    ...
    switch (subsampling)
    {
        ...
        case Subsampling::k420:
            uvSize = { down2 (w), down2 (h) };
            break;
        case Subsampling::k440:
            uvSize = { w, down2 (h) };
            break;
        case Subsampling::k411:
            uvSize = { down4 (w), h };
            break;
        case Subsampling::k410:
            uvSize = { down4 (w), down2 (h) };
            break;
    }
    switch (planeConfig)
    {
...
SkYUVAInfo::SkYUVAInfo (SkISize dimensions,
                        ...
                        Siting sitingX,
                        Siting sitingY)
    : fDimensions (dimensions), fPlaneConfig (planeConfig), fSubsampling (subsampling), fYUVColorSpace (yuvColorSpace), fOrigin (origin), fSitingX (sitingX), fSitingY (sitingY)
{
    if (fDimensions.isEmpty() || ! is_plane_config_compatible_with_subsampling (planeConfig, subsampling))
    {
        *this = {};
...
{
    SkASSERT (! this->isValid() || is_plane_config_compatible_with_subsampling (fPlaneConfig, fSubsampling));
    switch (fPlaneConfig)
    {
        case PlaneConfig::kUnknown:
            return PlanarConfig::kUnknown;
        ...
        case PlaneConfig::kY_U_V:
            switch (fSubsampling)
            {
                ...
                case Subsampling::k444:
                    return PlanarConfig::kY_U_V_444;
                case Subsampling::k422:
                    return PlanarConfig::kY_U_V_422;
                case Subsampling::k420:
                    return PlanarConfig::kY_U_V_420;
                case Subsampling::k440:
                    return PlanarConfig::kY_U_V_440;
                case Subsampling::k411:
                    return PlanarConfig::kY_U_V_411;
                case Subsampling::k410:
                    return PlanarConfig::kY_U_V_410;
            }
            SkUNREACHABLE;
...
{
    return fPlaneConfig == that.fPlaneConfig && fSubsampling == that.fSubsampling && fYUVColorSpace == that.fYUVColorSpace && fDimensions == that.fDimensions && fSitingX == that.fSitingX && fSitingY == that.fSitingY && fOrigin == that.fOrigin;
}
