...
inline static SkAlpha ScalarToAlpha (SkScalar a)
{
    ...
    return alpha > 247 ? 0xFF : alpha < 8 ? 0
                                          : alpha;
}
...
void SkBlitter::blitFatAntiRect (const SkRect& rect)
{
    ...
    if (bounds.height() > 2)
    {
        this->blitAntiRect (bounds.fLeft, bounds.fTop + 1, bounds.width() - 2, bounds.height() - 2, ScalarToAlpha (partialL), ScalarToAlpha (partialR));
    }
...
static inline void bits_to_runs (SkBlitter* blitter, int x, int y, const uint8_t bits[], uint8_t left_mask, ptrdiff_t rowBytes, uint8_t right_mask)
{
    int inFill = 0;
    int pos = 0;
...
void SkRectClipBlitter::blitAntiRect (int left, int y, int width, int height, SkAlpha leftAlpha, SkAlpha rightAlpha)
{
    SkIRect r;
    ...
    if (r.intersect (fClipRect))
    {
        ...
        {
            fBlitter->blitAntiRect (r.fLeft, r.fTop, r.width() - 2, r.height(), leftAlpha, rightAlpha);
        }
    }
}
...
void SkRgnClipBlitter::blitAntiRect (int x, int y, int width, int height, SkAlpha leftAlpha, SkAlpha rightAlpha)
{
    ...
    while (! iter.done())
    {
        ...
        else if (1 == r.width())
        {
            if (r.fLeft == x)
            {
                fBlitter->blitV (r.fLeft, r.fTop, r.height(), effectiveLeftAlpha);
            }
            else
            {
                SkASSERT (r.fLeft == x + width + 1);
                fBlitter->blitV (r.fLeft, r.fTop, r.height(), effectiveRightAlpha);
            }
        }
        else
        {
            fBlitter->blitAntiRect (r.fLeft, r.fTop, r.width() - 2, r.height(), effectiveLeftAlpha, effectiveRightAlpha);
        }
        iter.next();
    }
}
...
bool SkBlitter::UseRasterPipelineBlitter (const SkPixmap& device, const SkPaint& paint, const SkMatrix& matrix)
{
    ...
    if (device.alphaType() == kUnpremul_SkAlphaType || paint.getBlendMode() > SkBlendMode::kLastCoeffMode || (mf && mf->getFormat() == SkMask::k3D_Format))
    {
        return true;
    }
                              ...
                              sk_sp<SkShader> clipShader)
{
    ...
    if (gUseSkVMBlitter)
    {
        if (auto blitter = SkCreateSkVMBlitter (device, *paint, matrixProvider, alloc, clipShader))
        {
            return blitter;
        }
    }
    ...
    auto create_SkRP_or_SkVMBlitter = [&]() -> SkBlitter* {
        if (auto blitter = SkCreateRasterPipelineBlitter (device, *paint, matrixProvider, alloc, clipShader))
        {
            return blitter;
        }
        if (auto blitter = SkCreateSkVMBlitter (device, *paint, matrixProvider, alloc, clipShader))
    ...
    SkASSERT (device.colorType() == kN32_SkColorType || device.colorType() == kRGB_565_SkColorType);
    ...
    if (paint->getShader())
    {
        shaderContext = as_SB (paint->getShader())->makeContext ({ *paint, ctm, nullptr, device.colorType(), device.colorSpace() }, alloc);
...
SkShaderBlitter::SkShaderBlitter (const SkPixmap& device, const SkPaint& paint, SkShaderBase::Context* shaderContext)
...
void SkRectClipCheckBlitter::blitAntiRect (int x, int y, int width, int height, SkAlpha leftAlpha, SkAlpha rightAlpha)
{
