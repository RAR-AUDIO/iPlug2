...
static void decal_nofilter_scale (uint32_t dst[], SkFixed fx, SkFixed dx, int count)
{
    ...
    {
        case 2:
            ((uint16_t*) dst)[1] = SkToU16 ((fx + dx) >> 16);
            [[fallthrough]];
        case 1:
            ((uint16_t*) dst)[0] = SkToU16 ((fx + 0) >> 16);
    }
}
                          ...
                          int y)
{
    ...
    if (tryDecal && (unsigned) SkFractionalIntToInt (fx) < maxX && (unsigned) SkFractionalIntToInt (fx + dx * (count - 1)) < maxX)
    {
        while (count-- > 0)
        {
...
static const SkBitmapProcState::MatrixProc RepeatX_RepeatY_Procs[] = {
    ...
    filter_scale<repeat, repeat, extract_low_bits_general, false>,
    nofilter_affine<repeat, repeat>,
    filter_affine<repeat, repeat, extract_low_bits_general>
};
static const SkBitmapProcState::MatrixProc MirrorX_MirrorY_Procs[] = {
    nofilter_scale<mirror, mirror, false>,
    filter_scale<mirror, mirror, extract_low_bits_general, false>,
    nofilter_affine<mirror, mirror>,
    filter_affine<mirror, mirror, extract_low_bits_general>,
};
        ...
        if (translate_only_matrix && kNone_SkFilterQuality == fFilterQuality)
        {
            switch (fTileModeX)
            {
                ...
                case SkTileMode::kClamp:
                    return clampx_nofilter_trans<int_clamp>;
                case SkTileMode::kRepeat:
                    return repeatx_nofilter_trans<int_repeat>;
                case SkTileMode::kMirror:
                    return mirrorx_nofilter_trans<int_mirror>;
            }
        }
