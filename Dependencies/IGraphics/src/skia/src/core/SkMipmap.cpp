...
struct ColorTypeFilter_1010102
{
    ...
    {
        return (((x) &0x3ff)) | (((x >> 10) & 0x3ff) << 20) | (((x >> 20) & 0x3ff) << 40) | (((x >> 30) & 0x3) << 60);
    }
    static uint32_t Compact (uint64_t x)
    {
        return (((x) &0x3ff)) | (((x >> 20) & 0x3ff) << 10) | (((x >> 40) & 0x3ff) << 20) | (((x >> 60) & 0x3) << 30);
    }
};
...
SkMipmap* SkMipmap::Build (const SkPixmap& src, SkDiscardableFactoryProc fact, bool computeContents)
{
...
bool SkMipmap::validForRootLevel (const SkImageInfo& root) const
{
    ...
    if (pm.width() != std::max (1, dimension.width() >> 1) || pm.height() != std::max (1, dimension.height() >> 1))
    {
        return false;
    }
    ...
    {
        const SkPixmap& pm = fLevels[0].fPixmap;
        if (pm.colorType() != root.colorType() || pm.alphaType() != root.alphaType())
            return false;
    }
    return true;
}
