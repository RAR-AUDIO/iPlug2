...
static inline SkFixed quick_div (SkFDot6 a, SkFDot6 b)
{
    ...
    if (abs_b >= (1 << kMinBits) && abs_b < kInverseTableSize && abs_a < kMaxAbsA)
    {
        ...
        SkASSERT (
            (SkFDot6Div (a, b) == 0 && ourAnswer == 0) || SkFixedDiv (SkAbs32 (SkFDot6Div (a, b) - ourAnswer), SkAbs32 (SkFDot6Div (a, b))) <= 1 << 10);
        return ourAnswer;
    }
...
bool SkAnalyticEdge::setLine (const SkPoint& p0, const SkPoint& p1)
{
    ...
    fUpperY = y0;
    fLowerY = y1;
    fDY = dx == 0 || slope == 0 ? SK_MaxS32 : absSlope < kInverseTableSize ? quick_inverse (absSlope)
...
bool SkAnalyticQuadraticEdge::updateQuadratic()
{
    ...
    do
    {
        ...
        if (--count > 0)
        {
            ...
            { // only snap when dy is large enough
