...
SkData::SkData (size_t size)
    : fReleaseProc (nullptr), fReleaseProcContext (nullptr), fPtr ((const char*) (this + 1)), fSize (size)
{
}
...
sk_sp<SkData> SkData::MakeSubset (const SkData* src, size_t offset, size_t length)
{
    ...
    return sk_sp<SkData> (new SkData (src->bytes() + offset, length, sk_dataref_releaseproc, const_cast<SkData*> (src)));
}
