...
bool SkBitmapProcState::init (const SkMatrix& inv, const SkPaint& paint)
{
    ...
    if (kLow_SkFilterQuality == fFilterQuality && (! valid_for_filtering (fPixmap.width() | fPixmap.height()) || integral_translate_only))
    {
...
bool SkBitmapProcState::chooseProcs()
{
    ...
    SkASSERT (fPixmap.alphaType() == kPremul_SkAlphaType || fPixmap.alphaType() == kOpaque_SkAlphaType);
                                       ...
                                       int count)
{
    ...
    else
    {
...
#ifdef SK_DEBUG
        {
            ...
            if (! s.fInvMatrix.isTranslate() && (SkTileMode::kClamp != s.fTileModeX || SkTileMode::kClamp != s.fTileModeY))
            {
...
SkBitmapProcState::ShaderProc32 SkBitmapProcState::chooseShaderProc32()
{
    ...
    {
        if (kNone_SkFilterQuality == fFilterQuality && fInvMatrix.isTranslate() && ! this->setupForTranslate())
        {
...
static void check_scale_nofilter (uint32_t bitmapXY[], int count, unsigned mx, unsigned my)
{
    unsigned y = *bitmapXY++;
    SkASSERT (y < my);
...
static void check_scale_filter (uint32_t bitmapXY[], int count, unsigned mx, unsigned my)
{
