...
void SkPathRef::writeToBuffer (SkWBuffer* buffer) const
{
    ...
    // don't read it.
    int32_t packed = ((fIsFinite & 1) << kIsFinite_SerializationShift) | (fSegmentMask << kSegmentMask_SerializationShift);
...
{
    return uint32_t (5 * sizeof (uint32_t) + fVerbs.bytes() + fPoints.bytes() + fConicWeights.bytes() + sizeof (SkRect));
}
...
{
    SkDEBUGCODE (this->validate();) this->resetToSize (ref.fVerbs.count(), ref.fPoints.count(), ref.fConicWeights.count(), additionalReserveVerbs, additionalReservePoints);
    fVerbs = ref.fVerbs;
    fPoints = ref.fPoints;
...
std::tuple<SkPoint*, SkScalar*> SkPathRef::growForVerbsInPath (const SkPathRef& path)
{
    ...
    SkDEBUGCODE (this->validate();) return { pts, weights };
}
    ...
    while ((verb = iter.next (pts)) != SkPath::kDone_Verb)
    {
        if (SkPath::kConic_Verb == verb)
        {
            ...
            SkRRect::Corner corner =
                pts[1].fX == bounds.fLeft ? pts[1].fY == bounds.fTop ? SkRRect::kUpperLeft_Corner : SkRRect::kLowerLeft_Corner : pts[1].fY == bounds.fTop ? SkRRect::kUpperRight_Corner
