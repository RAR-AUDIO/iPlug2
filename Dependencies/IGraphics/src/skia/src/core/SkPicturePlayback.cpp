...
#define BREAK_ON_READ_ERROR(r) \
    ...
    switch (op)
    {
        ...
        case DRAW_EDGEAA_IMAGE_SET:
        {
            ...
            if (! reader->validate ((maxMatrixIndex + 1) <= matrixCount) || ! reader->validate (SkSafeMath::Mul (matrixCount, kMatrixSize) <= reader->available()))
            {
            ...
            canvas->experimental_DrawEdgeAAImageSet (set.get(), cnt, dstClips, matrices.begin(), paint, constraint);
        }
        break;
        case DRAW_IMAGE:
        {
        ...
        case DRAW_VERTICES_OBJECT:
        {
            ...
            if (vertices)
            { // TODO: read error if vertices == null?
