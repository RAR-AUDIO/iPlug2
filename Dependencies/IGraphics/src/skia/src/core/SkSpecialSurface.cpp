...
SkSpecialSurface::SkSpecialSurface (const SkIRect& subset,
    ...
    : fProps (SkSurfacePropsCopyOrDefault (props).flags(), kUnknown_SkPixelGeometry), fSubset (subset)
{
...
sk_sp<SkSpecialSurface> SkSpecialSurface::MakeFromBitmap (const SkIRect& subset, SkBitmap& bm, const SkSurfaceProps* props)
{
...
public:
    SkSpecialSurface_Gpu (GrRecordingContext* context,
                          ...
                          int height,
                          const SkIRect& subset)
        : INHERITED (subset, &renderTargetContext->surfaceProps()), fReadView (renderTargetContext->readSurfaceView())
    {
        auto device = SkGpuDevice::Make (context, std::move (renderTargetContext), SkGpuDevice::kUninit_InitContents);
        if (! device)
        {
            return;
        }
                                                            ...
                                                            const SkSurfaceProps* props)
{
    ...
    auto renderTargetContext = GrRenderTargetContext::Make (
        context, colorType, std::move (colorSpace), SkBackingFit::kApprox, { width, height }, 1, GrMipmapped::kNo, GrProtected::kNo, kBottomLeft_GrSurfaceOrigin, SkBudgeted::kYes, props);
    ...
    return sk_make_sp<SkSpecialSurface_Gpu> (context, std::move (renderTargetContext), width, height, subset);
}
