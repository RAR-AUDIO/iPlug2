...
SkScalar SkStrokeRec::GetInflationRadius (const SkPaint& paint, SkPaint::Style style)
{
    ...
    return GetInflationRadius (paint.getStrokeJoin(), paint.getStrokeMiter(), paint.getStrokeCap(), width);
}

SkScalar SkStrokeRec::GetInflationRadius (SkPaint::Join join, SkScalar miterLimit, SkPaint::Cap cap, SkScalar strokeWidth)
{
    if (strokeWidth < 0)
    { // fill
        return 0;
