...
skvm::Color SkColorFilterBase::program (skvm::Builder* p, skvm::Color c, SkColorSpace* dstCS, skvm::Uniforms* uniforms, SkArenaAlloc* alloc) const
{
...
SkColor4f SkColorFilter::filterColor4f (const SkColor4f& origSrcColor, SkColorSpace* srcCS, SkColorSpace* dstCS) const
{
    SkColor4f color = origSrcColor;
    SkColorSpaceXformSteps (srcCS, kUnpremul_SkAlphaType, dstCS, kPremul_SkAlphaType).apply (color.vec());
...
public:
    ...
    SkSRGBGammaColorFilter (Direction dir) : fDir (dir), fSteps ([&] {
                                                 ...
                                                 {
                                                     return SkColorSpaceXformSteps { sk_srgb_linear_singleton(), kUnpremul_SkAlphaType, sk_srgb_singleton(), kUnpremul_SkAlphaType };
                                                 }
                                                 else
                                                 {
                                                     return SkColorSpaceXformSteps { sk_srgb_singleton(), kUnpremul_SkAlphaType, sk_srgb_linear_singleton(), kUnpremul_SkAlphaType };
                                                 }
                                             }()) {}
...
public:
                                    ...
                                    const GrColorInfo& dstColorInfo) const override
    {
        ...
        return GrFPSuccess (GrMixerEffect::Make (std::move (inputFP), std::move (fp0), std::move (fp1), fWeight));
    }
#endif
...
sk_sp<SkColorFilter> SkColorFilters::Lerp (float weight, sk_sp<SkColorFilter> cf0, sk_sp<SkColorFilter> cf1)
{
    if (! cf0 && ! cf1)
    {
        return nullptr;
