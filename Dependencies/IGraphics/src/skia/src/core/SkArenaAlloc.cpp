...
{
    return firstHeapAllocation > 0 ? firstHeapAllocation : blockSize > 0 ? blockSize
                                                                         : 1024;
}

SkArenaAlloc::SkArenaAlloc (char* block, size_t size, size_t firstHeapAllocation)
    : fDtorCursor { block }, fCursor { block }, fEnd { block + ToU32 (size) }, fNextHeapAlloc { first_allocated_block (ToU32 (size), ToU32 (firstHeapAllocation)) }, fYetNextHeapAlloc { fNextHeapAlloc }
{
...
SkArenaAllocWithReset::SkArenaAllocWithReset (char* block,
    ...
    : SkArenaAlloc (block, size, firstHeapAllocation), fFirstBlock { block }, fFirstSize { to_uint32_t (size) }, fFirstHeapAllocationSize { to_uint32_t (firstHeapAllocation) } {}
