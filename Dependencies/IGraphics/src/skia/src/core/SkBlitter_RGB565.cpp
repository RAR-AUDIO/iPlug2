...
static void D16_S32X_src_coverage (uint16_t dst[], const SkPMColor src[], int count, uint8_t coverage)
{
    switch (coverage)
    {
        case 0:
...
static void D16_S32A_srcover_coverage (uint16_t dst[], const SkPMColor src[], int count, uint8_t coverage)
{
    switch (coverage)
    {
        case 0:
...
bool SkRGB565_Shader_Blitter::Supports (const SkPixmap& device, const SkPaint& paint)
{
    ...
    if (paint.getBlendMode() != SkBlendMode::kSrcOver && paint.getBlendMode() != SkBlendMode::kSrc)
    {
        return false;
    }
    if (paint.isDither())
...
void SkRGB565_Shader_Blitter::blitAntiH (int x, int y, const SkAlpha coverage[], const int16_t runs[])
{
    SkPMColor* span = fBuffer;
    uint16_t* device = fDevice.writable_addr16 (x, y);
