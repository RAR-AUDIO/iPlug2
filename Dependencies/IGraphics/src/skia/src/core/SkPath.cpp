...
SkPath::SkPath (sk_sp<SkPathRef> pr, SkPathFillType ft, bool isVolatile, SkPathConvexity ct, SkPathFirstDirection firstDirection)
    : fPathRef (std::move (pr)), fLastMoveToIndex (INITIAL_LASTMOVETOINDEX_VALUE), fConvexity ((uint8_t) ct), fFirstDirection ((uint8_t) firstDirection), fFillType ((unsigned) ft), fIsVolatile (isVolatile)
{
}
...
bool SkPath::isInterpolatable (const SkPath& compare) const
{
    ...
    return fPathRef->fPoints.count() == compare.fPathRef->fPoints.count() && fPathRef->fVerbs == compare.fPathRef->fVerbs && fPathRef->fConicWeights == compare.fPathRef->fConicWeights;
}
...
{
    SkDEBUGCODE (this->validate();) if (inc > 0)
    {
...
SkPath& SkPath::rConicTo (SkScalar dx1, SkScalar dy1, SkScalar dx2, SkScalar dy2, SkScalar w)
{
    this->injectMoveToIfNeeded(); // This can change the result of this->getLastPt().
    SkPoint pt;
    this->getLastPt (&pt);
...
SkPath& SkPath::rCubicTo (SkScalar x1, SkScalar y1, SkScalar x2, SkScalar y2, SkScalar x3, SkScalar y3)
{
    this->injectMoveToIfNeeded(); // This can change the result of this->getLastPt().
    SkPoint pt;
    this->getLastPt (&pt);
    return this->cubicTo (pt.fX + x1, pt.fY + y1, pt.fX + x2, pt.fY + y2, pt.fX + x3, pt.fY + y3);
}
...
SkPath& SkPath::addRoundRect (const SkRect& rect, const SkScalar radii[], SkPathDirection dir)
{
    SkRRect rrect;
...
SkPath& SkPath::addRRect (const SkRRect& rrect, SkPathDirection dir, unsigned startIndex)
{
    ...
    SkDEBUGCODE (fPathRef->validate();) return *this;
}
...
SkPath& SkPath::arcTo (const SkRect& oval, SkScalar startAngle, SkScalar sweepAngle, bool forceMoveTo)
{
    ...
    auto addPt = [&forceMoveTo, this] (const SkPoint& pt) {
        ...
        else if (! this->getLastPt (&lastPt) || ! SkScalarNearlyEqual (lastPt.fX, pt.fX) || ! SkScalarNearlyEqual (lastPt.fY, pt.fY))
        {
            this->lineTo (pt);
        }
    };
...
SkPath& SkPath::arcTo (SkScalar rx, SkScalar ry, SkScalar angle, SkPath::ArcSize arcLarge, SkPathDirection arcSweep, SkScalar x, SkScalar y)
{
    ...
    bool expectIntegers = SkScalarNearlyZero (SK_ScalarPI / 2 - SkScalarAbs (thetaWidth)) && scalar_is_integer (rx) && scalar_is_integer (ry) && scalar_is_integer (x) && scalar_is_integer (y);
...
SkPath& SkPath::rArcTo (SkScalar rx, SkScalar ry, SkScalar xAxisRotate, SkPath::ArcSize largeArc, SkPathDirection sweep, SkScalar dx, SkScalar dy)
{
...
SkPath& SkPath::addArc (const SkRect& oval, SkScalar startAngle, SkScalar sweepAngle)
{
    ...
    if (sweepAngle >= kFullCircleAngle || sweepAngle <= -kFullCircleAngle)
    {
        ...
        if (SkScalarNearlyEqual (error, 0))
        {
            ...
            return this->addOval (oval, sweepAngle > 0 ? SkPathDirection::kCW : SkPathDirection::kCCW, (unsigned) startIndex);
        }
    }
...
void SkPath::transform (const SkMatrix& matrix, SkPath* dst, SkApplyPerspectiveClip pc) const
{
    ...
    if (matrix.hasPerspective())
    {
        ...
        if (pc == SkApplyPerspectiveClip::kYes && SkPathPriv::PerspectiveClip (*this, matrix, &clipped))
        {
            src = &clipped;
        }
        ...
        while ((verb = iter.next (pts)) != kDone_Verb)
        {
            switch (verb)
            {
                ...
                case kQuad_Verb:
                    ...
                    tmp.conicTo (pts[1], pts[2], SkConic::TransformW (pts, SK_Scalar1, matrix));
                    break;
                case kConic_Verb:
    ...
    else
    {
        ...
        {
            SkScalar det2x2 =
                matrix.get (SkMatrix::kMScaleX) * matrix.get (SkMatrix::kMScaleY) - matrix.get (SkMatrix::kMSkewX) * matrix.get (SkMatrix::kMSkewY);
            if (det2x2 < 0)
            {
...
struct Convexicator
{
    ...
    bool addPt (const SkPoint& pt)
    {
        ...
        fCurrPt = pt;
        if (fPriorPt == fLastPt)
        { // should only be true for first non-zero vector
...
private:
    DirChange directionChange (const SkVector& curVec)
    {
        ...
        if (almost_equal (largest, largest + cross))
        {
            ...
            if (SkScalarNearlyZero (SkPointPriv::LengthSqd (fLastVec), nearlyZeroSqd) || SkScalarNearlyZero (SkPointPriv::LengthSqd (curVec), nearlyZeroSqd))
            {
                return kUnknown_DirChange;
            }
...
SkPathConvexity SkPath::computeConvexity() const
{
    ...
    if (this->getFirstDirection() == SkPathFirstDirection::kUnknown)
    {
        ...
        {
            return setComputedConvexity (state.reversals() < 3 ? SkPathConvexity::kConvex : SkPathConvexity::kConcave);
        }
...
bool SkPathPriv::IsSimpleRect (const SkPath& path, bool isSimpleFill, SkRect* rect, SkPathDirection* direction, unsigned* start)
{
    ...
    if (rectPts[0].fX == rectPts[3].fX && rectPts[1].fX == rectPts[2].fX && rectPts[0].fY == rectPts[1].fY && rectPts[3].fY == rectPts[2].fY)
    {
    ...
    else if (rectPts[0].fY == rectPts[3].fY && rectPts[1].fY == rectPts[2].fY && rectPts[0].fX == rectPts[1].fX && rectPts[3].fX == rectPts[2].fX)
    {
        // Make sure it has non-zero width and height
    ...
    unsigned sortFlags =
        ((rectPts[0].fX < rectPts[2].fX) ? 0b00 : 0b01) | ((rectPts[0].fY < rectPts[2].fY) ? 0b00 : 0b10);
    switch (sortFlags)
    {
        case 0b00:
...
void SkPathPriv::CreateDrawArcPath (SkPath* path, const SkRect& oval, SkScalar startAngle, SkScalar sweepAngle, bool useCenter, bool isFillNoPathEffect)
{
...
{
    return exact ? p1 == p2 && p2 == p3 : SkPointPriv::EqualsWithinTolerance (p1, p2) && SkPointPriv::EqualsWithinTolerance (p2, p3);
}

bool SkPath::IsCubicDegenerate (const SkPoint& p1, const SkPoint& p2, const SkPoint& p3, const SkPoint& p4, bool exact)
{
    return exact ? p1 == p2 && p2 == p3 && p3 == p4 : SkPointPriv::EqualsWithinTolerance (p1, p2) && SkPointPriv::EqualsWithinTolerance (p2, p3) && SkPointPriv::EqualsWithinTolerance (p3, p4);
}
...
SkPath SkPath::Make (const SkPoint pts[], int pointCount, const uint8_t vbs[], int verbCount, const SkScalar ws[], int wCount, SkPathFillType ft, bool isVolatile)
{
    if (verbCount <= 0)
    {
        return SkPath();
...
SkPath SkPath::Polygon (const SkPoint pts[], int count, bool isClosed, SkPathFillType ft, bool isVolatile)
{
    return SkPathBuilder().addPolygon (pts, count, isClosed).setFillType (ft).setIsVolatile (isVolatile).detach();
}
...
bool SkPathPriv::IsRectContour (const SkPath& path, bool allowPartial, int* currVerb, const SkPoint** ptsPtr, bool* isClosed, SkPathDirection* direction, SkRect* rect)
{
    ...
    if (direction)
    {
        *direction = directions[0] == ((directions[1] + 1) & 3) ? SkPathDirection::kCW : SkPathDirection::kCCW;
    }
    return true;
}
...
{
    SkDEBUGCODE (path.validate();) int currVerb = 0;
...
struct SkHalfPlane
{
    ...
    bool normalize()
    {
        ...
        if (! sk_float_isfinite (a) || ! sk_float_isfinite (b) || ! sk_float_isfinite (c) || (a == 0 && b == 0))
        {
            fA = fB = 0;
            fC = SK_Scalar1;
            return false;
...
static SkPath clip (const SkPath& path, const SkHalfPlane& plane)
{
    ...
    mx.setAll (plane.fB, plane.fA, p0.fX, -plane.fA, plane.fB, p0.fY, 0, 0, 1);
    if (! mx.invert (&inv))
    {
        return SkPath();
    }
    ...
    SkEdgeClipper::ClipPath (
        rotated, clip, false, [] (SkEdgeClipper* clipper, bool newCtr, void* ctx) {
            Rec* rec = (Rec*) ctx;
...
bool SkPathPriv::PerspectiveClip (const SkPath& path, const SkMatrix& matrix, SkPath* clippedPath)
{
    ...
    if (plane.normalize())
    {
        switch (plane.test (path.getBounds()))
        {
            ...
            break;
            default:
                break; // handled outside of the switch
        }
    }
