...
SkColorSpaceXformSteps::SkColorSpaceXformSteps (const SkColorSpace* src, SkAlphaType srcAT, const SkColorSpace* dst, SkAlphaType dstAT)
{
    ...
    if (this->flags.linearize && ! this->flags.gamut_transform && this->flags.encode && src->transferFnHash() == dst->transferFnHash())
    {
#ifdef SK_DEBUG
...
void SkColorSpaceXformSteps::apply (float* rgba) const
{
    ...
    if (flags.gamut_transform)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            rgba[i] = src_to_dst_matrix[i] * temp[0] + src_to_dst_matrix[3 + i] * temp[1] + src_to_dst_matrix[6 + i] * temp[2];
        }
    }
    if (flags.encode)
    {
...
skvm::Color sk_program_transfer_fn (skvm::Builder* p, skvm::Uniforms* uniforms, const skcms_TransferFunction& tf, skvm::Color c)
{
    ...
    auto apply = [&] (skvm::F32 v) -> skvm::F32 {
        ...
        switch (classify_transfer_fn (tf))
        {
            ...
            case sRGBish_TF:
                v = select (v <= D, C * v + F, approx_powf (A * v + B, G) + E);
                break;
...
skvm::Color SkColorSpaceXformSteps::program (skvm::Builder* p, skvm::Uniforms* uniforms, skvm::Color c) const
{
    if (flags.unpremul)
    {
        c = unpremul (c);
