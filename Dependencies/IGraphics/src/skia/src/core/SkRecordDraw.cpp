...
void SkRecordPartialDraw (const SkRecord& record, SkCanvas* canvas, SkPicture const* const drawablePicts[], int drawableCount, int start, int stop, const SkMatrix& initialCTM)
{
    SkAutoCanvasRestore saveRestore (canvas, true /*save now, restore at exit*/);
...
DRAW (Save, save());
DRAW (SaveLayer, saveLayer (SkCanvas::SaveLayerRec (r.bounds, r.paint, r.backdrop.get(), r.saveLayerFlags)));
...
template <>
void Draw::draw (const DrawBehind& r)
{
...
template <>
void Draw::draw (const DrawImageLattice& r)
{
...
DRAW (DrawAtlas, drawAtlas (r.atlas.get(), r.xforms, r.texs, r.colors, r.count, r.mode, r.cull, r.paint));
...
DRAW (DrawEdgeAAQuad, experimental_DrawEdgeAAQuad (r.rect, r.clip, r.aa, r.color, r.mode));
DRAW (DrawEdgeAAImageSet, experimental_DrawEdgeAAImageSet (r.set.get(), r.count, r.dstClips, r.preViewMatrices, r.paint, r.constraint));
