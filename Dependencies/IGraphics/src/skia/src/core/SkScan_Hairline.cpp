...
void SkScan::HairLineRgn (const SkPoint array[], int arrayCount, const SkRegion* clip, SkBlitter* origBlitter)
{
    ...
    for (int i = 0; i < arrayCount - 1; ++i)
    {
        ...
        if (clip)
        {
            ...
            clipR.setLTRB (SkIntToFDot6 (bounds.fLeft), SkIntToFDot6 (bounds.fTop), SkIntToFDot6 (bounds.fRight), SkIntToFDot6 (bounds.fBottom));
...
static inline void hairquad (const SkPoint pts[3], const SkRegion* clip, const SkRect* insetClip, const SkRect* outsetClip, SkBlitter* blitter, int level, SkScan::HairRgnProc lineproc)
{
    if (insetClip)
    {
        SkASSERT (outsetClip);
...
static inline void haircubic (const SkPoint pts[4], const SkRegion* clip, const SkRect* insetClip, const SkRect* outsetClip, SkBlitter* blitter, int level, SkScan::HairRgnProc lineproc)
{
    if (insetClip)
    {
        SkASSERT (outsetClip);
...
void hair_path (const SkPath& path, const SkRasterClip& rclip, SkBlitter* blitter, SkScan::HairRgnProc lineproc)
{
    ...
    while (iter != end)
    {
        ...
        if (SkPaint::kButt_Cap != capStyle)
        {
            if (prevVerb == SkPath::kMove_Verb && verb >= SkPath::kLine_Verb && verb <= SkPath::kCubic_Verb)
            {
...
void SkScan::FrameRect (const SkRect& r, const SkPoint& strokeSize, const SkRasterClip& clip, SkBlitter* blitter)
{
...
void SkScan::HairLine (const SkPoint pts[], int count, const SkRasterClip& clip, SkBlitter* blitter)
{
    if (clip.isBW())
    {
...
void SkScan::AntiHairLine (const SkPoint pts[], int count, const SkRasterClip& clip, SkBlitter* blitter)
{
    if (clip.isBW())
    {
