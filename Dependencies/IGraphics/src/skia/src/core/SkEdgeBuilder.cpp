...
int SkEdgeBuilder::build (const SkPath& path, const SkIRect* iclip, bool canCullToTheRight)
{
    ...
    if (iclip)
    {
        ...
        SkEdgeClipper::ClipPath (
            path, clip, canCullToTheRight, [] (SkEdgeClipper* clipper, bool, void* ctx) {
                Rec* rec = (Rec*) ctx;
                SkPoint pts[4];
                SkPath::Verb verb;
                ...
                while ((verb = clipper->next (pts)) != SkPath::kDone_Verb)
                {
                    ...
                    switch (verb)
                    {
                        case SkPath::kLine_Verb:
                            rec->fBuilder->addLine (pts);
                            break;
                        case SkPath::kQuad_Verb:
                            rec->fBuilder->addQuad (pts);
                            break;
                        case SkPath::kCubic_Verb:
                            rec->fBuilder->addCubic (pts);
                            break;
                        default:
                            break;
                    }
