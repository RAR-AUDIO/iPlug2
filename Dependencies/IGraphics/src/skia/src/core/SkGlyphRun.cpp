...
SkGlyphRunList::SkGlyphRunList (const SkGlyphRun& glyphRun, const SkPaint& paint)
    : fGlyphRuns { SkSpan<const SkGlyphRun> { &glyphRun, 1 } }, fOriginalPaint { &paint }, fOriginalTextBlob { nullptr }, fOrigin { SkPoint::Make (0, 0) } {}
...
// -- SkGlyphRunBuilder ----------------------------------------------------------------------------
void SkGlyphRunBuilder::drawTextUTF8 (const SkPaint& paint, const SkFont& font, const void* bytes, size_t byteLength, SkPoint origin)
{
...
void SkGlyphRunBuilder::drawTextBlob (const SkPaint& paint, const SkTextBlob& blob, SkPoint origin, SkBaseDevice* device)
{
    ...
    for (SkTextBlobRunIterator it (&blob); ! it.done(); it.next())
    {
        ...
        else
        {
            ...
            device->drawGlyphRunRSXform (it.font(), it.glyphs(), (const SkRSXform*) it.pos(), it.glyphCount(), origin, paint);
...
void SkGlyphRunBuilder::drawGlyphsWithPositions (const SkPaint& paint, const SkFont& font, SkSpan<const SkGlyphID> glyphIDs, const SkPoint* pos)
{
    if (! glyphIDs.empty())
    {
        this->initialize (glyphIDs.size());
