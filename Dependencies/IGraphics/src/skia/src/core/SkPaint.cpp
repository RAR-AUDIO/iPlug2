...
void SkPaint::setColor (const SkColor4f& color, SkColorSpace* colorSpace)
{
    ...
    SkColorSpaceXformSteps steps { colorSpace, kUnpremul_SkAlphaType, sk_srgb_singleton(), kUnpremul_SkAlphaType };
    fColor4f = color;
...
void SkPaintPriv::Flatten (const SkPaint& paint, SkWriteBuffer& buffer)
{
    ...
    if (paint.getPathEffect() || paint.getShader() || paint.getMaskFilter() || paint.getColorFilter() || paint.getImageFilter())
    {
...
bool SkPaint::getFillPath (const SkPath& src, SkPath* dst, const SkRect* cullRect, SkScalar resScale) const
{
    if (! src.isFinite())
    {
        dst->reset();
