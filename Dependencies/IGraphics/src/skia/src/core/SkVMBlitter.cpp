...
static void build_program (skvm::Builder* p, const Params& params, skvm::Uniforms* uniforms, SkArenaAlloc* alloc)
{
    ...
    skvm::Color src = as_SB (params.shader)->program (p, device, /*local=*/device, paint, params.matrices, /*localM=*/nullptr, params.quality, params.dst, uniforms, alloc);
    SkASSERT (src);
    ...
    switch (params.coverage)
    {
        ...
        case Coverage::MaskLCD16:
        {
            ...
            cov.a = select (src.a < dst.a, min (cov.r, min (cov.g, cov.b)), max (cov.r, max (cov.g, cov.b)));
        }
        break;
    }
    if (params.clip)
    {
        skvm::Color clip = as_SB (params.clip)->program (p, device, /*local=*/device, paint, params.matrices, /*localM=*/nullptr, params.quality, params.dst, uniforms, alloc);
        SkAssertResult (clip);
...
{
    skvm::Color onProgram (skvm::Builder*, skvm::Color c, SkColorSpace*, skvm::Uniforms*, SkArenaAlloc*) const override
    {
        return c;
    }
...
struct DitherShader : public SkShaderBase
{
                           ...
                           SkArenaAlloc* alloc) const override
    {
        ...
        skvm::Color c = as_SB (fShader)->program (p, device, local, paint, matrices, localM, quality, dst, uniforms, alloc);
        if (! c)
        {
            return {};
        }
                                ...
                                sk_sp<SkShader> clip)
{
    ...
    SkColorSpaceXformSteps { sk_srgb_singleton(), kUnpremul_SkAlphaType, device.colorSpace(), kUnpremul_SkAlphaType }
...
public:
    Blitter (const SkPixmap& device,
             ...
             sk_sp<SkShader> clip,
             bool* ok)
        : fDevice (device), fSprite (sprite ? *sprite : SkPixmap {}), fSpriteOffset (spriteOffset), fUniforms (kBlitterUniformsCount), fParams (effective_params (device, sprite, paint, matrices, std::move (clip))), fKey (cache_key (fParams, &fUniforms, &fAlloc, ok))
    {
    }
...
private:
    ...
    void blitMask (const SkMask& mask, const SkIRect& clip) override
    {
        ...
        {
            default:
                SkUNREACHABLE; // ARGB and SDF masks shouldn't make it here.
        ...
        if (program)
        {
            for (int y = clip.top(); y < clip.bottom(); y++)
            {
                ...
                if (program == &fBlitMask3D)
                {
                    ...
                    if (const void* sprite = this->isSprite (x, y))
                    {
                        program->eval (w, fUniforms.buf.data(), dptr, sprite, mptr + 1 * plane, mptr + 2 * plane, mptr + 0 * plane);
                    }
                    else
                    {
                        program->eval (w, fUniforms.buf.data(), dptr, mptr + 1 * plane, mptr + 2 * plane, mptr + 0 * plane);
                    }
                }
                else
                {
...
{
    bool ok = true;
    auto blitter = alloc->make<Blitter> (device, paint, /*sprite=*/nullptr, SkIPoint { 0, 0 }, matrices, std::move (clip), &ok);
                                      ...
                                      sk_sp<SkShader> clip)
{
    ...
    bool ok = true;
    auto blitter = alloc->make<Blitter> (device, paint, &sprite, SkIPoint { left, top }, SkSimpleMatrixProvider { SkMatrix {} }, std::move (clip), &ok);
    return ok ? blitter : nullptr;
}
