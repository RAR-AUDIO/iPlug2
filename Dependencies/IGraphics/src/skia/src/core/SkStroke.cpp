...
public:
    ...
    {
        return fInner.isZeroLengthSincePoint (0) && fOuter.isZeroLengthSincePoint (fFirstOuterPtIndexInContour);
    }
...
private:
    ...
    static ReductionType CheckCubicLinear (const SkPoint cubic[4], SkPoint reduction[3], const SkPoint** tanPtPtr);
    ...
    void conicPerpRay (const SkConic&, SkScalar t, SkPoint* tPt, SkPoint* onPt, SkPoint* tangent) const;
    ...
    void cubicPerpRay (const SkPoint cubic[4], SkScalar t, SkPoint* tPt, SkPoint* onPt, SkPoint* tangent) const;
    ...
    void quadPerpRay (const SkPoint quad[3], SkScalar t, SkPoint* tPt, SkPoint* onPt, SkPoint* tangent) const;
    ...
    ResultType strokeCloseEnough (const SkPoint stroke[3], const SkPoint ray[2], SkQuadConstruct* STROKER_DEBUG_PARAMS (int depth)) const;
...
bool SkPathStroker::preJoinTo (const SkPoint& currPt, SkVector* normal, SkVector* unitNormal, bool currIsLine)
{
    ...
    { // we have a previous segment
        fJoiner (&fOuter, &fInner, fPrevUnitNormal, fPrevPt, *unitNormal, fRadius, fInvMiterLimit, fPrevIsLine, currIsLine);
    }
...
void SkPathStroker::finishContour (bool close, bool currIsLine)
{
    if (fSegmentCount > 0)
    {
        ...
        if (close)
        {
            fJoiner (&fOuter, &fInner, fPrevUnitNormal, fPrevPt, fFirstUnitNormal, fRadius, fInvMiterLimit, fPrevIsLine, currIsLine);
            fOuter.close();
        ...
        { // add caps to start and end
            // cap the end
            fInner.getLastPt (&pt);
            fCapper (&fOuter, fPrevPt, fPrevNormal, pt, currIsLine ? &fInner : nullptr);
            fOuter.reversePathTo (fInner);
            // cap the start
            fCapper (&fOuter, fFirstPt, -fFirstNormal, fFirstOuterPt, fPrevIsLine ? &fInner : nullptr);
            fOuter.close();
        }
        if (! fCusper.isEmpty())
        {
...
SkPathStroker::SkPathStroker (const SkPath& src,
                              ...
                              SkScalar resScale,
                              bool canIgnoreCenter)
    : fRadius (radius), fResScale (resScale), fCanIgnoreCenter (canIgnoreCenter)
{
...
void SkPathStroker::setQuadEndNormal (const SkPoint quad[3], const SkVector& normalAB, const SkVector& unitNormalAB, SkVector* normalBC, SkVector* unitNormalBC)
{
...
void SkPathStroker::setConicEndNormal (const SkConic& conic, const SkVector& normalAB, const SkVector& unitNormalAB, SkVector* normalBC, SkVector* unitNormalBC)
{
    setQuadEndNormal (conic.fPts, normalAB, unitNormalAB, normalBC, unitNormalBC);
}

void SkPathStroker::setCubicEndNormal (const SkPoint cubic[4], const SkVector& normalAB, const SkVector& unitNormalAB, SkVector* normalCD, SkVector* unitNormalCD)
{
...
void SkPathStroker::init (StrokeType strokeType, SkQuadConstruct* quadPts, SkScalar tStart, SkScalar tEnd)
{
...
void SkPathStroker::setRayPts (const SkPoint& tPt, SkVector* dxy, SkPoint* onPt, SkPoint* tangent) const
{
...
void SkPathStroker::conicPerpRay (const SkConic& conic, SkScalar t, SkPoint* tPt, SkPoint* onPt, SkPoint* tangent) const
{
    SkVector dxy;
...
void SkPathStroker::conicQuadEnds (const SkConic& conic, SkQuadConstruct* quadPts) const
{
    if (! quadPts->fStartSet)
    {
        ...
        this->conicPerpRay (conic, quadPts->fStartT, &conicStartPt, &quadPts->fQuad[0], &quadPts->fTangentStart);
        quadPts->fStartSet = true;
    }
    if (! quadPts->fEndSet)
    {
        SkPoint conicEndPt;
        this->conicPerpRay (conic, quadPts->fEndT, &conicEndPt, &quadPts->fQuad[2], &quadPts->fTangentEnd);
        quadPts->fEndSet = true;
    }
}

// Given a cubic and t, return the point on curve, its perpendicular, and the perpendicular tangent.
void SkPathStroker::cubicPerpRay (const SkPoint cubic[4], SkScalar t, SkPoint* tPt, SkPoint* onPt, SkPoint* tangent) const
{
    SkVector dxy;
    SkPoint chopped[7];
...
void SkPathStroker::cubicQuadEnds (const SkPoint cubic[4], SkQuadConstruct* quadPts)
{
    if (! quadPts->fStartSet)
    {
        ...
        this->cubicPerpRay (cubic, quadPts->fStartT, &cubicStartPt, &quadPts->fQuad[0], &quadPts->fTangentStart);
        quadPts->fStartSet = true;
    }
    if (! quadPts->fEndSet)
    {
        SkPoint cubicEndPt;
        this->cubicPerpRay (cubic, quadPts->fEndT, &cubicEndPt, &quadPts->fQuad[2], &quadPts->fTangentEnd);
        quadPts->fEndSet = true;
    }
}

void SkPathStroker::cubicQuadMid (const SkPoint cubic[4], const SkQuadConstruct* quadPts, SkPoint* mid) const
{
    SkPoint cubicMidPt;
...
void SkPathStroker::quadPerpRay (const SkPoint quad[3], SkScalar t, SkPoint* tPt, SkPoint* onPt, SkPoint* tangent) const
{
    SkVector dxy;
                                                       ...
                                                       IntersectRayType intersectRayType STROKER_DEBUG_PARAMS (int depth)) const
{
    ...
    { // if the control point is outside the quad ends
        ...
        {
            return STROKER_RESULT (kDegenerate_ResultType, depth, quadPts, "std::max(dist1=%g, dist2=%g) <= fInvResScaleSquared", dist1, dist2);
        }
    ...
    return STROKER_RESULT (kDegenerate_ResultType, depth, quadPts, "SkScalarNearlyZero(denom=%g)", denom);
}
...
SkPathStroker::ResultType SkPathStroker::strokeCloseEnough (const SkPoint stroke[3],
                                                            const SkPoint ray[2],
                                                            SkQuadConstruct* quadPts STROKER_DEBUG_PARAMS (int depth)) const
{
    ...
    if (points_within_dist (ray[0], strokeMid, fInvResScale))
    { // if the difference is small
        if (sharp_angle (quadPts->fQuad))
        {
            return STROKER_RESULT (kSplit_ResultType, depth, quadPts, "sharp_angle (1) =%g,%g, %g,%g, %g,%g", quadPts->fQuad[0].fX, quadPts->fQuad[0].fY, quadPts->fQuad[1].fX, quadPts->fQuad[1].fY, quadPts->fQuad[2].fX, quadPts->fQuad[2].fY);
        }
        return STROKER_RESULT (kQuad_ResultType, depth, quadPts, "points_within_dist(ray[0]=%g,%g, strokeMid=%g,%g, fInvResScale=%g)", ray[0].fX, ray[0].fY, strokeMid.fX, strokeMid.fY, fInvResScale);
    }
    ...
    { // if far, subdivide
        return STROKER_RESULT (kSplit_ResultType, depth, quadPts, "!pt_in_quad_bounds(stroke=(%g,%g %g,%g %g,%g), ray[0]=%g,%g)", stroke[0].fX, stroke[0].fY, stroke[1].fX, stroke[1].fY, stroke[2].fX, stroke[2].fY, ray[0].fX, ray[0].fY);
    }
    ...
    if (rootCount != 1)
    {
        return STROKER_RESULT (kSplit_ResultType, depth, quadPts, "rootCount=%d != 1", rootCount);
    }
    SkPoint quadPt = SkEvalQuadAt (stroke, roots[0]);
    SkScalar error = fInvResScale * (SK_Scalar1 - SkScalarAbs (roots[0] - 0.5f) * 2);
    if (points_within_dist (ray[0], quadPt, error))
    { // if the difference is small, we're done
        if (sharp_angle (quadPts->fQuad))
        {
            return STROKER_RESULT (kSplit_ResultType, depth, quadPts, "sharp_angle (2) =%g,%g, %g,%g, %g,%g", quadPts->fQuad[0].fX, quadPts->fQuad[0].fY, quadPts->fQuad[1].fX, quadPts->fQuad[1].fY, quadPts->fQuad[2].fX, quadPts->fQuad[2].fY);
        }
        return STROKER_RESULT (kQuad_ResultType, depth, quadPts, "points_within_dist(ray[0]=%g,%g, quadPt=%g,%g, error=%g)", ray[0].fX, ray[0].fY, quadPt.fX, quadPt.fY, error);
    }
    // otherwise, subdivide
                                                          ...
                                                          SkQuadConstruct* quadPts)
{
    ...
    {
        SkPoint quadStartPt;
        this->quadPerpRay (quad, quadPts->fStartT, &quadStartPt, &quadPts->fQuad[0], &quadPts->fTangentStart);
        quadPts->fStartSet = true;
    }
    if (! quadPts->fEndSet)
    {
        SkPoint quadEndPt;
        this->quadPerpRay (quad, quadPts->fEndT, &quadEndPt, &quadPts->fQuad[2], &quadPts->fTangentEnd);
        quadPts->fEndSet = true;
    }
    ResultType resultType = this->intersectRay (quadPts, kCtrlPt_RayType STROKER_DEBUG_PARAMS (fRecursionDepth));
    if (resultType != kQuad_ResultType)
    ...
    if (! fFoundTangents)
    {
        ...
        if (kQuad_ResultType != resultType)
        {
            if ((kDegenerate_ResultType == resultType
                 || points_within_dist (quadPts->fQuad[0], quadPts->fQuad[2], fInvResScale))
...
void SkStroke::strokePath (const SkPath& src, SkPath* dst) const
{
    ...
    bool ignoreCenter = fDoFill && (src.getSegmentMasks() == SkPath::kLine_SegmentMask) && src.isLastContourClosed() && src.isConvex();

    SkPathStroker stroker (src, radius, fMiterLimit, this->getCap(), this->getJoin(), fResScale, ignoreCenter);
