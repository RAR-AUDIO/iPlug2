...
bool SkDeferredDisplayListRecorder::init()
{
    ...
    if (usesGLFBO0)
    {
        if (GrBackendApi::kOpenGL != fContext->backend() || fCharacterization.isTextureable())
        {
            return false;
        }
    }
    ...
    if (fCharacterization.vulkanSecondaryCBCompatible())
    {
        ...
        // origin.
        if (usesGLFBO0 || vkRTSupportsInputAttachment || fCharacterization.isTextureable() || fCharacterization.origin() == kBottomLeft_GrSurfaceOrigin)
        {
            return false;
        }
    }
    ...
    else if (fCharacterization.sampleCount() > 1 && ! caps->msaaResolvesAutomatically() && fCharacterization.isTextureable())
    {
    ...
    auto rtc = std::make_unique<GrRenderTargetContext> (fContext.get(), std::move (readView), std::move (writeView), grColorType, fCharacterization.refColorSpace(), &fCharacterization.surfaceProps());
