...
size_t SkTextBlob::RunRecord::StorageSize (uint32_t glyphCount, uint32_t textSize, SkTextBlob::GlyphPositioning positioning, SkSafeMath* safe)
{
...
SkTextBlob::SkTextBlob (const SkRect& bounds)
    : fBounds (bounds), fUniqueID (next_id()), fCacheID (SK_InvalidUniqueID) {}
...
{
    SkASSERT (! this->done());
    static_assert (static_cast<GlyphPositioning> (SkTextBlob::kDefault_Positioning) == kDefault_Positioning, "");
    static_assert (static_cast<GlyphPositioning> (SkTextBlob::kHorizontal_Positioning) == kHorizontal_Positioning, "");
    static_assert (static_cast<GlyphPositioning> (SkTextBlob::kFull_Positioning) == kFull_Positioning, "");
    static_assert (static_cast<GlyphPositioning> (SkTextBlob::kRSXform_Positioning) == kRSXform_Positioning, "");
...
SkTextBlobBuilder::SkTextBlobBuilder()
    : fStorageSize (0), fStorageUsed (0), fRunCount (0), fDeferredBounds (false), fLastRun (0)
{
    fBounds.setEmpty();
}
...
SkRect SkTextBlobBuilder::TightRunBounds (const SkTextBlob::RunRecord& run)
{
    ...
    {
        font.measureText (run.glyphBuffer(), run.glyphCount() * sizeof (uint16_t), SkTextEncoding::kGlyphID, &bounds);
...
SkRect SkTextBlobBuilder::ConservativeRunBounds (const SkTextBlob::RunRecord& run)
{
    ...
    SkASSERT (SkTextBlob::kFull_Positioning == run.positioning() || SkTextBlob::kHorizontal_Positioning == run.positioning() || SkTextBlob::kRSXform_Positioning == run.positioning());
...
bool SkTextBlobBuilder::mergeRun (const SkFont& font, SkTextBlob::GlyphPositioning positioning, uint32_t count, SkPoint offset)
{
    if (0 == fLastRun)
    {
    ...
    SkSafeMath safe;
    size_t sizeDelta =
        SkTextBlob::RunRecord::StorageSize (run->glyphCount() + count, 0, positioning, &safe) - SkTextBlob::RunRecord::StorageSize (run->glyphCount(), 0, positioning, &safe);
    if (! safe)
    {
        return false;
    }
...
const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRun (const SkFont& font, int count, SkScalar x, SkScalar y, const SkRect* bounds)
{
    this->allocInternal (font, SkTextBlob::kDefault_Positioning, count, 0, { x, y }, bounds);
    return fCurrentRunBuffer;
}

const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunPosH (const SkFont& font, int count, SkScalar y, const SkRect* bounds)
{
...
const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunText (const SkFont& font, int count, SkScalar x, SkScalar y, int textByteCount, SkString lang, const SkRect* bounds)
{
...
const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunTextPosH (const SkFont& font, int count, SkScalar y, int textByteCount, SkString lang, const SkRect* bounds)
{
    this->allocInternal (font,
...
const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunTextPos (const SkFont& font, int count, int textByteCount, SkString lang, const SkRect* bounds)
{
    this->allocInternal (font,
...
const SkTextBlobBuilder::RunBuffer& SkTextBlobBuilder::allocRunRSXform (const SkFont& font, int count, int textByteCount, SkString lang, const SkRect* bounds)
{
    this->allocInternal (font,
...
void SkTextBlobPriv::Flatten (const SkTextBlob& blob, SkWriteBuffer& buffer)
{
    ...
    while (! it.done())
    {
        ...
        buffer.writeByteArray (it.pos(),
                               it.glyphCount() * sizeof (SkScalar) * SkTextBlob::ScalarsPerGlyph (SkTo<SkTextBlob::GlyphPositioning> (it.positioning())));
        if (pe.extended)
        {
...
sk_sp<SkTextBlob> SkTextBlobPriv::MakeFromBuffer (SkReadBuffer& reader)
{
    ...
    for (;;)
    {
        ...
        const size_t glyphSize = safe.mul (glyphCount, sizeof (uint16_t)),
                     posSize =
                         safe.mul (glyphCount, safe.mul (sizeof (SkScalar), SkTextBlob::ScalarsPerGlyph (pos))),
        ...
        switch (pos)
        {
            case SkTextBlob::kDefault_Positioning:
                buf = &blobBuilder.allocRunText (font, glyphCount, offset.x(), offset.y(), textSize, SkString(), &bounds);
                break;
            case SkTextBlob::kHorizontal_Positioning:
                buf = &blobBuilder.allocRunTextPosH (font, glyphCount, offset.y(), textSize, SkString(), &bounds);
                break;
            case SkTextBlob::kFull_Positioning:
        ...
        if (! buf->glyphs || ! buf->pos || (pe.extended && (! buf->clusters || ! buf->utf8text)))
        {
            return nullptr;
        }

        if (! reader.readByteArray (buf->glyphs, glyphSize) || ! reader.readByteArray (buf->pos, posSize))
        {
            return nullptr;
        }
        ...
        if (pe.extended)
        {
            if (! reader.readByteArray (buf->clusters, clusterSize) || ! reader.readByteArray (buf->utf8text, textSize))
            {
                return nullptr;
            }
        }
...
sk_sp<SkTextBlob> SkTextBlob::MakeFromText (const void* text, size_t byteLength, const SkFont& font, SkTextEncoding encoding)
{
...
sk_sp<SkTextBlob> SkTextBlob::MakeFromPosText (const void* text, size_t byteLength, const SkPoint pos[], const SkFont& font, SkTextEncoding encoding)
{
    const int count = font.countText (text, byteLength, encoding);
    if (count < 1)
    {
...
sk_sp<SkTextBlob> SkTextBlob::MakeFromPosTextH (const void* text, size_t byteLength, const SkScalar xpos[], SkScalar constY, const SkFont& font, SkTextEncoding encoding)
{
    const int count = font.countText (text, byteLength, encoding);
    if (count < 1)
    {
...
sk_sp<SkTextBlob> SkTextBlob::MakeFromRSXform (const void* text, size_t byteLength, const SkRSXform xform[], const SkFont& font, SkTextEncoding encoding)
{
    const int count = font.countText (text, byteLength, encoding);
    if (count < 1)
    {
...
sk_sp<SkTextBlob> SkTextBlob::Deserialize (const void* data, size_t length, const SkDeserialProcs& procs)
{
...
int SkTextBlob::getIntercepts (const SkScalar bounds[2], SkScalar intervals[], const SkPaint* paint) const
{
