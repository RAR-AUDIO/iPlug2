...
public:
    ...
    static SkBlitter* Create (const SkPixmap&, const SkPaint&, SkArenaAlloc*, const SkRasterPipeline& shaderPipeline, bool is_opaque, bool is_constant, sk_sp<SkShader> clipShader);

    SkRasterPipelineBlitter (SkPixmap dst,
                             SkBlendMode blend,
                             SkArenaAlloc* alloc)
        : fDst (dst), fBlend (blend), fAlloc (alloc), fColorPipeline (alloc)
    {
    }
                                          ...
                                          sk_sp<SkShader> clipShader)
{
    ...
    SkColorSpaceXformSteps (sk_srgb_singleton(), kUnpremul_SkAlphaType, dstCS, kUnpremul_SkAlphaType).apply (paintColor.vec());
    ...
    if (! shader)
    {
        ...
        bool is_opaque = paintColor.fA == 1.0f,
             is_constant = true;
        return SkRasterPipelineBlitter::Create (dst, paint, alloc, shaderPipeline, is_opaque, is_constant, std::move (clipShader));
    }
                                          ...
                                          sk_sp<SkShader> clipShader)
{
    ...
    return SkRasterPipelineBlitter::Create (dst, paint, alloc, shaderPipeline, is_opaque, is_constant, clipShader);
}
                                            ...
                                            sk_sp<SkShader> clipShader)
{
    ...
    if (is_constant && blitter->fBlend == SkBlendMode::kSrc)
    {
        ...
        {
            case 0:
                blitter->fMemset2D = [] (SkPixmap* dst, int x, int y, int w, int h, uint64_t c) {
...
void SkRasterPipelineBlitter::blitRect (int x, int y, int w, int h)
{
    ...
    if (! fBlitRect)
    {
        ...
        if (fBlend == SkBlendMode::kSrcOver
            && (fDst.info().colorType() == kRGBA_8888_SkColorType || fDst.info().colorType() == kBGRA_8888_SkColorType)
            && ! fDst.colorSpace()
