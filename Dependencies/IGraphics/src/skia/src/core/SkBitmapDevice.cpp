...
private:
    void stepAndSetupTileDraw()
    {
        ...
        fDone = fOrigin.fX >= fSrcBounds.fRight - kMaxDim && fOrigin.fY >= fSrcBounds.fBottom - kMaxDim;
...
SkBitmapDevice::SkBitmapDevice (const SkBitmap& bitmap)
    : INHERITED (bitmap.info(), SkSurfaceProps()), fBitmap (bitmap), fRCStack (bitmap.width(), bitmap.height()), fGlyphPainter (this->surfaceProps(), bitmap.colorType(), bitmap.colorSpace(), SkStrikeCache::GlobalStrikeCache())
{
...
SkBitmapDevice::SkBitmapDevice (const SkBitmap& bitmap, const SkSurfaceProps& surfaceProps, SkRasterHandleAllocator::Handle hndl, const SkBitmap* coverage)
    : INHERITED (bitmap.info(), surfaceProps), fBitmap (bitmap), fRasterHandle (hndl), fRCStack (bitmap.width(), bitmap.height()), fGlyphPainter (this->surfaceProps(), bitmap.colorType(), bitmap.colorSpace(), SkStrikeCache::GlobalStrikeCache())
{
    SkASSERT (valid_for_bitmap_device (bitmap.info(), nullptr));
...
void SkBitmapDevice::drawBitmap (const SkBitmap& bitmap, const SkMatrix& matrix, const SkRect* dstOrNull, const SkPaint& paint)
{
                                    ...
                                    SkCanvas::SrcRectConstraint constraint)
{
    ...
    if (src && ! src->contains (bitmapBounds) && SkCanvas::kFast_SrcRectConstraint == constraint && paint.getFilterQuality() != kNone_SkFilterQuality)
    {
    ...
    if (src)
    {
        ...
        SkRect extractedBitmapBounds = SkRect::MakeXYWH (dx, dy, SkIntToScalar (bitmapPtr->width()), SkIntToScalar (bitmapPtr->height()));
#else
...
USE_SHADER:
    ...
    auto s = SkMakeBitmapShaderForPaint (paint, *bitmapPtr, SkTileMode::kClamp, SkTileMode::kClamp, &matrix, kNever_SkCopyPixelsMode);
    if (! s)
    {
        return;
    }
...
void SkBitmapDevice::drawVertices (const SkVertices* vertices, SkBlendMode bmode, const SkPaint& paint)
{
    BDDraw (this).drawVertices (vertices, bmode, paint);
}

void SkBitmapDevice::drawAtlas (const SkImage* atlas, const SkRSXform xform[], const SkRect tex[], const SkColor colors[], int count, SkBlendMode mode, const SkPaint& paint)
{
...
{
    return SkSpecialImage::MakeFromImage (nullptr, SkIRect::MakeWH (image->width(), image->height()), image->makeNonTextureImage());
}
...
bool SkBitmapDevice::onClipIsWideOpen() const
{
    ...
    return rc.isBW() && rc.bwRgn().isRect() && rc.bwRgn().getBounds() == SkIRect { 0, 0, this->width(), this->height() };
}
