...
static bool SK_WARN_UNUSED_RESULT read_string (SkStream* stream, SkString* string)
{
    ...
    if (length > 0)
    {
        ...
        if (stream->read (string->writable_str(), length) != length)
        {
            return false;
        }
    }
...
static bool write_string (SkWStream* stream, const SkString& string, uint32_t id)
{
    ...
    return stream->writePackedUInt (id) && stream->writePackedUInt (string.size()) && stream->write (string.c_str(), string.size());
}
...
{
    size_t styleBits;
    if (! stream->readPackedUInt (&styleBits))
    {
        return false;
    }
    ...
    for (size_t id; (id = read_id (stream)) != kSentinel;)
    {
        switch (id)
        {
            case kFontFamilyName:
                if (! read_string (stream, &result->fFamilyName))
                {
                    return false;
                }
                break;
            case kFullName:
                if (! read_string (stream, &result->fFullName))
                {
                    return false;
                }
                break;
            case kPostscriptName:
                if (! read_string (stream, &result->fPostscriptName))
                {
                    return false;
                }
                break;
            case kFontAxes:
                ...
                {
                    if (! stream->readPackedUInt (&coordinateCount))
                    {
                        return false;
                    }
                    for (size_t i = 0; i < coordinateCount; ++i)
                    {
                        if (! stream->readS32 (&oldBadVariationValue))
                        {
                            return false;
                        }
                    }
            ...
            case kFontVariation:
                ...
                for (size_t i = 0; i < coordinateCount; ++i)
                {
                    if (! stream->readU32 (&result->fVariation[i].axis))
                    {
                        return false;
                    }
                    if (! stream->readScalar (&result->fVariation[i].value))
                    {
                        return false;
                    }
                }
            ...
            case kFontIndex:
                ...
                if (! SkTFitsIn<CollectionIndexType> (index))
                {
                    return false;
                }
