...
static uint32_t read_cpu_features()
{
    ...
    { // XMM and YMM state enabled.
        ...
        if ((xgetbv (0) & (7 << 5)) == (7 << 5))
        { // All ZMM state bits enabled too.
            if (abcd[1] & (1 << 16))
            {
                features |= SkCpu::AVX512F;
            }
            if (abcd[1] & (1 << 17))
            {
                features |= SkCpu::AVX512DQ;
            }
            if (abcd[1] & (1 << 21))
            {
                features |= SkCpu::AVX512IFMA;
            }
            if (abcd[1] & (1 << 26))
            {
                features |= SkCpu::AVX512PF;
            }
            if (abcd[1] & (1 << 27))
            {
                features |= SkCpu::AVX512ER;
            }
            if (abcd[1] & (1 << 28))
            {
                features |= SkCpu::AVX512CD;
            }
            if (abcd[1] & (1 << 30))
            {
                features |= SkCpu::AVX512BW;
            }
            if (abcd[1] & (1 << 31))
            {
                features |= SkCpu::AVX512VL;
            }
        }
    }
    return features;
...
static uint32_t read_cpu_features()
{
    ...
    if (hwcaps & kHWCAP_CRC32)
    {
        features |= SkCpu::CRC32;
    }
    if (hwcaps & kHWCAP_ASIMDHP)
    {
...
static uint32_t read_cpu_features()
{
    ...
    if (cpu_features & ANDROID_CPU_ARM_FEATURE_NEON_FMA)
    {
        features |= SkCpu::NEON_FMA;
    }
    if (cpu_features & ANDROID_CPU_ARM_FEATURE_VFP_FP16)
    {
