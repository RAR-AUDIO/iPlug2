...
{
    if (r.isEmpty() || SkRegion_kRunTypeSentinel == r.right() || SkRegion_kRunTypeSentinel == r.bottom())
    {
...
char* SkRegion::toString()
{
    ...
    while (! iter.done())
    {
        ...
        count += snprintf (result + count, max - count, "(%d,%d,%d,%d)", r.fLeft, r.fTop, r.fRight, r.fBottom);
        iter.next();
    }
                            ...
                            int max)
{
    ...
    while (! rec.done())
    {
        ...
        if ((unsigned) (rec.fInside - min) <= (unsigned) (max - min) && left < rite)
        { // skip if equal
...
public:
    RgnOper (int top, RunArray* array, SkRegion::Op op)
        : fMin (gOpMinMax[op].fMin), fMax (gOpMinMax[op].fMax), fArray (array), fTop ((SkRegionPriv::RunType) top) // just a first guess, we might update this
    {
    ...
    void addSpan (int bottom, const SkRegionPriv::RunType a_runs[], const SkRegionPriv::RunType b_runs[])
    {
        ...
        if (fPrevLen == len && (1 == len || ! memcmp (&(*fArray)[fPrevDst], &(*fArray)[start], (len - 1) * sizeof (SkRegionPriv::RunType))))
        {
            // update Y value
                    ...
                    bool quickExit)
{
    ...
    while (a_bot < SkRegion_kRunTypeSentinel || b_bot < SkRegion_kRunTypeSentinel)
    {
                          ...
                          int32_t intervalCount)
{
    ...
    // quick safety check:
    if (runs[runCount - 1] != SkRegion_kRunTypeSentinel || runs[runCount - 2] != SkRegion_kRunTypeSentinel)
    {
        return false;
    }
    ...
    do
    {
        ...
        while (xIntervals-- > 0)
        {
            rect.fLeft = *runs++;
            rect.fRight = *runs++;
            if (rect.fLeft == SkRegion_kRunTypeSentinel || rect.fRight == SkRegion_kRunTypeSentinel || rect.fLeft >= rect.fRight || // check non-empty rect
...
size_t SkRegion::readFromMemory (const void* storage, size_t length)
{
    ...
    if (count >= 0)
    {
        ...
        else
        {
            ...
            if (! buffer.readS32 (&ySpanCount) || ! buffer.readS32 (&intervalCount) || buffer.available() < count * sizeof (int32_t))
            {
                return 0;
            }
...
bool SkRegion::isValid() const
{
    ...
    return fRunHead && fRunHead->fRefCnt > 0 && validate_run (fRunHead->readonly_runs(), fRunHead->fRunCount, fBounds, fRunHead->getYSpanCount(), fRunHead->getIntervalCount());
}
