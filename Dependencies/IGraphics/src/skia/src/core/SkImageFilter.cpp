...
SkIRect SkImageFilter::filterBounds (const SkIRect& src, const SkMatrix& ctm, MapDirection direction, const SkIRect* inputRect) const
{
    ...
    if (kReverse_MapDirection == direction)
    {
        ...
        if (as_IFB (this)->cropRectIsSet())
        {
            skif::LayerSpace<SkIRect> outputCrop = mapping.paramToLayer (
                                                              skif::ParameterSpace<SkRect> (as_IFB (this)->getCropRect().rect()))
                                                       .roundOut();
...
skif::FilterResult<For::kOutput> SkImageFilter_Base::filterImage (const skif::Context& context) const
{
    ...
    SkImageFilterCacheKey key (fUniqueID, context.mapping().layerMatrix(), context.clipBounds(), srcGenID, srcSubset);
    ...
    const skif::ParameterSpace<SkRect>& contentBounds) const
{
    ...
    SkIRect dst;
    as_IFB (this)->getCropRect().applyTo (
        SkIRect (filterOutput), mapping.layerMatrix(), as_IFB (this)->affectsTransparentBlack(), &dst);
...
void SkImageFilter::CropRect::applyTo (const SkIRect& imageBounds, const SkMatrix& ctm, bool embiggen, SkIRect* cropped) const
{
    *cropped = imageBounds;
    if (fFlags)
    {
...
bool SkImageFilter_Base::applyCropRect (const Context& ctx, const SkIRect& srcBounds, SkIRect* dstBounds) const
{
...
// around it.
static sk_sp<SkSpecialImage> pad_image (SkSpecialImage* src, const SkImageFilter_Base::Context& ctx, int newWidth, int newHeight, int offX, int offY)
{
...
{
    const SkIRect srcBounds = SkIRect::MakeXYWH (srcOffset->x(), srcOffset->y(), src->width(), src->height());
    ...
    {
        sk_sp<SkSpecialImage> img (pad_image (src, ctx, bounds->width(), bounds->height(), Sk32_sat_sub (srcOffset->x(), bounds->x()), Sk32_sat_sub (srcOffset->y(), bounds->y())));
...
SkIRect SkImageFilter_Base::onFilterBounds (const SkIRect& src, const SkMatrix& ctm, MapDirection dir, const SkIRect* inputRect) const
{
...
SkIRect SkImageFilter_Base::onFilterNodeBounds (const SkIRect& src, const SkMatrix&, MapDirection, const SkIRect*) const
{
    return src;
}
    ...
    const skif::LayerSpace<SkIRect>& contentBounds) const
{
    ...
    for (int i = 0; i < this->countInputs(); ++i)
    {
        ...
        skif::LayerSpace<SkIRect> requiredInput =
            filter ? as_IFB (filter)->onGetInputLayerBounds (mapping, desiredOutput, contentBounds)
                   : desiredOutput;
    ...
    VisitChildren recurse) const
{
    ...
    SkIRect input = this->onFilterNodeBounds (SkIRect (desiredOutput), mapping.layerMatrix(), kReverse_MapDirection, &content);
    if (recurse == VisitChildren::kYes)
    {
        SkIRect aggregate = this->onFilterBounds (input, mapping.layerMatrix(), kReverse_MapDirection, &input);
    ...
    const skif::LayerSpace<SkIRect>& contentBounds) const
{
    ...
    SkIRect aggregate = this->onFilterBounds (SkIRect (contentBounds), mapping.layerMatrix(), kForward_MapDirection, nullptr);
    SkIRect output = this->onFilterNodeBounds (aggregate, mapping.layerMatrix(), kForward_MapDirection, nullptr);
                                                      ...
                                                      GrProtected isProtected)
{
    ...
    auto renderTargetContext = GrRenderTargetContext::Make (
        context, SkColorTypeToGrColorType (colorType), sk_ref_sp (colorSpace), SkBackingFit::kApprox, bounds.size(), 1, GrMipmapped::kNo, isProtected, kBottomLeft_GrSurfaceOrigin);
    ...
    renderTargetContext->fillRectToRect (nullptr, std::move (paint), GrAA::kNo, SkMatrix::I(), dstRect, srcRect);

    return SkSpecialImage::MakeDeferredFromGpu (
        context, dstIRect, kNeedNewImageUniqueID_SpecialImage, renderTargetContext->readSurfaceView(), renderTargetContext->colorInfo().colorType(), renderTargetContext->colorInfo().refColorSpace());
}
                                                             ...
                                                             SkColorSpace* colorSpace)
{
    ...
    auto colorSpaceXform = GrColorSpaceXform::Make (src->getColorSpace(), src->alphaType(), colorSpace, kPremul_SkAlphaType);
    ...
    sk_sp<SkSpecialSurface> surf (src->makeSurface (colorType, colorSpace, SkISize::Make (src->width(), src->height())));
    if (! surf)
    {
        return sk_ref_sp (src);
    }
                                                       ...
                                                       const SkIRect& originalSrcBounds)
{
    ...
    tmp.adjust (-filterOffset.fX, -filterOffset.fY, filterSize.fWidth - filterOffset.fX, filterSize.fHeight - filterOffset.fY);
...
static sk_sp<SkImageFilter> apply_ctm_to_filter (sk_sp<SkImageFilter> input, const SkMatrix& ctm, SkMatrix* remainder)
{
