...
SkScalerContext::SkScalerContext (sk_sp<SkTypeface> typeface, const SkScalerContextEffects& effects, const SkDescriptor* desc)
    : fRec (PreprocessRec (*typeface, effects, *desc)), fTypeface (std::move (typeface)), fPathEffect (sk_ref_sp (effects.fPathEffect)), fMaskFilter (sk_ref_sp (effects.fMaskFilter))
...
size_t SkScalerContext::GetGammaLUTSize (SkScalar contrast, SkScalar paintGamma, SkScalar deviceGamma, int* width, int* height)
{
    SkAutoMutexExclusive ama (mask_gamma_cache_mutex());
...
bool SkScalerContext::GetGammaLUTData (SkScalar contrast, SkScalar paintGamma, SkScalar deviceGamma, uint8_t* data)
{
    SkAutoMutexExclusive ama (mask_gamma_cache_mutex());
    const SkMaskGamma& maskGamma = cached_mask_gamma (contrast,
                                                      paintGamma,
...
void SkScalerContext::getMetrics (SkGlyph* glyph)
{
    ...
    else
    {
        ...
        else
        {
            ...
            const bool fromLCD = (glyph->fMaskFormat == SkMask::kLCD16_Format) || (glyph->fMaskFormat == SkMask::kA8_Format && a8FromLCD);
...
static void pack4xHToMask (const SkPixmap& src, const SkMask& dst, const SkMaskGamma::PreBlend& maskPreBlend, const bool doBGR, const bool doVert)
{
...
#define LCD_PER_PIXEL 3
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        for (int sample_x = -4; sample_x < sample_width + 4; sample_x += 4)
        {
            ...
            for (int sample_index = std::max (0, sample_x - 4), coeff_index = sample_index - (sample_x - 4); sample_index < std::min (sample_x + 8, sample_width); ++sample_index, ++coeff_index)
            {
...
static void generateMask (const SkMask& mask, const SkPath& path, const SkMaskGamma::PreBlend& maskPreBlend, const bool doBGR, const bool doVert, const bool a8FromLCD)
{
    SkASSERT (mask.fFormat == SkMask::kBW_Format || mask.fFormat == SkMask::kA8_Format || mask.fFormat == SkMask::kLCD16_Format);
    ...
    const bool fromLCD = (mask.fFormat == SkMask::kLCD16_Format) || (mask.fFormat == SkMask::kA8_Format && a8FromLCD);
    ...
    if (fromLCD)
    {
        if (doVert)
        {
            dstW = 4 * dstH - 8;
            dstH = srcW;
            matrix.setAll (0, 4, -SkIntToScalar (mask.fBounds.fTop + 1) * 4, 1, 0, -SkIntToScalar (mask.fBounds.fLeft), 0, 0, 1);
        }
        else
        {
            dstW = 4 * dstW - 8;
            matrix.setAll (4, 0, -SkIntToScalar (mask.fBounds.fLeft + 1) * 4, 0, 1, -SkIntToScalar (mask.fBounds.fTop), 0, 0, 1);
        }
    }
...
bool SkScalerContextRec::computeMatrices (PreMatrixScale preMatrixScale, SkVector* s, SkMatrix* sA, SkMatrix* GsA, SkMatrix* G_inv, SkMatrix* A_out)
{
    ...
    if (skewedOrFlipped)
    {
        ...
        {
            G_inv->setAll (
                G.get (SkMatrix::kMScaleX), -G.get (SkMatrix::kMSkewX), G.get (SkMatrix::kMTransX), -G.get (SkMatrix::kMSkewY), G.get (SkMatrix::kMScaleY), G.get (SkMatrix::kMTransY), G.get (SkMatrix::kMPersp0), G.get (SkMatrix::kMPersp1), G.get (SkMatrix::kMPersp2));
        }
    }
    else
    {
    ...
    if (SkScalarAbs (GA.get (SkMatrix::kMScaleX)) <= SK_ScalarNearlyZero || SkScalarAbs (GA.get (SkMatrix::kMScaleY)) <= SK_ScalarNearlyZero || ! GA.isFinite())
    {
        s->fX = SK_Scalar1;
        s->fY = SK_Scalar1;
        sA->setScale (0, 0);
...
void SkScalerContext::MakeRecAndEffects (const SkFont& font, const SkPaint& paint, const SkSurfaceProps& surfaceProps, SkScalerContextFlags scalerContextFlags, const SkMatrix& deviceMatrix, SkScalerContextRec* rec, SkScalerContextEffects* effects)
{
                                          ...
                                          SkBinaryWriteBuffer* effectBuffer)
{
    ...
    if (effects.fPathEffect || effects.fMaskFilter)
    {
        ...
        {
            effectBuffer->writeFlattenable (effects.fPathEffect);
        }
        if (effects.fMaskFilter)
        {
            effectBuffer->writeFlattenable (effects.fMaskFilter);
        }
        entryCount += 1;
    ...
    const SkDescriptor* desc)
{
    ...
    {
    public:
        SkScalerContext_Empty (sk_sp<SkTypeface> typeface, const SkScalerContextEffects& effects, const SkDescriptor* desc)
