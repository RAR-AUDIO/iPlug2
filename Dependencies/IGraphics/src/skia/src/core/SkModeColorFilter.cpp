...
bool SkModeColorFilter::onAppendStages (const SkStageRec& rec, bool shaderIsOpaque) const
{
    ...
    SkColorSpaceXformSteps (sk_srgb_singleton(), kUnpremul_SkAlphaType, rec.fDstCS, kUnpremul_SkAlphaType).apply (color.vec());
    rec.fPipeline->append_constant_color (rec.fAlloc, color.premul().vec());
    SkBlendMode_AppendStages (fMode, rec.fPipeline);
    return true;
}

skvm::Color SkModeColorFilter::onProgram (skvm::Builder* p, skvm::Color c, SkColorSpace* dstCS, skvm::Uniforms* uniforms, SkArenaAlloc*) const
{
    SkColor4f color = SkColor4f::FromColor (fColor);
    SkColorSpaceXformSteps (sk_srgb_singleton(), kUnpremul_SkAlphaType, dstCS, kPremul_SkAlphaType).apply (color.vec());
    skvm::Color dst = c,
                                                   ...
                                                   const GrColorInfo& dstColorInfo) const
{
    ...
    auto xferFP = GrBlendFragmentProcessor::Make (
        std::move (colorFP), std::move (inputFP), fMode, GrBlendFragmentProcessor::BlendBehavior::kSkModeBehavior);
    ...
    SkASSERT (fMode > SkBlendMode::kLastCoeffMode || xferFP->hasConstantOutputForConstantInput() >= fpHasConstIO);
...
sk_sp<SkColorFilter> SkColorFilters::Blend (SkColor color, SkBlendMode mode)
{
    ...
    if (SkBlendMode::kDst == mode || (0 == alpha && (SkBlendMode::kSrcOver == mode || SkBlendMode::kDstOver == mode || SkBlendMode::kDstOut == mode || SkBlendMode::kSrcATop == mode || SkBlendMode::kXor == mode || SkBlendMode::kDarken == mode)) || (0xFF == alpha && SkBlendMode::kDstIn == mode))
    {
        return nullptr;
    }
