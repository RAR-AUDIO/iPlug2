...
static void write_one_instruction (Val id, const I& inst, SkWStream* o, Fs... fs)
{
    ...
    {
        case Op::assert_true:
            write (o, op, V { x }, V { y }, fs (id)...);
            break;

        case Op::store8:
            write (o, op, Arg { immy }, V { x }, fs (id)...);
            break;
        case Op::store16:
            write (o, op, Arg { immy }, V { x }, fs (id)...);
            break;
        case Op::store32:
            ...
            break;
        case Op::store64:
            write (o, op, Arg { immz }, V { x }, V { y }, fs (id)...);
            break;
        case Op::store128:
            write (o, op, Arg { immz >> 1 }, V { x }, V { y }, Hex { immz & 1 }, fs (id)...);
            break;

        case Op::index:
            write (o, V { id }, "=", op, fs (id)...);
            break;

        case Op::load8:
            write (o, V { id }, "=", op, Arg { immy }, fs (id)...);
            break;
        case Op::load16:
            write (o, V { id }, "=", op, Arg { immy }, fs (id)...);
            break;
        case Op::load32:
            ...
            break;
        case Op::load64:
            write (o, V { id }, "=", op, Arg { immy }, Hex { immz }, fs (id)...);
            break;
        case Op::load128:
            write (o, V { id }, "=", op, Arg { immy }, Hex { immz }, fs (id)...);
            break;
        ...
        case Op::gather8:
            write (o, V { id }, "=", op, Arg { immy }, Hex { immz }, V { x }, fs (id)...);
            break;
        case Op::gather16:
            write (o, V { id }, "=", op, Arg { immy }, Hex { immz }, V { x }, fs (id)...);
            break;
        ...
        case Op::splat:
            write (o, V { id }, "=", op, Splat { immy }, fs (id)...);
            break;
        case Op::splat_q14:
            write (o, V { id }, "=", op, Splat { immy }, fs (id)...);
            break;
        ...
        case Op::max_f32:
            ...
            break;
        case Op::fma_f32:
            write (o, V { id }, "=", op, V { x }, V { y }, V { z }, fs (id)...);
            break;
        case Op::fms_f32:
            write (o, V { id }, "=", op, V { x }, V { y }, V { z }, fs (id)...);
            break;
        ...
        case Op::shl_i32:
            write (o, V { id }, "=", op, V { x }, Shift { immy }, fs (id)...);
            break;
        case Op::shr_i32:
            write (o, V { id }, "=", op, V { x }, Shift { immy }, fs (id)...);
            break;
        ...
        case Op::select_q14:
            ...
            break;
        case Op::pack:
            write (o, V { id }, "=", op, V { x }, V { y }, Shift { immz }, fs (id)...);
            break;
    ...
    for (Val i = 0; i < (Val) fImpl->instructions.size(); i++)
    {
        ...
        switch (op)
        {
            ...
            case Op::store32:
                ...
                break;
            case Op::store64:
                write (o, op, Arg { immz }, R { x }, R { y });
                break;
            case Op::store128:
                write (o, op, Arg { immz >> 1 }, R { x }, R { y }, Hex { immz & 1 });
                break;
            ...
            case Op::load32:
                ...
                break;
            case Op::load64:
                write (o, R { d }, "=", op, Arg { immy }, Hex { immz });
                break;
            case Op::load128:
                write (o, R { d }, "=", op, Arg { immy }, Hex { immz });
                break;
            ...
            case Op::gather8:
                write (o, R { d }, "=", op, Arg { immy }, Hex { immz }, R { x });
                break;
            case Op::gather16:
                write (o, R { d }, "=", op, Arg { immy }, Hex { immz }, R { x });
                break;
            ...
            case Op::select_q14:
                ...
                break;
            case Op::pack:
                write (o, R { d }, "=", op, R { x }, R { y }, Shift { immz });
                break;
...
F32 Builder::add (F32 x, F32 y)
{
    ...
    if (this->isImm (y.id, 0.0f))
    {
        return x;
    } // x+0 == x
    if (this->isImm (x.id, 0.0f))
    {
        return y;
    } // 0+y == y
...
static F32 approx_atan_unit (F32 x)
{
    ...
    return poly (x, 0.14130025741326729f, -0.34312835980675116f, -0.016172900528248768f, 1.0037696976200385f, -0.00014758242182738969f);
}
...
I32 Builder::mul (I32 x, I32 y)
{
    ...
    if (this->isImm (x.id, 1))
    {
        return y;
    }
    if (this->isImm (y.id, 1))
    {
        return x;
    }
...
I32 Builder::select (I32 x, I32 y, I32 z)
{
    ...
    if (int X, Y, Z; this->allImm (x.id, &X, y.id, &Y, z.id, &Z))
    {
        return splat (X ? Y : Z);
    }
    if (this->isImm (x.id, ~0))
    ...
    if (this->isImm (y.id, 0))
    {
        return bit_clear (z, x);
    } //     x ? 0 : z == ~x&z
    if (this->isImm (z.id, 0))
    {
        return bit_and (y, x);
    } //     x ? y : 0 ==  x&y
...
{
    if (unsigned Z; this->allImm (z.id, &Z) && (~0u >> bits) == Z)
    {
        return shr (x, bits);
    }
...
bool SkColorType_to_PixelFormat (SkColorType ct, PixelFormat* f)
{
    ...
    switch (ct)
    {
        ...
        case kRGBA_F32_SkColorType:
            *f = { FLOAT, 32, 32, 32, 32, 0, 32, 64, 96 };
            return true;

        case kRGBA_F16Norm_SkColorType:
            *f = { FLOAT, 16, 16, 16, 16, 0, 16, 32, 48 };
            return true;
        case kRGBA_F16_SkColorType:
            *f = { FLOAT, 16, 16, 16, 16, 0, 16, 32, 48 };
            return true;
        case kR16G16B16A16_unorm_SkColorType:
            *f = { UNORM, 16, 16, 16, 16, 0, 16, 32, 48 };
            return true;

        case kA16_float_SkColorType:
            *f = { FLOAT, 0, 0, 0, 16, 0, 0, 0, 0 };
            return true;
        case kR16G16_float_SkColorType:
            *f = { FLOAT, 16, 16, 0, 0, 0, 16, 0, 0 };
            return true;

        case kAlpha_8_SkColorType:
            *f = { UNORM, 0, 0, 0, 8, 0, 0, 0, 0 };
            return true;
        case kGray_8_SkColorType:
            *f = { UNORM, 8, 8, 8, 0, 0, 0, 0, 0 };
            return true; // Subtle.

        case kRGB_565_SkColorType:
            *f = { UNORM, 5, 6, 5, 0, 11, 5, 0, 0 };
            return true; // (BGR)
        case kARGB_4444_SkColorType:
            *f = { UNORM, 4, 4, 4, 4, 12, 8, 4, 0 };
            return true; // (ABGR)

        case kRGBA_8888_SkColorType:
            *f = { UNORM, 8, 8, 8, 8, 0, 8, 16, 24 };
            return true;
        case kRGB_888x_SkColorType:
            *f = { UNORM, 8, 8, 8, 0, 0, 8, 16, 32 };
            return true; // 32-bit
        case kBGRA_8888_SkColorType:
            *f = { UNORM, 8, 8, 8, 8, 16, 8, 0, 24 };
            return true;

        case kRGBA_1010102_SkColorType:
            *f = { UNORM, 10, 10, 10, 2, 0, 10, 20, 30 };
            return true;
        case kBGRA_1010102_SkColorType:
            *f = { UNORM, 10, 10, 10, 2, 20, 10, 0, 30 };
            return true;
        case kRGB_101010x_SkColorType:
            *f = { UNORM, 10, 10, 10, 0, 0, 10, 20, 0 };
            return true;
        case kBGR_101010x_SkColorType:
            *f = { UNORM, 10, 10, 10, 0, 20, 10, 0, 0 };
            return true;

        case kR8G8_unorm_SkColorType:
            *f = { UNORM, 8, 8, 0, 0, 0, 8, 0, 0 };
            return true;
        case kR16G16_unorm_SkColorType:
            *f = { UNORM, 16, 16, 0, 0, 0, 16, 0, 0 };
            return true;
        case kA16_unorm_SkColorType:
            *f = { UNORM, 0, 0, 0, 16, 0, 0, 0, 0 };
            return true;
    }
    return false;
}
    ...
    auto unpack_channel = [=] (int bits, int shift) {
        ...
        switch (f.encoding)
        {
            case PixelFormat::UNORM:
                return from_unorm (bits, channel);
            case PixelFormat::FLOAT:
                return from_half (channel);
        }
        SkUNREACHABLE;
    };
    return {
...
Color Builder::load (PixelFormat f, Arg ptr)
{
    ...
    {
        case 1:
            return unpack (f, load8 (ptr));
        case 2:
            return unpack (f, load16 (ptr));
        case 4:
            return unpack (f, load32 (ptr));
        case 8:
        {
            PixelFormat lo, hi;
...
Color Builder::gather (PixelFormat f, Arg ptr, int offset, I32 index)
{
    ...
    {
        case 1:
            return unpack (f, gather8 (ptr, offset, index));
        case 2:
            return unpack (f, gather16 (ptr, offset, index));
        case 4:
            return unpack (f, gather32 (ptr, offset, index));
        case 8:
        {
            PixelFormat lo, hi;
            split_disjoint_8byte_format (f, &lo, &hi);
...
static I32 pack32 (PixelFormat f, Color c)
{
    ...
    auto pack_channel = [&] (F32 channel, int bits, int shift) {
        ...
        {
            case PixelFormat::UNORM:
                encoded = to_unorm (bits, channel);
                break;
            case PixelFormat::FLOAT:
                encoded = to_half (channel);
                break;
        }
...
bool Builder::store (PixelFormat f, Arg ptr, Color c)
{
    ...
    if (f.r_bits == f.g_bits && f.g_bits == f.b_bits && f.r_shift == f.g_shift && f.g_shift == f.b_shift)
    {
    ...
    {
        case 1:
            store8 (ptr, pack32 (f, c));
            return true;
        case 2:
            store16 (ptr, pack32 (f, c));
            return true;
        case 4:
            store32 (ptr, pack32 (f, c));
            return true;
        case 8:
        {
            PixelFormat lo, hi;
...
void Builder::unpremul (F32* r, F32* g, F32* b, F32 a)
{
    ...
    invA = select (invA < inf, invA, 0.0f);
    *r *= invA;
    *g *= invA;
    *b *= invA;
}
...
HSLA Builder::to_hsla (Color c)
{
    ...
    F32 h = (1 / 6.0f) * select (mx == mn, 0.0f, select (mx == c.r, invd * (c.g - c.b) + g_lt_b, select (mx == c.g, invd * (c.b - c.r) + 2.0f, invd * (c.r - c.g) + 4.0f)));

    F32 sum = mx + mn,
        l = sum * 0.5f,
        s = select (mx == mn, 0.0f, d / select (l > 0.5f, 2.0f - sum, sum));
    return { h, s, l, c.a };
}
...
Color Builder::blend (SkBlendMode mode, Color src, Color dst)
{
    ...
    switch (mode)
    {
        ...
        case SkBlendMode::kClear:
            return { splat (0.0f), splat (0.0f), splat (0.0f), splat (0.0f) };
        ...
        case SkBlendMode::kDarken:
            return apply_rgb_srcover_a ([&] (auto s, auto d) {
                return s + (d - max (s * dst.a, d * src.a));
            });

        case SkBlendMode::kLighten:
            return apply_rgb_srcover_a ([&] (auto s, auto d) {
                return s + (d - min (s * dst.a, d * src.a));
            });

        case SkBlendMode::kDifference:
            return apply_rgb_srcover_a ([&] (auto s, auto d) {
                return s + (d - two (min (s * dst.a, d * src.a)));
            });
        ...
        case SkBlendMode::kColorBurn:
            return apply_rgb_srcover_a ([&] (auto s, auto d) {
                ...
                return select (d == dst.a, s * (1 - dst.a) + d, select (is_finite (burn), burn, d * (1 - src.a) + s));
            });

        case SkBlendMode::kColorDodge:
            return apply_rgb_srcover_a ([&] (auto s, auto d) {
                auto dodge = src.a * min (dst.a, d * src.a / (src.a - s))
                             + mma (s, 1 - dst.a, d, 1 - src.a);
                return select (d == 0.0f, s * (1 - dst.a) + d, select (is_finite (dodge), dodge, d * (1 - src.a) + s));
            });

        case SkBlendMode::kHardLight:
            return apply_rgb_srcover_a ([&] (auto s, auto d) {
                return mma (s, 1 - dst.a, d, 1 - src.a) + select (two (s) <= src.a, two (s * d), src.a * dst.a - two ((dst.a - d) * (src.a - s)));
            });

        case SkBlendMode::kOverlay:
            return apply_rgb_srcover_a ([&] (auto s, auto d) {
                return mma (s, 1 - dst.a, d, 1 - src.a) + select (two (d) <= dst.a, two (s * d), src.a * dst.a - two ((dst.a - d) * (src.a - s)));
            });
        ...
        case SkBlendMode::kSoftLight:
            return apply_rgb_srcover_a ([&] (auto s, auto d) {
                auto m = select (dst.a > 0.0f, d / dst.a, 0.0f),
                     s2 = two (s),
                     m4 = 4 * m;
                ...
                auto darkSrc = d * ((s2 - src.a) * (1 - m) + src.a),
                    ...
                    liteDst = sqrt (m) - m,
                     // Used in 2 or 3?
                    liteSrc = dst.a * (s2 - src.a) * select (4 * d <= dst.a, darkDst, liteDst)
                              + d * src.a;
                return s * (1 - dst.a) + d * (1 - src.a) + select (s2 <= src.a, darkSrc, liteSrc);
            });
...
static uint8_t rex (bool W, // If set, operation is 64-bit, otherwise default, usually 32-bit.
                    ...
                    bool B)
{ // Extra top bit for SIB base or ModRM rm register.
...
static VEX vex (bool WE, // Like REX W for int operations, or opcode extension for float?
                ...
                int pp)
{ // SSE mandatory prefix: 0x66, 0xf3, 0xf2, else none.
...
void Assembler::op (int prefix, int map, int opcode, int dst, int x, Operand y, W w, L l)
{
    switch (y.kind)
    {
        ...
        case Operand::MEM:
        {
            ...
            VEX v = vex (w, dst >> 3, m.index >> 3, m.base >> 3, map, x, l, prefix);
            this->bytes (v.bytes, v.len);
            this->byte (opcode);
...
void Assembler::vgatherdps (Ymm dst, Scale scale, Ymm ix, GP64 base, Ymm mask)
{
    ...
    int prefix = 0x66,
        map = 0x380f,
        opcode = 0x92;
    VEX v = vex (0, dst >> 3, ix >> 3, base >> 3, map, mask, /*ymm?*/ 1, prefix);
    this->bytes (v.bytes, v.len);
    this->byte (opcode);
...
#define SKVM_JIT_STATS 0
    ...
    if (jit_entry != nullptr && gSkVMAllowJIT)
    {
        ...
        {
            case 0:
                return ((void (*) (int)) jit_entry) (n);
            case 1:
                return ((void (*) (int, void*)) jit_entry) (n, a[0]);
            case 2:
                return ((void (*) (int, void*, void*)) jit_entry) (n, a[0], a[1]);
            case 3:
                return ((void (*) (int, void*, void*, void*)) jit_entry) (n, a[0], a[1], a[2]);
            case 4:
                return ((void (*) (int, void*, void*, void*, void*)) jit_entry) (n, a[0], a[1], a[2], a[3]);
            case 5:
                return ((void (*) (int, void*, void*, void*, void*, void*)) jit_entry) (n, a[0], a[1], a[2], a[3], a[4]);
            case 6:
                return ((void (*) (int, void*, void*, void*, void*, void*, void*)) jit_entry) (n, a[0], a[1], a[2], a[3], a[4], a[5]);
            default:
                SkASSERT (false); // TODO: >6 args?
        }
    }
#endif

    // So we'll sometimes use the interpreter here even if later calls will use the JIT.
    SkOpts::interpret_skvm (fImpl->instructions.data(), (int) fImpl->instructions.size(), this->nregs(), this->loop(), fImpl->strides.data(), this->nargs(), n, args);
}
                         ...
                         const char* debug_name)
{
    ...
    llvm::FunctionType* fn_type = llvm::FunctionType::get (llvm::Type::getVoidTy (*ctx),
                                                           arg_types,
                                                           /*vararg?=*/false);
    llvm::Function* fn = llvm::Function::Create (fn_type, llvm::GlobalValue::ExternalLinkage, debug_name, *mod);
    for (size_t i = 0; i < fImpl->strides.size(); i++)
    {
    ...
    auto emit = [&] (size_t i, bool scalar, IRBuilder* b) {
        ...
        switch (llvm::Type* t = nullptr; op)
        {
            ...
            case Op::splat:
                vals[i] = llvm::ConstantInt::get (I32, immy);
                break;
            ...
            case Op::bit_and:
                ...
                break;
            case Op::bit_or:
                vals[i] = b->CreateOr (vals[x], vals[y]);
                break;
            case Op::bit_xor:
                vals[i] = b->CreateXor (vals[x], vals[y]);
                break;
            case Op::bit_clear:
                vals[i] = b->CreateAnd (vals[x], b->CreateNot (vals[y]));
                break;

            case Op::pack:
                vals[i] = b->CreateOr (vals[x], b->CreateShl (vals[y], immz));
                break;
            ...
            case Op::add_i32:
                vals[i] = b->CreateAdd (vals[x], vals[y]);
                break;
            case Op::sub_i32:
                vals[i] = b->CreateSub (vals[x], vals[y]);
                break;
            case Op::mul_i32:
                vals[i] = b->CreateMul (vals[x], vals[y]);
                break;

            case Op::shl_i32:
                vals[i] = b->CreateShl (vals[x], immy);
                break;
            case Op::sra_i32:
                vals[i] = b->CreateAShr (vals[x], immy);
                break;
            case Op::shr_i32:
                vals[i] = b->CreateLShr (vals[x], immy);
                break;

            case Op::eq_i32:
                vals[i] = S (I32, b->CreateICmpEQ (vals[x], vals[y]));
                break;
            case Op::gt_i32:
                vals[i] = S (I32, b->CreateICmpSGT (vals[x], vals[y]));
                break;

            case Op::add_f32:
                vals[i] = I (b->CreateFAdd (F (vals[x]), F (vals[y])));
                break;
            case Op::sub_f32:
                vals[i] = I (b->CreateFSub (F (vals[x]), F (vals[y])));
                break;
            case Op::mul_f32:
                vals[i] = I (b->CreateFMul (F (vals[x]), F (vals[y])));
                break;
            case Op::div_f32:
                vals[i] = I (b->CreateFDiv (F (vals[x]), F (vals[y])));
                break;

            case Op::eq_f32:
                vals[i] = S (I32, b->CreateFCmpOEQ (F (vals[x]), F (vals[y])));
                break;
            case Op::neq_f32:
                vals[i] = S (I32, b->CreateFCmpUNE (F (vals[x]), F (vals[y])));
                break;
            case Op::gt_f32:
                vals[i] = S (I32, b->CreateFCmpOGT (F (vals[x]), F (vals[y])));
                break;
            case Op::gte_f32:
                vals[i] = S (I32, b->CreateFCmpOGE (F (vals[x]), F (vals[y])));
                break;

            case Op::fma_f32:
                vals[i] = I (b->CreateIntrinsic (llvm::Intrinsic::fma, { F32 }, { F (vals[x]), F (vals[y]), F (vals[z]) }));
                break;

            case Op::fms_f32:
                vals[i] = I (b->CreateIntrinsic (llvm::Intrinsic::fma, { F32 }, { F (vals[x]), F (vals[y]), b->CreateFNeg (F (vals[z])) }));
                break;

            case Op::fnma_f32:
                vals[i] = I (b->CreateIntrinsic (llvm::Intrinsic::fma, { F32 }, { b->CreateFNeg (F (vals[x])), F (vals[y]), F (vals[z]) }));
                break;
            ...
            case Op::to_f32:
                vals[i] = I (b->CreateSIToFP (vals[x], F32));
                break;
            case Op::trunc:
                vals[i] = b->CreateFPToSI (F (vals[x]), I32);
                break;
            case Op::round:
            {
    ...
    // loopK:  ... insts on K x T vectors; N -= K, args += K*stride; goto testK;
    {
        ...
        for (size_t i = 0; i < fImpl->strides.size(); i++)
        {
            llvm::Value* arg_next = b.CreateConstInBoundsGEP1_32 (nullptr, args[i], K * fImpl->strides[i]);
    ...
    // loop1:  ... insts on scalars; N -= 1, args += stride; goto test1;
    {
        ...
        for (size_t i = 0; i < fImpl->strides.size(); i++)
        {
            llvm::Value* arg_next = b.CreateConstInBoundsGEP1_32 (nullptr, args[i], fImpl->strides[i]);
                                        ...
                                        .create())
    {
        ...
        fImpl->llvm_compiling = std::async (std::launch::async, [dst = &fImpl->jit_entry, ee = fImpl->llvm_ee.get(), name = std::string (debug_name)] {
            // std::atomic<void*>*    dst;
    ...
    *registers_used = 0; // We'll update this as we go.
    ...
    auto emit = [&] (Val id, bool scalar) {
        ...
        switch (op)
        {
            ...
            case Op::uniform8:
                a->movzbq (GP0, A::Mem { arg[immy], immz });
                a->vmovd ((A::Xmm) dst(), GP0);
                a->vbroadcastss (dst(), dst());
                break;

            case Op::uniform16:
                a->movzwq (GP0, A::Mem { arg[immy], immz });
                a->vmovd ((A::Xmm) dst(), GP0);
                a->vbroadcastss (dst(), dst());
                break;

            case Op::uniform32:
                a->vbroadcastss (dst(), A::Mem { arg[immy], immz });
                break;
            ...
            case Op::add_f32:
                if (in_reg (x))
                {
                    a->vaddps (dst (x), r (x), any (y));
                }
                else
                {
                    a->vaddps (dst (y), r (y), any (x));
                }
                break;
            ...
            case Op::mul_f32:
                if (in_reg (x))
                {
                    a->vmulps (dst (x), r (x), any (y));
                }
                else
                {
                    a->vmulps (dst (y), r (y), any (x));
                }
                break;

            case Op::sub_f32:
                a->vsubps (dst (x), r (x), any (y));
                break;
            case Op::div_f32:
                a->vdivps (dst (x), r (x), any (y));
                break;
            case Op::min_f32:
                a->vminps (dst (y), r (y), any (x));
                break; // Order matters,
            case Op::max_f32:
                a->vmaxps (dst (y), r (y), any (x));
                break; // see test SkVM_min_max.

            case Op::fma_f32:
                if (try_alias (x))
                {
                    a->vfmadd132ps (dst (x), r (z), any (y));
                }
                else if (try_alias (y))
                {
                    a->vfmadd213ps (dst (y), r (x), any (z));
                }
                else if (try_alias (z))
                {
                    a->vfmadd231ps (dst (z), r (x), any (y));
                }
                else
                {
                    a->vmovups (dst(), any (x));
            ...
            case Op::fms_f32:
                if (try_alias (x))
                {
                    a->vfmsub132ps (dst (x), r (z), any (y));
                }
                else if (try_alias (y))
                {
                    a->vfmsub213ps (dst (y), r (x), any (z));
                }
                else if (try_alias (z))
                {
                    a->vfmsub231ps (dst (z), r (x), any (y));
                }
                else
                {
                    a->vmovups (dst(), any (x));
            ...
            case Op::fnma_f32:
                if (try_alias (x))
                {
                    a->vfnmadd132ps (dst (x), r (z), any (y));
                }
                else if (try_alias (y))
                {
                    a->vfnmadd213ps (dst (y), r (x), any (z));
                }
                else if (try_alias (z))
                {
                    a->vfnmadd231ps (dst (z), r (x), any (y));
                }
                else
                {
                    a->vmovups (dst(), any (x));
            ...
            case Op::sub_i32:
                a->vpsubd (dst (x), r (x), any (y));
                break;
            case Op::sub_q14:
                a->vpsubw (dst (x), r (x), any (y));
                break;
            ...
            case Op::bit_xor_q14:
                ...
                {
                    a->vpxor (dst (y), r (y), any (x));
                }
                break;

            case Op::bit_clear:
            case Op::bit_clear_q14:
                a->vpandn (dst (y), r (y), any (x));
                break; // Notice, y then x.
            ...
            case Op::select_q14:
                ...
                {
                    a->vpblendvb (dst (x), r (z), any (y), r (x));
                }
                break;
            ...
            case Op::shl_i32:
                a->vpslld (dst (x), r (x), immy);
                break;
            case Op::shr_i32:
                a->vpsrld (dst (x), r (x), immy);
                break;
            case Op::sra_i32:
                a->vpsrad (dst (x), r (x), immy);
                break;

            case Op::shl_q14:
                a->vpsllw (dst (x), r (x), immy);
                break;
            case Op::shr_q14:
                a->vpsrlw (dst (x), r (x), immy);
                break;
            case Op::sra_q14:
                a->vpsraw (dst (x), r (x), immy);
                break;

            case Op::eq_i32:
                if (in_reg (x))
                {
                    a->vpcmpeqd (dst (x), r (x), any (y));
                }
                else
                {
                    a->vpcmpeqd (dst (y), r (y), any (x));
                }
                break;
            ...
            case Op::gt_i32:
                a->vpcmpgtd (dst(), r (x), any (y));
                break;
            case Op::gt_q14:
                a->vpcmpgtw (dst(), r (x), any (y));
                break;

            case Op::eq_f32:
                if (in_reg (x))
                {
                    a->vcmpeqps (dst (x), r (x), any (y));
                }
                else
                {
                    a->vcmpeqps (dst (y), r (y), any (x));
                }
                break;
            case Op::neq_f32:
                if (in_reg (x))
                {
                    a->vcmpneqps (dst (x), r (x), any (y));
                }
                else
                {
                    a->vcmpneqps (dst (y), r (y), any (x));
                }
                break;

            case Op::gt_f32:
                a->vcmpltps (dst (y), r (y), any (x));
                break;
            case Op::gte_f32:
                a->vcmpleps (dst (y), r (y), any (x));
                break;
            ...
            case Op::floor:
                if (in_reg (x))
                {
                    a->vroundps (dst (x), r (x), Assembler::FLOOR);
                }
                else
                {
                    a->vroundps (dst(), any (x), Assembler::FLOOR);
                }
                break;
            ...
            case Op::to_f32:
                if (in_reg (x))
                {
                    a->vcvtdq2ps (dst (x), r (x));
                }
                else
                {
                    a->vcvtdq2ps (dst(), any (x));
                }
                break;
            ...
            case Op::trunc:
                if (in_reg (x))
                {
                    a->vcvttps2dq (dst (x), r (x));
                }
                else
                {
                    a->vcvttps2dq (dst(), any (x));
                }
                break;
            ...
            case Op::round:
                if (in_reg (x))
                {
                    a->vcvtps2dq (dst (x), r (x));
                }
                else
                {
                    a->vcvtps2dq (dst(), any (x));
                }
                break;
            ...
            case Op::add_f32:
                a->fadd4s (dst(), r (x), r (y));
                break;
            case Op::sub_f32:
                a->fsub4s (dst(), r (x), r (y));
                break;
            case Op::mul_f32:
                a->fmul4s (dst(), r (x), r (y));
                break;
            case Op::div_f32:
                a->fdiv4s (dst(), r (x), r (y));
                break;

            case Op::fma_f32: // fmla.4s is z += x*y
                if (try_alias (z))
                {
                    a->fmla4s (r (z), r (x), r (y));
                }
                else
                {
            ...
            case Op::gt_f32:
                ...
                break;
            case Op::gte_f32:
                a->fcmge4s (dst(), r (x), r (y));
                break;
            case Op::eq_f32:
                a->fcmeq4s (dst(), r (x), r (y));
                break;
            case Op::neq_f32:
                a->fcmeq4s (dst(), r (x), r (y));
                a->not16b (dst(), dst());
            ...
            case Op::bit_and:
                a->and16b (dst(), r (x), r (y));
                break;
            case Op::bit_or:
                a->orr16b (dst(), r (x), r (y));
                break;
            case Op::bit_xor:
                a->eor16b (dst(), r (x), r (y));
                break;
            case Op::bit_clear:
                a->bic16b (dst(), r (x), r (y));
                break;

            case Op::select: // bsl16b is x = x ? y : z
                if (try_alias (x))
                {
                    a->bsl16b (r (x), r (y), r (z));
                }
                else
                {
            ...
            case Op::shl_i32:
                a->shl4s (dst(), r (x), immy);
                break;
            case Op::shr_i32:
                a->ushr4s (dst(), r (x), immy);
                break;
            case Op::sra_i32:
                a->sshr4s (dst(), r (x), immy);
                break;

            case Op::eq_i32:
                a->cmeq4s (dst(), r (x), r (y));
                break;
            case Op::gt_i32:
                a->cmgt4s (dst(), r (x), r (y));
                break;

            case Op::pack:
                if (try_alias (x))
                {
                    a->sli4s (r (x), r (y), immz);
                }
                else
                {
        ...
        if (rd != NA && dies_here (regs[rd]))
        {
            regs[rd] = NA;
        }
        if (rx != NA && regs[rx] != NA && dies_here (regs[rx]))
        {
            regs[rx] = NA;
        }
        if (ry != NA && regs[ry] != NA && dies_here (regs[ry]))
        {
            regs[ry] = NA;
        }
    ...
    auto restore_incoming_regs = [&, incoming = regs, saved_stack_slot = stack_slot, saved_next_stack_slot = next_stack_slot] {
