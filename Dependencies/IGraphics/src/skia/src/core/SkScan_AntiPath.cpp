...
{
public:
    BaseSuperBlitter (SkBlitter* realBlitter, const SkIRect& ir, const SkIRect& clipBounds, bool isInverse);
...
BaseSuperBlitter::BaseSuperBlitter (SkBlitter* realBlit, const SkIRect& ir, const SkIRect& clipBounds, bool isInverse)
{
...
{
public:
    SuperBlitter (SkBlitter* realBlitter, const SkIRect& ir, const SkIRect& clipBounds, bool isInverse);
...
SuperBlitter::SuperBlitter (SkBlitter* realBlitter, const SkIRect& ir, const SkIRect& clipBounds, bool isInverse)
...
void SuperBlitter::blitH (int x, int y, int width)
{
    ...
    fOffsetX = fRuns.add (x >> SHIFT, coverage_to_partial_alpha (fb), n, coverage_to_partial_alpha (fe), (1 << (8 - SHIFT)) - (((y & MASK) + 1) >> SHIFT), fOffsetX);
...
void SuperBlitter::blitRect (int x, int y, int width, int height)
{
    ...
    if (count > 0)
    {
        ...
        if (n < 0)
        {
            ...
            SkASSERT (xleft > 0);
            fRealBlitter->blitV (ileft + fLeft, start_y, count, coverage_to_exact_alpha (xleft));
        }
        else
        {
            ...
            fRealBlitter->blitAntiRect (ileft + fLeft, start_y, n, count, coverageL, coverageR);
        }
...
MaskSuperBlitter::MaskSuperBlitter (SkBlitter* realBlitter, const SkIRect& ir, const SkIRect& clipBounds, bool isInverse)
    : BaseSuperBlitter (realBlitter, ir, clipBounds, isInverse)
{
...
void MaskSuperBlitter::blitH (int x, int y, int width)
{
    ...
    else
    {
        ...
        add_aa_span (row, coverage_to_partial_alpha (fb), n, coverage_to_partial_alpha (fe), (1 << (8 - SHIFT)) - (((y & MASK) + 1) >> SHIFT));
    }
...
static void compute_complexity (const SkPath& path, SkScalar& avgLength, SkScalar& complexity)
{
    ...
    if (sk_float_isnan (complexity))
    { // it may be possible to have 0.0 / 0.0; inf is fine for us.
        complexity = -1;
    }
}
...
void SkScan::SAAFillPath (const SkPath& path, SkBlitter* blitter, const SkIRect& ir, const SkIRect& clipBounds, bool forceRLE)
{
...
void SkScan::AntiFillPath (const SkPath& path, const SkRegion& origClip, SkBlitter* blitter, bool forceRLE)
{
    if (origClip.isEmpty())
    {
        return;
    ...
    SkASSERT (clipper.getClipRect() == nullptr || *clipper.getClipRect() == clipRgn->getBounds());
