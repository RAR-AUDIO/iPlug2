...
static void load_color (SkRasterPipeline_UniformColorCtx* ctx, const float rgba[])
{
    ...
    ctx->r = rgba[0];
    ctx->rgba[1] = SkScalarRoundToInt (rgba[1] * 255);
    ctx->g = rgba[1];
    ctx->rgba[2] = SkScalarRoundToInt (rgba[2] * 255);
    ctx->b = rgba[2];
    ctx->rgba[3] = SkScalarRoundToInt (rgba[3] * 255);
    ctx->a = rgba[3];
}

void SkDraw::drawAtlas (const SkImage* atlas, const SkRSXform xform[], const SkRect textures[], const SkColor colors[], int count, SkBlendMode bmode, const SkPaint& paint)
{
    ...
    SkColorSpaceXformSteps steps (sk_srgb_singleton(), kUnpremul_SkAlphaType, rec.fDstCS, kUnpremul_SkAlphaType);
    ...
    if (auto blitter = SkCreateRasterPipelineBlitter (fDst, p, pipeline, isOpaque, &alloc, fRC->clipShader()))
    {
        SkPath scratchPath;
