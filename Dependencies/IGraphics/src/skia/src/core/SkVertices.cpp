...
SkVertices::Attribute::Attribute (Type t, Usage u, const char* markerName)
    : fType (t), fUsage (u), fMarkerName (markerName)
{
...
bool SkVertices::Attribute::isValid() const
{
    ...
    switch (fUsage)
    {
        case Usage::kRaw:
            return fMarkerID == 0;
        case Usage::kColor:
            return fMarkerID == 0 && (fType == Type::kFloat3 || fType == Type::kFloat4 || fType == Type::kByte4_unorm);
        case Usage::kVector:
        case Usage::kNormalVector:
        case Usage::kPosition:
...
SkVertices::Builder::Builder (VertexMode mode, int vertexCount, int indexCount, uint32_t builderFlags)
{
...
void SkVertices::Builder::init (const Desc& desc)
{
    ...
    sk_careful_memcpy (fVertices->fAttributes, desc.fAttributes, desc.fAttributeCount * sizeof (Attribute));
...
sk_sp<SkVertices> SkVertices::MakeCopy (VertexMode mode, int vertexCount, const SkPoint pos[], const SkPoint texs[], const SkColor colors[], int indexCount, const uint16_t indices[])
{
...
sk_sp<SkVertices> SkVerticesPriv::Decode (SkReadBuffer& buffer)
{
    ...
    auto decode = [] (SkReadBuffer& buffer) -> sk_sp<SkVertices> {
        ...
        if (! safe // Invalid header fields
            ...
            || (attrCount > 0 && (hasTexs || hasColors)))
        { // Overspecified (incompatible features)
            return nullptr;
        }
        ...
        const SkVertices::Desc desc { mode, vertexCount, indexCount, hasTexs, hasColors, attrCount ? attrs : nullptr, attrCount };
