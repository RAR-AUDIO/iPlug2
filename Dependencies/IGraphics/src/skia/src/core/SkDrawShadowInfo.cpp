...
bool GetSpotShadowTransform (const SkPoint3& lightPos, SkScalar lightRadius, const SkMatrix& ctm, const SkPoint3& zPlaneParams, const SkRect& pathBounds, SkMatrix* shadowTransform, SkScalar* radius)
{
    ...
    if (! ctm.hasPerspective())
    {
        SkScalar scale;
        SkVector translate;
        SkDrawShadowMetrics::GetSpotParams (occluderHeight, lightPos.fX, lightPos.fY, lightPos.fZ, lightRadius, radius, &scale, &translate);
    ...
    else
    {
        ...
        shadowTransform->setAll (h0.fX / h2.fZ, h1.fX / h2.fZ, h2.fX / h2.fZ, h0.fY / h2.fZ, h1.fY / h2.fZ, h2.fY / h2.fZ, h0.fZ / h2.fZ, h1.fZ / h2.fZ, 1);
        ...
        toHomogeneous.setAll (xScale, 0, -xScale * pathBounds.fLeft - 1, 0, yScale, -yScale * pathBounds.fTop - 1, 0, 0, 1);
...
void GetLocalBounds (const SkPath& path, const SkDrawShadowRec& rec, const SkMatrix& ctm, SkRect* bounds)
{
    ...
    else
    {
        ...
        occluderZ = std::max (occluderZ, compute_z (ambientBounds.fRight, ambientBounds.fTop, rec.fZPlaneParams));
        occluderZ = std::max (occluderZ, compute_z (ambientBounds.fLeft, ambientBounds.fBottom, rec.fZPlaneParams));
        occluderZ = std::max (occluderZ, compute_z (ambientBounds.fRight, ambientBounds.fBottom, rec.fZPlaneParams));
    }
    ...
    if (ctm.hasPerspective())
    {
        ...
        SkDrawShadowMetrics::GetSpotParams (occluderZ, devLightPos.fX, devLightPos.fY, rec.fLightPos.fZ, rec.fLightRadius, &spotBlur, &spotScale, &spotOffset);
    }
    else
    {
        ...
        SkDrawShadowMetrics::GetSpotParams (occluderZ, rec.fLightPos.fX, rec.fLightPos.fY, rec.fLightPos.fZ, rec.fLightRadius, &spotBlur, &spotScale, &spotOffset);
