...
void SkTypeface::serialize (SkWStream* wstream, SerializeBehavior behavior) const
{
    ...
    switch (behavior)
    {
        ...
        case SerializeBehavior::kIncludeDataIfLocal:
            shouldSerializeData = isLocalData;
            break;
    }
...
int SkTypeface::getVariationDesignParameters (
    SkFontParameters::Variation::Axis parameters[],
...
bool SkTypeface::getKerningPairAdjustments (const uint16_t glyphs[], int count, int32_t adjustments[]) const
{
    SkASSERT (count >= 0);
...
bool SkTypeface::onGetKerningPairAdjustments (const uint16_t glyphs[], int count, int32_t adjustments[]) const
{
    return false;
}
...
bool SkTypeface::onComputeBounds (SkRect* bounds) const
{
    ...
    bounds->setLTRB (fm.fXMin * invTextSize, fm.fTop * invTextSize, fm.fXMax * invTextSize, fm.fBottom * invTextSize);
    return true;
}
