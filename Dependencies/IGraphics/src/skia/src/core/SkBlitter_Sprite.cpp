...
public:
    SkRasterPipelineSpriteBlitter (const SkPixmap& src, SkArenaAlloc* alloc, sk_sp<SkShader> clipShader)
        : INHERITED (src), fAlloc (alloc), fBlitter (nullptr), fSrcPtr { nullptr, 0 }, fClipShader (std::move (clipShader))
    {
    }
...
SkBlitter* SkBlitter::ChooseSprite (const SkPixmap& dst, const SkPaint& paint, const SkPixmap& source, int left, int top, SkArenaAlloc* alloc, sk_sp<SkShader> clipShader)
{
