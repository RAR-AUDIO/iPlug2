...
static inline bool is_degenerate_2x2 (SkScalar scaleX, SkScalar skewX, SkScalar skewY, SkScalar scaleY)
{
...
static inline SkScalar sdot (SkScalar a, SkScalar b, SkScalar c, SkScalar d, SkScalar e, SkScalar f)
{
    return a * b + c * d + e * f;
}
...
{
    *this = SkMatrix (1, 0, dx, 0, 1, dy, 0, 0, 1, (dx != 0 || dy != 0) ? kTranslate_Mask | kRectStaysRect_Mask : kIdentity_Mask | kRectStaysRect_Mask);
    return *this;
}
...
{
    *this = SkMatrix (sx, 0, 0, 0, sy, 0, 0, 0, 1, (sx == 1 && sy == 1) ? kIdentity_Mask | kRectStaysRect_Mask : kScale_Mask | kRectStaysRect_Mask);
    return *this;
}
...
{
    *this = SkMatrix (1, sx, -sx * py, sy, 1, -sy * px, 0, 0, 1, kUnknown_Mask | kOnlyPerspectiveValid_Mask);
    return *this;
}
...
static double sk_determinant (const float mat[9], int isPerspective)
{
    if (isPerspective)
    {
        return mat[SkMatrix::kMScaleX] * dcross (mat[SkMatrix::kMScaleY], mat[SkMatrix::kMPersp2], mat[SkMatrix::kMTransY], mat[SkMatrix::kMPersp1])
    ...
    {
        return dcross (mat[SkMatrix::kMScaleX], mat[SkMatrix::kMScaleY], mat[SkMatrix::kMSkewX], mat[SkMatrix::kMSkewY]);
    }
}
...
void SkMatrix::Persp_pts (const SkMatrix& m, SkPoint dst[], const SkPoint src[], int count)
{
...
const SkMatrix::MapPtsProc SkMatrix::gMapPtsProcs[] = {
    SkMatrix::Identity_pts,
    SkMatrix::Trans_pts,
    SkMatrix::Scale_pts,
    SkMatrix::Scale_pts,
    SkMatrix::Affine_vpts,
    SkMatrix::Affine_vpts,
    SkMatrix::Affine_vpts,
    SkMatrix::Affine_vpts,
...
void SkMatrixPriv::MapHomogeneousPointsWithStride (const SkMatrix& mx, SkPoint3 dst[], size_t dstStride, const SkPoint3 src[], size_t srcStride, int count)
{
    SkASSERT ((dst && src && count > 0) || 0 == count);
    // no partial overlap
    SkASSERT (src == dst || &dst[count] <= &src[0] || &src[count] <= &dst[0]);
...
{
    SkMatrixPriv::MapHomogeneousPointsWithStride (*this, dst, sizeof (SkPoint3), src, sizeof (SkPoint3), count);
}
...
void SkMatrix::Persp_xy (const SkMatrix& m, SkScalar sx, SkScalar sy, SkPoint* pt)
{
    SkASSERT (m.hasPerspective());
...
void SkMatrix::RotTrans_xy (const SkMatrix& m, SkScalar sx, SkScalar sy, SkPoint* pt)
{
...
void SkMatrix::Rot_xy (const SkMatrix& m, SkScalar sx, SkScalar sy, SkPoint* pt)
{
    SkASSERT ((m.getType() & (kAffine_Mask | kPerspective_Mask)) == kAffine_Mask);
...
void SkMatrix::ScaleTrans_xy (const SkMatrix& m, SkScalar sx, SkScalar sy, SkPoint* pt)
{
...
void SkMatrix::Scale_xy (const SkMatrix& m, SkScalar sx, SkScalar sy, SkPoint* pt)
{
    SkASSERT ((m.getType() & (kScale_Mask | kAffine_Mask | kPerspective_Mask))
              == kScale_Mask);
    SkASSERT (0 == m.fMat[kMTransX]);
...
void SkMatrix::Trans_xy (const SkMatrix& m, SkScalar sx, SkScalar sy, SkPoint* pt)
{
...
void SkMatrix::Identity_xy (const SkMatrix& m, SkScalar sx, SkScalar sy, SkPoint* pt)
{
...
const SkMatrix::MapXYProc SkMatrix::gMapXYProcs[] = {
    ...
    SkMatrix::Trans_xy,
    SkMatrix::Scale_xy,
    SkMatrix::ScaleTrans_xy,
    SkMatrix::Rot_xy,
    SkMatrix::RotTrans_xy,
    SkMatrix::Rot_xy,
    SkMatrix::RotTrans_xy,
    // repeat the persp proc 8 times
    SkMatrix::Persp_xy,
...
{
    static constexpr SkMatrix invalid (SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, SK_ScalarMax, kTranslate_Mask | kScale_Mask | kAffine_Mask | kPerspective_Mask);
    return invalid;
}
...
SkScalar SkMatrixPriv::DifferentialAreaScale (const SkMatrix& m, const SkPoint& p)
{
    ...
    SkMatrix jacobian = SkMatrix::MakeAll (xyw.fX, xyw.fY, xyw.fZ, m.getScaleX(), m.getSkewY(), m.getPerspX(), m.getSkewX(), m.getScaleY(), m.getPerspY());
