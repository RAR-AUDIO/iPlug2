    ...
    if (clip)
    {
        ...
        if (clip->isRect())
        {
            ...
            else
            {
                ...
                if (irPreClipped || fClipRect->fLeft > ir.fLeft || fClipRect->fRight < ir.fRight)
                {
...
void SkScan::FillPath (const SkPath& path, const SkRegion& origClip, SkBlitter* blitter)
{
    ...
    if (blitter)
    {
        ...
        SkASSERT (clipper.getClipRect() == nullptr || *clipper.getClipRect() == clipPtr->getBounds());
        sk_fill_path (path, clipPtr->getBounds(), blitter, ir.fTop, ir.fBottom, 0, clipper.getClipRect() == nullptr);
        if (path.isInverseFillType())
        {
