...
void SkRRect::setNinePatch (const SkRect& rect, SkScalar leftRad, SkScalar topRad, SkScalar rightRad, SkScalar bottomRad)
{
    if (! this->initializeRect (rect))
    {
        return;
...
bool SkRRect::checkCornerContainment (SkScalar x, SkScalar y) const
{
    ...
    SkScalar dist = SkScalarSquare (canonicalPt.fX) * SkScalarSquare (fRadii[index].fY) + SkScalarSquare (canonicalPt.fY) * SkScalarSquare (fRadii[index].fX);
...
{
    return SkScalarNearlyEqual (rr.fRadii[0].fX, rr.fRadii[0].fY, tolerance) && SkScalarNearlyEqual (rr.fRadii[1].fX, rr.fRadii[1].fY, tolerance) && SkScalarNearlyEqual (rr.fRadii[2].fX, rr.fRadii[2].fY, tolerance) && SkScalarNearlyEqual (rr.fRadii[3].fX, rr.fRadii[3].fY, tolerance);
}
...
bool SkRRectPriv::ReadFromBuffer (SkRBuffer* buffer, SkRRect* rr)
{
    ...
    SkRRect storage;
    return buffer->read (&storage, SkRRect::kSizeInMemory) && (rr->readFromMemory (&storage, SkRRect::kSizeInMemory) == SkRRect::kSizeInMemory);
}
...
bool SkRRect::isValid() const
{
    ...
    switch (fType)
    {
        ...
        case kOval_Type:
            ...
            for (int i = 0; i < 4; ++i)
            {
                if (! SkScalarNearlyEqual (fRadii[i].fX, SkScalarHalf (fRect.width())) || ! SkScalarNearlyEqual (fRadii[i].fY, SkScalarHalf (fRect.height())))
                {
                    return false;
                }
            }
...
bool SkRRect::AreRectAndRadiiValid (const SkRect& rect, const SkVector radii[4])
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        if (! are_radius_check_predicates_valid (radii[i].fX, rect.fLeft, rect.fRight) || ! are_radius_check_predicates_valid (radii[i].fY, rect.fTop, rect.fBottom))
        {
            return false;
        }
    }
...
SkRRect SkRRectPriv::ConservativeIntersect (const SkRRect& a, const SkRRect& b)
{
    ...
    auto getCorner = [] (const SkRect& r, SkRRect::Corner corner) -> SkPoint {
        switch (corner)
        {
            case SkRRect::kUpperLeft_Corner:
                return { r.fLeft, r.fTop };
            case SkRRect::kUpperRight_Corner:
                return { r.fRight, r.fTop };
            case SkRRect::kLowerLeft_Corner:
                return { r.fLeft, r.fBottom };
            case SkRRect::kLowerRight_Corner:
                return { r.fRight, r.fBottom };
            default:
                SkUNREACHABLE;
        }
    };
    ...
    auto getIntersectionRadii = [&] (const SkRect& r, SkRRect::Corner corner, SkVector* radii) {
        ...
        else
        {
            ...
            *radii = { 0.f, 0.f };
            return a.checkCornerContainment (test.fX, test.fY) && b.checkCornerContainment (test.fX, test.fY);
        }
    };
    ...
    if (! SkRRect::AreRectAndRadiiValid (intersection.fRect, intersection.fRadii) || intersection.scaleRadii())
    {
        return SkRRect::MakeEmpty();
    }
