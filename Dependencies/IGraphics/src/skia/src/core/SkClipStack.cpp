...
SkClipStack::Element::~Element()
{
    ...
    {
        fProxyProvider->processInvalidUniqueKey (fKeysToInvalidate[i], nullptr, GrProxyProvider::InvalidateGPUResource::kYes);
    }
#endif
}
...
bool SkClipStack::Element::operator== (const Element& element) const
{
    ...
    if (fOp != element.fOp || fDeviceSpaceType != element.fDeviceSpaceType || fDoAA != element.fDoAA || fSaveCount != element.fSaveCount)
    {
        return false;
    }
...
void SkClipStack::Element::initRect (int saveCount, const SkRect& rect, const SkMatrix& m, SkClipOp op, bool doAA)
{
    if (m.rectStaysRect())
    {
        SkRect devRect;
...
void SkClipStack::Element::initRRect (int saveCount, const SkRRect& rrect, const SkMatrix& m, SkClipOp op, bool doAA)
{
...
void SkClipStack::Element::initPath (int saveCount, const SkPath& path, const SkMatrix& m, SkClipOp op, bool doAA)
{
...
void SkClipStack::Element::initAsPath (int saveCount, const SkPath& path, const SkMatrix& m, SkClipOp op, bool doAA)
{
...
{
    if (DeviceSpaceType::kEmpty == fDeviceSpaceType && (kDifference_SkClipOp == op || kIntersect_SkClipOp == op))
    {
        return true;
    }
    // Only clips within the same save/restore frame (as captured by
    // the save count) can be merged
    return fSaveCount == saveCount && kIntersect_SkClipOp == op && (kIntersect_SkClipOp == fOp || kReplace_SkClipOp == fOp);
}
...
void SkClipStack::Element::updateBoundAndGenID (const Element* prior)
{
    ...
    switch (fDeviceSpaceType)
    {
        case DeviceSpaceType::kRect:
            ...
            if (kReplace_SkClipOp == fOp || (kIntersect_SkClipOp == fOp && nullptr == prior) || (kIntersect_SkClipOp == fOp && prior->fIsIntersectionOfRects && prior->rectRectIntersectAllowed (this->getDeviceSpaceRect(), fDoAA)))
            {
...
void SkClipStack::clipRRect (const SkRRect& rrect, const SkMatrix& matrix, SkClipOp op, bool doAA)
{
    ...
    {
        Element restriction (fSaveCount, fClipRestrictionRect, SkMatrix::I(), kIntersect_SkClipOp, false);
...
void SkClipStack::clipPath (const SkPath& path, const SkMatrix& matrix, SkClipOp op, bool doAA)
{
...
bool SkClipStack::isRRect (const SkRect& bounds, SkRRect* rrect, bool* aa) const
{
    ...
    if (back->getDeviceSpaceType() != SkClipStack::Element::DeviceSpaceType::kRect && back->getDeviceSpaceType() != SkClipStack::Element::DeviceSpaceType::kRRect)
    {
        return false;
    }
    ...
    if (back->getOp() == kIntersect_SkClipOp)
    {
        ...
        if (cnt > 1)
        {
            ...
            {
                if ((prior->getOp() != kIntersect_SkClipOp && prior->getOp() != kReplace_SkClipOp) || ! prior->contains (backBounds))
                {
                    return false;
                }
...
uint32_t SkClipStack::getTopmostGenID() const
{
    ...
    const Element* back = static_cast<const Element*> (fDeque.back());
    if (kInsideOut_BoundsType == back->fFiniteBoundType && back->fFiniteBound.isEmpty() && Element::DeviceSpaceType::kShader != back->fDeviceSpaceType)
    {
        return kWideOpenGenID;
    }
...
void SkClipStack::Element::dump() const
{
    ...
    SkDebugf ("Type: %s, Op: %s, AA: %s, Save Count: %d\n", kTypeStrings[(int) fDeviceSpaceType], kOpStrings[static_cast<int> (fOp)], (fDoAA ? "yes" : "no"), fSaveCount);
    switch (fDeviceSpaceType)
    {
        case DeviceSpaceType::kEmpty:
            SkDebugf ("\n");
