...
bool SkPicture::IsValidPictInfo (const SkPictInfo& info)
{
    ...
    if (info.getVersion() < SkPicturePriv::kMin_Version || info.getVersion() > SkPicturePriv::kCurrent_Version)
    {
        return false;
    }
    return true;
...
sk_sp<SkPicture> SkPicture::MakeFromData (const void* data, size_t size, const SkDeserialProcs* procs)
{
    if (! data)
    {
        return nullptr;
...
sk_sp<SkPicture> SkPicture::MakeFromStream (SkStream* stream, const SkDeserialProcs* procsPtr, SkTypefacePlayback* typefaces)
{
    SkPictInfo info;
...
// to fill typefaceSet.
void SkPicture::serialize (SkWStream* stream, const SkSerialProcs* procsPtr, SkRefCntSet* typefaceSet, bool textBlobsOnly) const
{
    SkSerialProcs procs;
    if (procsPtr)
    {
