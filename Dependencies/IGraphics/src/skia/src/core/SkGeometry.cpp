...
inline static skvx::Vec<N, T> unchecked_mix (const skvx::Vec<N, T>& a, const skvx::Vec<N, T>& b, const skvx::Vec<N, T>& t)
{
    return (b - a) * t + a;
}
...
float SkFindCubicMidTangent (const SkPoint src[4])
{
    ...
    static const Sk4f kM[4] = { Sk4f (-1, 2, -1, 0),
                                Sk4f (3, -4, 1, 0),
                                Sk4f (-3, 2, 0, 0) };
    Sk4f C_x = fma (kM[0], src[0].fX, fma (kM[1], src[1].fX, fma (kM[2], src[2].fX, Sk4f (src[3].fX, 0, 0, 0))));
    Sk4f C_y = fma (kM[0], src[0].fY, fma (kM[1], src[1].fY, fma (kM[2], src[2].fY, Sk4f (src[3].fY, 0, 0, 0))));
...
{
    SkScalar tValues[2];
    int roots = SkFindCubicExtrema (src[0].fX, src[1].fX, src[2].fX, src[3].fX, tValues);
...
inline static void write_cubic_inflection_roots (double t0, double s0, double t1, double s1, double* t, double* s)
{
    t[0] = t0;
    s[0] = s0;
...
static bool cubic_dchop_at_intercept (const SkPoint src[4], SkScalar intercept, SkPoint dst[7], InterceptProc method)
{
    SkDCubic cubic;
    double roots[3];
...
int SkConic::chopIntoQuadsPOW2 (SkPoint pts[], int pow2) const
{
    ...
    *pts = fPts[0];
    ...
    { // If an extreme weight generates many quads ...
        ...
        if (SkPointPriv::EqualsWithinTolerance (dst[0].fPts[1], dst[0].fPts[2]) && SkPointPriv::EqualsWithinTolerance (dst[1].fPts[0], dst[1].fPts[1]))
        {
...
int SkConic::BuildUnitArc (const SkVector& uStart, const SkVector& uStop, SkRotationDirection dir, const SkMatrix* userMatrix, SkConic dst[kMaxConicsForArc])
{
