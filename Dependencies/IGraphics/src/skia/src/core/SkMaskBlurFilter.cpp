...
public:
    ...
    {
    public:
        Scan (uint64_t weight, int noChangeCount, uint32_t* buffer0, uint32_t* buffer0End, uint32_t* buffer1, uint32_t* buffer1End, uint32_t* buffer2, uint32_t* buffer2End)
            : fWeight { weight }, fNoChangeCount { noChangeCount }, fBuffer0 { buffer0 }, fBuffer0End { buffer0End }, fBuffer1 { buffer1 }, fBuffer1End { buffer1End }, fBuffer2 { buffer2 }, fBuffer2End { buffer2End }
        {
        }

        template <typename AlphaIter>
        void blur (const AlphaIter srcBegin, const AlphaIter srcEnd, uint8_t* dst, int dstStride, uint8_t* dstEnd) const
        {
    ...
    Scan makeBlurScan (int width, uint32_t* buffer) const
    {
        ...
        return Scan (
            fWeight, noChangeCount, buffer0, buffer0End, buffer1, buffer1End, buffer2, buffer2End);
    }
...
static void blur_y_rect (ToA8 toA8, const int strideOf8, BlurY blur, int radius, uint16_t* gauss, const uint8_t* src, size_t srcRB, int srcW, int srcH, uint8_t* dst, size_t dstRB)
{
    ...
    for (; x <= srcW - 8; x += 8)
    {
        blur_column (toA8, blur, radius, 8, g0, g1, g2, g3, g4, src, srcRB, srcH, dst, dstRB);
        src += strideOf8;
        dst += 8;
    }
    ...
    if (xTail > 0)
    {
        blur_column (toA8, blur, radius, xTail, g0, g1, g2, g3, g4, src, srcRB, srcH, dst, dstRB);
    }
}
...
static void direct_blur_y (ToA8 toA8, const int strideOf8, int radius, uint16_t* gauss, const uint8_t* src, size_t srcRB, int srcW, int srcH, uint8_t* dst, size_t dstRB)
{
    ...
    {
        case 1:
            blur_y_rect (toA8, strideOf8, blur_y_radius_1, 1, gauss, src, srcRB, srcW, srcH, dst, dstRB);
            break;

        case 2:
            blur_y_rect (toA8, strideOf8, blur_y_radius_2, 2, gauss, src, srcRB, srcW, srcH, dst, dstRB);
            break;

        case 3:
            blur_y_rect (toA8, strideOf8, blur_y_radius_3, 3, gauss, src, srcRB, srcW, srcH, dst, dstRB);
            break;

        case 4:
            blur_y_rect (toA8, strideOf8, blur_y_radius_4, 4, gauss, src, srcRB, srcW, srcH, dst, dstRB);
            break;
    ...
    *dst = SkMask::PrepareDestination (radiusX, radiusY, src);
    ...
    switch (src.fFormat)
    {
        case SkMask::kBW_Format:
            direct_blur_y (bw_to_a8, 1, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH, dst->fImage + radiusX, dstRB);
            break;
        case SkMask::kA8_Format:
            direct_blur_y (nullptr, 8, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH, dst->fImage + radiusX, dstRB);
            break;
        case SkMask::kARGB32_Format:
            direct_blur_y (argb32_to_a8, 32, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH, dst->fImage + radiusX, dstRB);
            break;
        case SkMask::kLCD16_Format:
            direct_blur_y (lcd_to_a8, 16, radiusY, gaussFactorsY, src.fImage, srcRB, srcW, srcH, dst->fImage + radiusX, dstRB);
            break;
        default:
    ...
    direct_blur_x (radiusX, gaussFactorsX, dst->fImage + radiusX, dstRB, srcW, dst->fImage, dstRB, dstW, dstH);
...
SkIPoint SkMaskBlurFilter::blur (const SkMask& src, SkMask* dst) const
{
    ...
    *dst = SkMask::PrepareDestination (borderW, borderH, src);
    ...
    for (int y = 0; y < tmpH; y++)
    {
        ...
        scanH.blur (tmpStart, tmpStart + tmpW, dstStart, dst->fRowBytes, dstStart + dst->fRowBytes * dstH);
    }
