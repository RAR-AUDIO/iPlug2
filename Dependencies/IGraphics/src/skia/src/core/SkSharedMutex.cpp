...
SkSharedMutex::SkSharedMutex()
    : fCurrentShared (new ThreadIDSet), fWaitingExclusive (new ThreadIDSet), fWaitingShared (new ThreadIDSet)
{
...
void SkSharedMutex::release()
{
    ...
    } while (! fQueueCounts.compare_exchange_strong (oldQueueCounts, newQueueCounts, std::memory_order_release, std::memory_order_relaxed));
...
void SkSharedMutex::acquireShared()
{
    ...
    } while (! fQueueCounts.compare_exchange_strong (oldQueueCounts, newQueueCounts, std::memory_order_acquire, std::memory_order_relaxed));
