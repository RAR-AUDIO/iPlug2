                                                ...
                                                SkAutoDescriptor* ad)
{
    ...
    // Effects.
    {
        ...
        if (ptr)
        {
            desc->addEntry (kEffects_SkDescriptorTag, size, ptr);
        }
    }
...
public:
    ...
    bool readDescriptor (SkAutoDescriptor* ad)
    {
        ...
        if (! read<uint32_t> (&descLength))
            return false;
        if (descLength < sizeof (SkDescriptor))
            return false;
        if (descLength != SkAlign4 (descLength))
            return false;
        ...
        if (ad->getDesc()->getLength() > descLength)
            return false;
...
private:
    const volatile char* ensureAtLeast (size_t size, size_t alignment)
    {
        ...
        // Not enough data.
        if (padded > fMemorySize)
            return nullptr;
        if (size > fMemorySize - padded)
            return nullptr;
...
public:
    ...
    void prepareForMaskDrawing (
        ...
        SkSourceGlyphBuffer* rejects) override;
...
RemoteStrike::RemoteStrike (
    ...
    : fNumberOfGlyphs (context->getGlyphCount()), fDescriptor { descriptor }, fDiscardableHandleId (discardableHandleId), fRoundingSpec { context->isSubpixel(), context->computeAxisAlignmentForHText() } // N.B. context must come last because it is used above.
      ,
...
void RemoteStrike::prepareForSDFTDrawing (
    ...
    SkSourceGlyphBuffer* rejects)
{
    this->commonMaskLoop (drawables, rejects, [] (MaskSummary summary) { return ! summary.canDrawAsSDFT; });
}
...
sk_sp<SkData> SkStrikeServerImpl::serializeTypeface (SkTypeface* tf)
{
    ...
    WireTypeface wire (SkTypeface::UniqueID (tf), tf->countGlyphs(), tf->fontStyle(), tf->isFixedPitch());
...
void SkStrikeServerImpl::checkForDeletedEntries()
{
    ...
    while (fDescToRemoteStrike.size() > fMaxEntriesInDescriptorMap && it != fDescToRemoteStrike.end())
    {
    ...
    SkScalerContextEffects effects)
{
    ...
    TRACE_EVENT1 ("skia", "RecForDesc", "rec", TRACE_STR_COPY ([&desc]() {
    ...
    if (! fCachedTypefaces.contains (typefaceId))
    {
        ...
        fTypefacesToSend.emplace_back (typefaceId, typeface.countGlyphs(), typeface.fontStyle(), typeface.isFixedPitch());
    }
...
public:
    TrackLayerDevice (
        ...
        bool DFTSupport)
        : SkNoPixelsDevice (bounds, props, std::move (colorSpace)), fStrikeServerImpl (server), fDFTSupport (DFTSupport), fPainter { props, kUnknown_SkColorType, imageInfo().colorSpace(), fStrikeServerImpl }
    {
    ...
    SkBaseDevice* onCreateDevice (const CreateInfo& cinfo, const SkPaint*) override
    {
        ...
        return new TrackLayerDevice (this->getGlobalBounds(), surfaceProps, fStrikeServerImpl, cinfo.fInfo.refColorSpace(), fDFTSupport);
    }
...
protected:
    void drawGlyphRunList (const SkGlyphRunList& glyphRunList) override
    {
        ...
        GrSDFTOptions options = { ctxOptions.fMinDistanceFieldFontSize, ctxOptions.fGlyphsAsPathsFontSize };
...
SkTextBlobCacheDiffCanvas::SkTextBlobCacheDiffCanvas (int width, int height, const SkSurfaceProps& props, SkStrikeServer* strikeServer, bool DFTSupport)
    : SkTextBlobCacheDiffCanvas { width, height, props, strikeServer, nullptr, DFTSupport } {}

SkTextBlobCacheDiffCanvas::SkTextBlobCacheDiffCanvas (int width, int height, const SkSurfaceProps& props, SkStrikeServer* strikeServer, sk_sp<SkColorSpace> colorSpace, bool DFTSupport)
...
void SkTextBlobCacheDiffCanvas::onDrawTextBlob (const SkTextBlob* blob, SkScalar x, SkScalar y, const SkPaint& paint)
{
...
bool SkStrikeClientImpl::ReadGlyph (SkTLazy<SkGlyph>& glyph, Deserializer* deserializer)
{
    ...
    if (! deserializer->read<SkPackedGlyphID> (&glyphID))
        return false;
    glyph.init (glyphID);
    if (! deserializer->read<float> (&glyph->fAdvanceX))
        return false;
    if (! deserializer->read<float> (&glyph->fAdvanceY))
        return false;
    if (! deserializer->read<uint16_t> (&glyph->fWidth))
        return false;
    if (! deserializer->read<uint16_t> (&glyph->fHeight))
        return false;
    if (! deserializer->read<int16_t> (&glyph->fTop))
        return false;
    if (! deserializer->read<int16_t> (&glyph->fLeft))
        return false;
    if (! deserializer->read<uint8_t> (&glyph->fMaskFormat))
        return false;
    if (! SkMask::IsValidFormat (glyph->fMaskFormat))
        return false;
...
#define READ_FAILURE                                                                                           \
    {                                                                                                          \
        ...
        SkStrikeClient::DiscardableHandleManager::ReadFailureData data = {                                     \
            memorySize, deserializer.bytesRead(), typefaceSize, strikeCount, glyphImagesCount, glyphPathsCount \
        };                                                                                                     \
...
bool SkStrikeClientImpl::readStrikeData (const volatile void* memory, size_t memorySize)
{
    ...
    if (! deserializer.read<uint64_t> (&typefaceSize))
        READ_FAILURE
    for (size_t i = 0; i < typefaceSize; ++i)
    {
        WireTypeface wire;
        if (! deserializer.read<WireTypeface> (&wire))
            READ_FAILURE
    ...
    if (! deserializer.read<uint64_t> (&strikeCount))
        READ_FAILURE

    for (size_t i = 0; i < strikeCount; ++i)
    {
        StrikeSpec spec;
        if (! deserializer.read<StrikeSpec> (&spec))
            READ_FAILURE

        SkAutoDescriptor sourceAd;
        if (! deserializer.readDescriptor (&sourceAd))
            READ_FAILURE

        bool fontMetricsInitialized;
        if (! deserializer.read (&fontMetricsInitialized))
            READ_FAILURE

        SkFontMetrics fontMetrics {};
        if (! fontMetricsInitialized)
        {
            if (! deserializer.read<SkFontMetrics> (&fontMetrics))
                READ_FAILURE
        }
        ...
        if (fontMetricsInitialized && strike == nullptr)
            READ_FAILURE
        if (strike == nullptr)
        {
            ...
            strike = fStrikeCache->createStrike (
                *client_desc, std::move (scaler), &fontMetrics, std::make_unique<DiscardableStrikePinner> (spec.discardableHandleId, fDiscardableHandleManager));
        }

        if (! deserializer.read<uint64_t> (&glyphImagesCount))
            READ_FAILURE
        for (size_t j = 0; j < glyphImagesCount; j++)
        {
            SkTLazy<SkGlyph> glyph;
            if (! ReadGlyph (glyph, &deserializer))
                READ_FAILURE
        ...
        if (! deserializer.read<uint64_t> (&glyphPathsCount))
            READ_FAILURE
        for (size_t j = 0; j < glyphPathsCount; j++)
        {
            ...
            if (! deserializer.read<uint64_t> (&pathSize))
                READ_FAILURE
            ...
            if (pathSize > 0)
            {
                ...
                if (! pathData)
                    READ_FAILURE
                if (! path.readFromMemory (const_cast<const void*> (pathData), pathSize))
                    READ_FAILURE
                pathPtr = &path;
            }
...
sk_sp<SkTypeface> SkStrikeClientImpl::addTypeface (const WireTypeface& wire)
{
    ...
    auto newTypeface = sk_make_sp<SkTypefaceProxy> (
        wire.typefaceID, wire.glyphCount, wire.style, wire.isFixed, fDiscardableHandleManager, fIsLogging);
