...
SkSpecialImage::SkSpecialImage (const SkIRect& subset,
    ...
    : fProps (SkSurfacePropsCopyOrDefault (props)), fSubset (subset), fUniqueID (kNeedNewImageUniqueID_SpecialImage == uniqueID ? SkNextID::ImageID() : uniqueID)
{
}
                                                     ...
                                                     const SkSurfaceProps* props)
{
    ...
    if (rContext)
    {
        ...
        return MakeDeferredFromGpu (rContext, subset, image->uniqueID(), view, SkColorTypeToGrColorType (image->colorType()), image->refColorSpace(), props);
    }
#endif
...
public:
    ...
    void onDraw (SkCanvas* canvas, SkScalar x, SkScalar y, const SkPaint* paint) const override
    {
        ...
        canvas->drawBitmapRect (fBitmap, this->subset(), dst, paint, SkCanvas::kStrict_SrcRectConstraint);
    }
    ...
    sk_sp<SkSpecialSurface> onMakeSurface (SkColorType colorType, const SkColorSpace* colorSpace, const SkISize& size, SkAlphaType at, const SkSurfaceProps* props) const override
    {
    ...
    sk_sp<SkSurface> onMakeTightSurface (SkColorType colorType, const SkColorSpace* colorSpace, const SkISize& size, SkAlphaType at) const override
    {
        // Ignore the requested color type, the raster backend currently only supports N32
        colorType = kN32_SkColorType; // TODO: find ways to allow f16
        SkImageInfo info = SkImageInfo::Make (size, colorType, at, sk_ref_sp (colorSpace));
                                                      ...
                                                      const SkSurfaceProps* props)
{
    ...
    {
        if (! tmp.tryAllocPixels (bm.info().makeColorType (kN32_SkColorType)) || ! bm.readPixels (tmp.info(), tmp.getPixels(), tmp.rowBytes(), 0, 0))
        {
            return nullptr;
        }
        srcBM = &tmp;
...
#if SK_SUPPORT_GPU
///////////////////////////////////////////////////////////////////////////////
static sk_sp<SkImage> wrap_proxy_in_image (GrRecordingContext* context, GrSurfaceProxyView view, SkColorType colorType, SkAlphaType alphaType, sk_sp<SkColorSpace> colorSpace)
{
...
{
public:
    SkSpecialImage_Gpu (GrRecordingContext* context, const SkIRect& subset, uint32_t uniqueID, GrSurfaceProxyView view, GrColorType ct, SkAlphaType at, sk_sp<SkColorSpace> colorSpace, const SkSurfaceProps* props)
        : INHERITED (subset, uniqueID, props), fContext (context), fView (std::move (view)), fColorType (ct), fAlphaType (at), fColorSpace (std::move (colorSpace))
    {
    }
    ...
    sk_sp<SkSpecialSurface> onMakeSurface (SkColorType colorType, const SkColorSpace* colorSpace, const SkISize& size, SkAlphaType at, const SkSurfaceProps* props) const override
    {
        ...
        return SkSpecialSurface::MakeRenderTarget (fContext, size.width(), size.height(), SkColorTypeToGrColorType (colorType), sk_ref_sp (colorSpace), props);
    }
    ...
    sk_sp<SkImage> onAsImage (const SkIRect* subset) const override
    {
        ...
        if (subset)
        {
            ...
            auto subsetView = GrSurfaceProxyView::Copy (fContext, fView, GrMipmapped::kNo, *subset, SkBackingFit::kExact, SkBudgeted::kYes);
            if (! subsetView)
            {
                return nullptr;
            }
            ...
            return wrap_proxy_in_image (fContext, std::move (subsetView), this->colorType(), fAlphaType, fColorSpace);
        }
                                                           ...
                                                           SkAlphaType at)
{
    ...
    return sk_make_sp<SkSpecialImage_Gpu> (context, subset, uniqueID, std::move (view), colorType, at, std::move (colorSpace), props);
}
#endif
