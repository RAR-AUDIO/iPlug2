...
void SkPictureRecord::onDrawPoints (PointMode mode, size_t count, const SkPoint pts[], const SkPaint& paint)
{
...
void SkPictureRecord::onDrawArc (const SkRect& oval, SkScalar startAngle, SkScalar sweepAngle, bool useCenter, const SkPaint& paint)
{
    // op + paint index + rect + start + sweep + bool (as int)
    size_t size = 2 * kUInt32Size + sizeof (oval) + sizeof (startAngle) + sizeof (sweepAngle) + sizeof (int);
...
void SkPictureRecord::onDrawDRRect (const SkRRect& outer, const SkRRect& inner, const SkPaint& paint)
{
...
void SkPictureRecord::onDrawImage (const SkImage* image, SkScalar x, SkScalar y, const SkPaint* paint)
{
...
void SkPictureRecord::onDrawImageRect (const SkImage* image, const SkRect* src, const SkRect& dst, const SkPaint* paint, SrcRectConstraint constraint)
{
...
void SkPictureRecord::onDrawImageNine (const SkImage* img, const SkIRect& center, const SkRect& dst, const SkPaint* paint)
{
...
void SkPictureRecord::onDrawImageLattice (const SkImage* image, const Lattice& lattice, const SkRect& dst, const SkPaint* paint)
{
...
void SkPictureRecord::onDrawTextBlob (const SkTextBlob* blob, SkScalar x, SkScalar y, const SkPaint& paint)
{
...
void SkPictureRecord::onDrawPicture (const SkPicture* picture, const SkMatrix* matrix, const SkPaint* paint)
{
    // op + picture index
    size_t size = 2 * kUInt32Size;
...
void SkPictureRecord::onDrawPatch (const SkPoint cubics[12], const SkColor colors[4], const SkPoint texCoords[4], SkBlendMode bmode, const SkPaint& paint)
{
...
void SkPictureRecord::onDrawAtlas (const SkImage* atlas, const SkRSXform xform[], const SkRect tex[], const SkColor colors[], int count, SkBlendMode mode, const SkRect* cull, const SkPaint* paint)
{
...
void SkPictureRecord::onDrawEdgeAAQuad (const SkRect& rect, const SkPoint clip[4], SkCanvas::QuadAAFlags aa, const SkColor4f& color, SkBlendMode mode)
{
    // op + rect + aa flags + color + mode + hasClip(as int) + clipCount*points
    size_t size = 4 * kUInt32Size + sizeof (SkColor4f) + sizeof (rect) + (clip ? 4 : 0) * sizeof (SkPoint);
...
void SkPictureRecord::onDrawEdgeAAImageSet (const SkCanvas::ImageSetEntry set[], int count, const SkPoint dstClips[], const SkMatrix preViewMatrices[], const SkPaint* paint, SkCanvas::SrcRectConstraint constraint)
{
    ...
    size_t size = 6 * kUInt32Size + sizeof (SkPoint) * totalDstClipCount + kMatrixSize * totalMatrixCount + (4 * kUInt32Size + 2 * sizeof (SkRect) + sizeof (SkScalar)) * count;
