...
SkParticleEffectParams::SkParticleEffectParams()
    : fMaxCount (128), fDrawable (nullptr), fEffectCode (kDefaultEffectCode), fParticleCode (kDefaultParticleCode) {}
...
void SkParticleEffectParams::prepare (const skresources::ResourceProvider* resourceProvider)
{
    ...
    auto buildProgram = [this] (const SkSL::String& code, Program* p) {
        ...
        auto program = compiler.convertProgram (SkSL::Program::kGeneric_Kind, code, settings, &externalValues);
        if (! program)
        {
...
SkParticleEffect::SkParticleEffect (sk_sp<SkParticleEffectParams> params)
    : fParams (std::move (params)), fLooping (false), fCount (0), fLastTime (-1.0), fSpawnRemainder (0.0f)
{
    fState.fAge = -1.0f;
    this->setCapacity (fParams->fMaxCount);
}

void SkParticleEffect::start (double now, bool looping, SkPoint position, SkVector heading, float scale, SkVector velocity, float spin, SkColor4f color, float frame, uint32_t flags, uint32_t seed)
{
    fCount = 0;
    fLastTime = now;
...
void SkParticleEffect::processEffectSpawnRequests (double now)
{
    for (const auto& spawnReq : fSpawnRequests)
    {
        ...
        newEffect->start (now, spawnReq.fLoop, fState.fPosition, fState.fHeading, fState.fScale, fState.fVelocity, fState.fSpin, fState.fColor, fState.fFrame, fState.fFlags, fState.fRandom);
...
void SkParticleEffect::runEffectScript (double now, const char* entry)
{
    if (const auto& byteCode = fParams->fEffectProgram.fByteCode)
    {
        if (auto fun = byteCode->getFunction (entry))
        {
            ...
            SkAssertResult (byteCode->run (fun, &fState.fAge, sizeof (EffectState) / sizeof (float), nullptr, 0, fEffectUniforms.data(), fEffectUniforms.count()));
...
void SkParticleEffect::processParticleSpawnRequests (double now, int start)
{
    ...
    for (const auto& spawnReq : fSpawnRequests)
    {
        ...
        sk_sp<SkParticleEffect> newEffect (new SkParticleEffect (std::move (spawnReq.fParams)));
        newEffect->start (now, spawnReq.fLoop, { data[SkParticles::kPositionX][idx], data[SkParticles::kPositionY][idx] }, { data[SkParticles::kHeadingX][idx], data[SkParticles::kHeadingY][idx] }, data[SkParticles::kScale][idx], { data[SkParticles::kVelocityX][idx], data[SkParticles::kVelocityY][idx] }, data[SkParticles::kVelocityAngular][idx], { data[SkParticles::kColorR][idx], data[SkParticles::kColorG][idx], data[SkParticles::kColorB][idx], data[SkParticles::kColorA][idx] }, data[SkParticles::kSpriteFrame][idx], float_to_bits (data[SkParticles::kFlags][idx]), float_to_bits (data[SkParticles::kRandom][idx]));
        fSubEffects.push_back (std::move (newEffect));
    }
    fSpawnRequests.reset();
}
...
void SkParticleEffect::runParticleScript (double now, const char* entry, int start, int count)
{
    if (const auto& byteCode = fParams->fParticleProgram.fByteCode)
    {
        if (auto fun = byteCode->getFunction (entry))
        {
            ...
            SkAssertResult (byteCode->runStriped (fun, count, args, SkParticles::kNumChannels, nullptr, 0, fParticleUniforms.data(), fParticleUniforms.count()));
