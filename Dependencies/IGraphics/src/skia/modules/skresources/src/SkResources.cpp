    ...
    auto decode = [] (sk_sp<SkImage> image) {
        ...
        if (image_area > kMaxArea)
        {
            ...
            const auto info = SkImageInfo::MakeN32Premul (scale * image->width(),
                                                          scale * image->height());
            SkBitmap bm;
            if (bm.tryAllocPixels (info, info.minRowBytes()) && image->scalePixels (bm.pixmap(), SkFilterQuality::kMedium_SkFilterQuality, SkImage::kDisallow_CachingHint))
            {
        ...
        if (const auto* encoding_start = strstr (data + prefix_len, kDataURIEncodingStr))
        {
            ...
            if (SkBase64::kNoError == b64.decode (data_start, strlen (data_start)))
            {
                return SkData::MakeWithProc (
                    b64.getData(), b64.getDataSize(), [] (const void* ptr, void*) {
