...
namespace textlayout
{
    ...
    TextLine::TextLine (ParagraphImpl* owner,
        ...
        : fOwner (owner), fBlockRange (blocks), fTextRange (text), fTextWithWhitespacesRange (textWithSpaces), fClusterRange (clusters), fGhostClusterRange (clustersWithGhosts), fRunsInVisualOrder(), fAdvance (advance), fOffset (offset), fShift (0.0), fWidthWithSpaces (widthWithSpaces), fEllipsis (nullptr), fSizes (sizes), fHasBackground (false), fHasShadows (false), fHasDecorations (false), fAscentStyle (LineMetricStyle::CSS), fDescentStyle (LineMetricStyle::CSS)
    {
        // Reorder visual runs
    ...
    SkRect TextLine::paint (SkCanvas* textCanvas, SkScalar x, SkScalar y)
    {
        ...
        if (fHasBackground)
        {
            this->iterateThroughVisualRuns (false,
                                            [textCanvas, x, y, this] (const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
                                                *runWidthInLine = this->iterateThroughSingleRunByStyles (
                                                    run, runOffsetInLine, textRange, StyleType::kBackground, [textCanvas, x, y, this] (TextRange textRange, const TextStyle& style, const ClipContext& context) {
        ...
        {
            this->iterateThroughVisualRuns (false,
                                            [textCanvas, x, y, &bounds, this] (const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
                                                *runWidthInLine = this->iterateThroughSingleRunByStyles (
                                                    run, runOffsetInLine, textRange, StyleType::kShadow, [textCanvas, x, y, &bounds, this] (TextRange textRange, const TextStyle& style, const ClipContext& context) {
        ...
        this->iterateThroughVisualRuns (false,
                                        [textCanvas, x, y, &bounds, this] (const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
                                            ...
                                            *runWidthInLine = this->iterateThroughSingleRunByStyles (
                                                run, runOffsetInLine, textRange, StyleType::kForeground, [textCanvas, x, y, &bounds, this] (TextRange textRange, const TextStyle& style, const ClipContext& context) {
        ...
        if (fHasDecorations)
        {
            this->iterateThroughVisualRuns (false,
                                            [textCanvas, x, y, this] (const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
                                                *runWidthInLine = this->iterateThroughSingleRunByStyles (
                                                    run, runOffsetInLine, textRange, StyleType::kDecorations, [textCanvas, x, y, this] (TextRange textRange, const TextStyle& style, const ClipContext& context) {
        ...
        this->iterateThroughVisualRuns (false,
                                        [this, visitor, styleType] (const Run* run, SkScalar runOffset, TextRange textRange, SkScalar* width) {
                                            *width = this->iterateThroughSingleRunByStyles (
                                                run, runOffset, textRange, styleType, [visitor] (TextRange textRange, const TextStyle& style, const ClipContext& context) {
    ...
    SkRect TextLine::paintText (SkCanvas* canvas, SkScalar x, SkScalar y, TextRange textRange, const TextStyle& style, const ClipContext& context) const
    {
        ...
        canvas->drawTextBlob (blob,
                              x + this->offset().fX + context.fTextShift,
                              y + this->offset().fY + correctedBaseline,
                              paint);
    ...
    void TextLine::justify (SkScalar maxWidth)
    {
        ...
        this->iterateThroughClustersInGlyphsOrder (false, false, [&whitespacePatches, &textLen, &whitespacePatch] (const Cluster* cluster, bool ghost) {
    ...
    std::unique_ptr<Run> TextLine::shapeEllipsis (const SkString& ellipsis, Run* run)
    {
        ...
        shaper->shape (ellipsis.c_str(), ellipsis.size(), run->font(), true, std::numeric_limits<SkScalar>::max(), &handler);
...
*/
                                                        ...
                                                        const RunStyleVisitor& visitor) const
    {
        ...
        for (BlockIndex index = fBlockRange.start; index <= fBlockRange.end; ++index)
        {
            ...
            // Measure the text
            ClipContext clipContext = this->measureTextInsideOneRun (runStyleTextRange, run, runOffset, textOffsetInRun, false, false);
            if (clipContext.clip.height() == 0)
            {
                continue;
            }
    ...
    LineMetrics TextLine::getMetrics() const
    {
        ...
        this->iterateThroughVisualRuns (false,
                                        [this, &result] (const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
                                            if (run->placeholderStyle() != nullptr)
                                            {
                                                *runWidthInLine = run->advance().fX;
                                                return true;
                                            ...
                                            *runWidthInLine = this->iterateThroughSingleRunByStyles (
                                                run, runOffsetInLine, textRange, StyleType::kForeground, [&result, &run] (TextRange textRange, const TextStyle& style, const ClipContext& context) {
    ...
    bool TextLine::endsWithHardLineBreak() const
    {
        ...
        //  To be removed...
        return fOwner->cluster (fGhostClusterRange.end - 1).isHardBreak() || fEllipsis != nullptr || fGhostClusterRange.end == fOwner->clusters().size() - 1;
    }
                                     ...
                                     std::vector<TextBox>& boxes)
    {
        ...
        this->iterateThroughVisualRuns (true,
                                        [textRange0, rectHeightStyle, rectWidthStyle, &boxes, &lastRun, startBox, this] (const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
                                            *runWidthInLine = this->iterateThroughSingleRunByStyles (
                                                run, runOffsetInLine, textRange, StyleType::kNone, [run, runOffsetInLine, textRange0, rectHeightStyle, rectWidthStyle, &boxes, &lastRun, startBox, this] (TextRange textRange, const TextStyle& style, const TextLine::ClipContext& lineContext) {
                                                    ...
                                                    if (this->trimmedText().end<this->textWithSpaces().end&& // Line has trailing spaces
                                                                                this->textWithSpaces()
                                                                                    .end
                                                                                == intersect.end&& // Range is at the end of the line
                                                                                   this->trimmedText()
                                                                                       .end> intersect.start) // Range has more than just spaces
                                                    {
                                                        ...
                                                        else if (paragraphStyle.getTextDirection() == TextDirection::kRtl && ! run->leftToRight())
                                                        {
                                                            // Split
                                                            trailingSpaces = clip;
                                                        ...
                                                        else if (paragraphStyle.getTextDirection() == TextDirection::kLtr && run->leftToRight())
                                                        {
                                                            // Split
                                                            trailingSpaces = clip;
                                                            trailingSpaces.fLeft = this->width();
                                                    ...
                                                    auto merge = [&lastRun, &context, &boxes] (SkRect clip) {
                                                        ...
                                                        if (! boxes.empty() && lastRun != nullptr && lastRun->placeholderStyle() == nullptr && context.run->placeholderStyle() == nullptr && nearlyEqual (lastRun->heightMultiplier(), context.run->heightMultiplier()) && lastRun->font() == context.run->font())
                                                        {
                                                            auto& lastBox = boxes.back();
                                                            if (nearlyEqual (lastBox.rect.fTop, clip.fTop) && nearlyEqual (lastBox.rect.fBottom, clip.fBottom) && (nearlyEqual (lastBox.rect.fLeft, clip.fRight) || nearlyEqual (lastBox.rect.fRight, clip.fLeft)))
                                                            {
                                                    ...
                                                    if (rectWidthStyle == RectWidthStyle::kMax && ! isLastLine())
                                                    {
                                                        ...
                                                        if (right.direction == TextDirection::kLtr && right.rect.fRight >= lineEnd && right.rect.fRight < fOwner->widthWithTrailingSpaces())
                                                        {
    ...
    PositionWithAffinity TextLine::getGlyphPositionAtCoordinate (SkScalar dx)
    {
        ...
        this->iterateThroughVisualRuns (true,
                                        [this, dx, &result] (const Run* run, SkScalar runOffsetInLine, TextRange textRange, SkScalar* runWidthInLine) {
                                            bool keepLooking = true;
                                            *runWidthInLine = this->iterateThroughSingleRunByStyles (
                                                run, runOffsetInLine, textRange, StyleType::kNone, [this, dx, &result, &keepLooking] (TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
    ...
    void TextLine::getRectsForPlaceholders (std::vector<TextBox>& boxes)
    {
        this->iterateThroughVisualRuns (
            true,
            [&boxes, this] (const Run* run, SkScalar runOffset, TextRange textRange, SkScalar* width) {
