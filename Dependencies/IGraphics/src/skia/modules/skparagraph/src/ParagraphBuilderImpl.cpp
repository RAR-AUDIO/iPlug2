...
namespace textlayout
{
    ...
    ParagraphBuilderImpl::ParagraphBuilderImpl (
        ...
        sk_sp<FontCollection> fontCollection,
        std::unique_ptr<SkUnicode> unicode)
        : ParagraphBuilder (style, fontCollection), fUtf8(), fFontCollection (std::move (fontCollection)), fUnicode (std::move (unicode))
    {
        SkASSERT (fUnicode);
    ...
    void ParagraphBuilderImpl::pushStyle (const TextStyle& style)
    {
        ...
        if (! fStyledBlocks.empty() && fStyledBlocks.back().fRange.end == fUtf8.size() && fStyledBlocks.back().fStyle == style)
        {
