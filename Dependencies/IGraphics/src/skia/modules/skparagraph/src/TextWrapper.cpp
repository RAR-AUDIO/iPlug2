...
namespace textlayout
{
    ...
    std::tuple<Cluster*, size_t, SkScalar> TextWrapper::trimStartSpaces (Cluster* endOfClusters)
    {
        ...
        if (fEndLine.endCluster() != fEndLine.startCluster() || fEndLine.endPos() != fEndLine.startPos())
        {
            ++cluster;
                                          ...
                                          const AddLineToParagraph& addLine)
    {
        ...
        while (fEndLine.endCluster() != end)
        {
            ...
            addLine (text, textWithSpaces, clusters, clustersWithGhosts, widthWithSpaces, fEndLine.startPos(), fEndLine.endPos(), SkVector::Make (0, fHeight), SkVector::Make (fEndLine.width(), fEndLine.metrics().height()), fEndLine.metrics(), needEllipsis && ! fHardLineBreak);
        ...
        if (fHardLineBreak)
        {
            ...
            addLine (empty, hardBreak, clusters, clusters, 0, 0, 0, SkVector::Make (0, fHeight), SkVector::Make (0, fEndLine.metrics().height()), fEndLine.metrics(), needEllipsis);
            fHeight += fEndLine.metrics().height();
