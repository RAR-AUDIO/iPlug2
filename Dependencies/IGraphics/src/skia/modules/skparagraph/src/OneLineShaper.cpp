...
namespace textlayout
{
    ...
    void OneLineShaper::finish (TextRange blockText, SkScalar height, SkScalar& advanceX)
    {
        ...
        std::sort (fResolvedBlocks.begin(), fResolvedBlocks.end(), [] (const RunBlock& a, const RunBlock& b) {
    ...
    void OneLineShaper::sortOutGlyphs (std::function<void (GlyphRange)>&& sortOutUnresolvedBLock)
    {
        ...
        for (size_t i = 0; i < fCurrentRun->size(); ++i)
        {
            ...
            if (glyph == 0 && ! isControl8)
            { // Unresolved glyph and not control codepoint
                ++unresolvedGlyphs;
            ...
            { // Resolved glyph or control codepoint
                if (block.start == EMPTY_INDEX)
                {
    ...
    bool OneLineShaper::shape()
    {
        ...
        auto result = iterateThroughShapingRegions (
            [this, limitlessWidth] (TextRange textRange, SkSpan<Block> styleSpan, SkScalar& advanceX, TextIndex textStart, uint8_t defaultBidiLevel) {
                ...
                iterateThroughFontStyles (textRange, styleSpan, [this, &shaper, defaultBidiLevel, limitlessWidth, &advanceX] (Block block, SkTArray<SkShaper::Feature> features) {
                    ...
                    matchResolvedFonts (block.fStyle, [&] (sk_sp<SkTypeface> typeface) {
                        ...
                        bool fakeBold =
                            wantedWeight >= SkFontStyle::kSemiBold_Weight && wantedWeight - font.getTypeface()->fontStyle().weight() >= 200;
                        bool fakeItalic =
                            block.fStyle.getFontStyle().slant() == SkFontStyle::kItalic_Slant && font.getTypeface()->fontStyle().slant() != SkFontStyle::kItalic_Slant;
                        ...
                        while (unresolvedCount-- > 0)
                        {
                            ...
                            LangIterator langIter (unresolvedText, blockSpan, fParagraph->paragraphStyle().getTextStyle());
                            SkShaper::TrivialBiDiRunIterator bidiIter (defaultBidiLevel, unresolvedText.size());
                            auto scriptIter = SkShaper::MakeSkUnicodeHbScriptRunIterator (fParagraph->getUnicode(), unresolvedText.begin(), unresolvedText.size());
                            fCurrentText = unresolvedRange;
                            shaper->shape (unresolvedText.begin(), unresolvedText.size(), fontIter, bidiIter, *scriptIter, langIter, features.data(), features.size(), limitlessWidth, this);
    ...
    {
        return SkGoodHash() (key.fUnicode) ^ SkGoodHash() (key.fFontStyle) ^ SkGoodHash() (key.fLocale.c_str());
    }
