...
namespace textlayout
{
    ...
    Paragraph::Paragraph (ParagraphStyle style, sk_sp<FontCollection> fonts)
        : fFontCollection (std::move (fonts)), fParagraphStyle (std::move (style)), fAlphabeticBaseline (0), fIdeographicBaseline (0), fHeight (0), fWidth (0), fMaxIntrinsicWidth (0), fMinIntrinsicWidth (0), fLongestLine (0), fExceededMaxLines (0)
    {
    }
    ...
    ParagraphImpl::ParagraphImpl (const SkString& text,
        ...
        : Paragraph (std::move (style), std::move (fonts)), fTextStyles (std::move (blocks)), fPlaceholders (std::move (placeholders)), fText (text), fState (kUnknown), fUnresolvedGlyphs (0), fPicture (nullptr), fStrutMetrics (false), fOldWidth (0), fOldHeight (0), fUnicode (std::move (unicode))
    {
        SkASSERT (fUnicode);
    }
    ...
    void ParagraphImpl::layout (SkScalar rawWidth)
    {
        ...
        if ((! SkScalarIsFinite (rawWidth) || fLongestLine <= floorWidth) && fState >= kLineBroken && fLines.size() == 1 && fLines.front().ellipsis() == nullptr)
        {
        ...
        if (fState < kShaped)
        {
            ...
            if (! this->shapeTextIntoEndlessLine())
            {
                ...
                if (fParagraphStyle.getStrutStyle().getStrutEnabled() && fParagraphStyle.getStrutStyle().getForceStrutHeight())
                {
...
*/
  ...
  */
    ...
    void ParagraphImpl::computeEmptyMetrics()
    {
        ...
        if (! paragraphStyle().getStrutStyle().getForceStrutHeight() && defaultTextStyle.getHeightOverride())
        {
            auto multiplier =
    ...
    {
        while (utf8 > 0 && (fCodeUnitProperties[utf8] & CodeUnitFlags::kGraphemeStart) == 0)
        {
            --utf8;
        }
        return utf8;
    ...
    {
        while (utf8 < fText.size() && (fCodeUnitProperties[utf8] & CodeUnitFlags::kGraphemeStart) == 0)
        {
            ++utf8;
        }
        return utf8;
