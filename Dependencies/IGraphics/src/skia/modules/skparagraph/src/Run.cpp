...
namespace textlayout
{
    Run::Run (ParagraphImpl* owner,
              ...
              size_t index,
              SkScalar offsetX)
        : fOwner (owner), fTextRange (firstChar + info.utf8Range.begin(), firstChar + info.utf8Range.end()), fClusterRange (EMPTY_CLUSTERS), fFont (info.fFont), fClusterStart (firstChar), fHeightMultiplier (heightMultiplier)
    {
    ...
    void Run::calculateMetrics()
    {
        ...
        {
            auto multiplier = fHeightMultiplier * fFont.getSize() / (fFontMetrics.fDescent - fFontMetrics.fAscent + fFontMetrics.fLeading);
    ...
    {
        return posX (pos) + fShifts[pos] + (fJustificationShifts.empty() ? 0 : fJustificationShifts[pos].fY);
    }
    ...
    Cluster::Cluster (ParagraphImpl* owner,
                      ...
                      SkScalar width,
                      SkScalar height)
        : fOwner (owner), fRunIndex (runIndex), fTextRange (text.begin() - fOwner->text().begin(), text.end() - fOwner->text().begin()), fGraphemeRange (EMPTY_RANGE), fStart (start), fEnd (end), fWidth (width), fSpacing (0), fHeight (height), fHalfLetterSpacing (0.0)
    {
