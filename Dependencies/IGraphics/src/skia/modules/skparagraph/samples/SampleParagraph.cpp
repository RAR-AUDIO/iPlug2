    ...
    void drawTest (SkCanvas* canvas, SkScalar w, SkScalar h, SkColor fg, SkColor bg)
    {
        ...
        for (auto i = 1; i < 5; ++i)
        {
            ...
            for (auto para : gParagraph)
            {
                ...
                std::string name = " " + std::get<0> (para) + " " + (std::get<1> (para) ? ", bold" : "") + (std::get<2> (para) ? ", italic" : "") + " " + std::to_string (std::get<3> (para) * i) + (std::get<4> (para) != bg ? ", background" : "") + (std::get<5> (para) != fg ? ", foreground" : "") + (std::get<6> (para) ? ", shadow" : "") + (test ? ", decorations " + deco : "") + ";";
                builder.addText (name.c_str(), name.length());
                builder.pop();
            }
...
protected:
    ...
    void drawText (SkCanvas* canvas, SkScalar w, SkScalar h, std::vector<const char*>& text, SkColor fg = SK_ColorDKGRAY, SkColor bg = SK_ColorWHITE, const char* ff = "sans-serif", SkScalar fs = 24, size_t lineLimit = 30, const std::u16string& ellipsis = u"\u2026")
    {
        SkAutoCanvasRestore acr (canvas, true);
    ...
    void drawLine (SkCanvas* canvas, SkScalar w, SkScalar h, const std::string& text, TextAlign align)
    {
        SkAutoCanvasRestore acr (canvas, true);
...
protected:
    ...
    void drawLine (SkCanvas* canvas, SkScalar w, SkScalar h, const std::string& text, TextAlign align, size_t lineLimit = std::numeric_limits<size_t>::max(), bool RTL = false, SkColor background = SK_ColorGRAY, const std::u16string& ellipsis = u"\u2026")
    {
        SkAutoCanvasRestore acr (canvas, true);
...
protected:
    ...
    void drawFlutter (SkCanvas* canvas, SkScalar w, SkScalar h, const char* ff = "Google Sans", SkScalar fs = 30, size_t lineLimit = std::numeric_limits<size_t>::max(), const std::u16string& ellipsis = u"\u2026")
    {
        SkAutoCanvasRestore acr (canvas, true);
...
protected:
    ...
    void bidi (SkCanvas* canvas, SkScalar w, SkScalar h, const std::u16string& text, const std::u16string& expected, size_t lineLimit = std::numeric_limits<size_t>::max(), const char* ff = "Roboto", SkScalar fs = 30, const std::u16string& ellipsis = u"\u2026")
    {
        SkAutoCanvasRestore acr (canvas, true);
...
protected:
    ...
    void drawText (SkCanvas* canvas, SkColor background, SkScalar w, SkScalar h)
    {
        ...
        std::vector<size_t> colors = { SK_ColorBLUE, SK_ColorCYAN, SK_ColorLTGRAY, SK_ColorGREEN, SK_ColorRED, SK_ColorWHITE, SK_ColorYELLOW, SK_ColorMAGENTA };
...
protected:
    ...
    void onDrawContent (SkCanvas* canvas) override
    {
        ...
        auto draw = [&] (SkScalar width, SkScalar height, TextDirection td, TextAlign ta, const char* t) {
            if (this->isVerbose())
            {
                SkDebugf ("draw '%s' dir:%s align:%s\n", t, td == TextDirection::kLtr ? "left" : "right", ta == TextAlign::kLeft ? "left" : "right");
            }
                ...
                line.iterateThroughVisualRuns (true,
                                               [&] (const Run* run, SkScalar runOffset, TextRange textRange, SkScalar* width) {
                                                   *width = line.measureTextInsideOneRun (textRange, run, runOffset, 0, true, false).clip.width();
                                                   if (this->isVerbose())
                                                   {
                                                       SkDebugf ("%d[%d: %d) @%f + %f %s\n", run->index(), textRange.start, textRange.end, runOffset, *width, run->leftToRight() ? "left" : "right");
                                                   }
                                                   return true;
                                               });
            }
    ...
    void onDrawContent (SkCanvas* canvas) override
    {
        ...
        if (this->isVerbose())
        {
            SkDebugf ("%d(%s) %d(%s) %d(%s)\n",
                      f1.position,
                      f1.affinity == Affinity::kUpstream ? "up" : "down",
                      f2.position,
                      f2.affinity == Affinity::kUpstream ? "up" : "down",
                      i.position,
                      i.affinity == Affinity::kUpstream ? "up" : "down");
...
protected:
    ...
    void onDrawContent (SkCanvas* canvas) override
    {
        ...
        SkColor colors[] = { SK_ColorBLUE, SK_ColorCYAN, SK_ColorLTGRAY, SK_ColorGREEN, SK_ColorRED, SK_ColorWHITE, SK_ColorYELLOW, SK_ColorMAGENTA };
        SkPaint paint;
        size_t wordPos = 0;
        size_t index = 0;
        while (wordPos < 72)
        {
            ...
            auto res3 = paragraph->getRectsForRange (
                res2.start, res2.end, RectHeightStyle::kTight, RectWidthStyle::kTight);
...
protected:
    ...
    void onDrawContent (SkCanvas* canvas) override
    {
        ...
        if (this->isVerbose())
        {
            ...
            {
                SkDebugf ("%d: [%d:%d) %s\n", c++, cluster.textRange().start, cluster.textRange().end, cluster.isSoftBreak() ? "soft" : cluster.isHardBreak() ? "hard"
