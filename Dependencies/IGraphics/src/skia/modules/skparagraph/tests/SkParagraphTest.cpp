...
public:
    ResourceFontCollection (bool testOnly = false)
        : fFontsFound (false), fResolvedFonts (0), fResourceDir (GetResourcePath ("fonts").c_str()), fFontProvider (sk_make_sp<TypefaceFontProvider>())
    {
...
DEF_TEST (SkParagraph_SimpleParagraph, reporter)
{
    ...
    for (auto& line : impl->lines())
    {
        line.scanStyles (StyleType::kDecorations,
                         [&index, reporter] (TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
...
DEF_TEST (SkParagraph_RainbowParagraph, reporter)
{
    ...
    text_style2.setFontStyle (SkFontStyle (SkFontStyle::kMedium_Weight, SkFontStyle::kNormal_Width, SkFontStyle::kUpright_Slant));
    text_style2.setLetterSpacing (10);
    text_style2.setDecorationColor (SK_ColorBLACK);
    text_style2.setDecoration ((TextDecoration) (TextDecoration::kUnderline | TextDecoration::kOverline | TextDecoration::kLineThrough));
    ...
    text_style4.setDecoration ((TextDecoration) (TextDecoration::kUnderline | TextDecoration::kOverline | TextDecoration::kLineThrough));
...
DEF_TEST (SkParagraph_DecorationsParagraph, reporter)
{
    ...
    for (auto& line : impl->lines())
    {
        line.scanStyles (
            ...
            [&] (TextRange textRange, const TextStyle& style, const TextLine::ClipContext& context) {
                auto decoration = (TextDecoration) (TextDecoration::kUnderline | TextDecoration::kOverline | TextDecoration::kLineThrough);
                REPORTER_ASSERT (reporter, style.getDecorationType() == decoration);
                switch (index)
                {
                    case 0:
                        REPORTER_ASSERT (reporter, style.getDecorationStyle() == TextDecorationStyle::kSolid);
                    ...
                    case 1: // The style appears on 2 lines so it has 2 pieces
                        REPORTER_ASSERT (reporter, style.getDecorationStyle() == TextDecorationStyle::kDouble);
                        REPORTER_ASSERT (reporter, style.getDecorationColor() == SK_ColorBLUE);
                        REPORTER_ASSERT (reporter,
                                         style.getDecorationThicknessMultiplier() == 1.0);
                        break;
                    case 2:
                        REPORTER_ASSERT (reporter, style.getDecorationStyle() == TextDecorationStyle::kDotted);
                        REPORTER_ASSERT (reporter, style.getDecorationColor() == SK_ColorBLACK);
                        REPORTER_ASSERT (reporter,
                                         style.getDecorationThicknessMultiplier() == 1.0);
                        break;
                    case 3:
                    case 4:
                        REPORTER_ASSERT (reporter, style.getDecorationStyle() == TextDecorationStyle::kDashed);
                        REPORTER_ASSERT (reporter, style.getDecorationColor() == SK_ColorBLACK);
                        REPORTER_ASSERT (reporter,
                                         style.getDecorationThicknessMultiplier() == 3.0);
                        break;
                    case 5:
                        REPORTER_ASSERT (reporter, style.getDecorationStyle() == TextDecorationStyle::kWavy);
...
DEF_TEST (SkParagraph_ItalicsParagraph, reporter)
{
    ...
    text_style.setFontStyle (SkFontStyle (SkFontStyle::kNormal_Weight, SkFontStyle::kNormal_Width, SkFontStyle::kItalic_Slant));
    builder.pushStyle (text_style);
    builder.addText (text2, strlen (text2));
    builder.pop();
    builder.addText (text3, strlen (text3));
...
DEF_TEST (SkParagraph_BaselineParagraph, reporter)
{
    ...
    SkRect rect1 = SkRect::MakeXYWH (0, paragraph->getIdeographicBaseline(), paragraph->getMaxWidth(), paragraph->getIdeographicBaseline());
    SkRect rect2 = SkRect::MakeXYWH (0, paragraph->getAlphabeticBaseline(), paragraph->getMaxWidth(), paragraph->getAlphabeticBaseline());
...
DEF_TEST (SkParagraph_FontFallbackParagraph, reporter)
{
    ...
    // [Apple + Noto] 2, 3
    // [Apple + Han] 4, 5
    auto robotoAdvance = impl->runs()[0].advance().fX + impl->runs()[1].advance().fX;
...
DEF_TEST_DISABLED (SkParagraph_JSON1, reporter)
{
    ...
    SkShaperJSONWriter::VisualizeClusters (
        text, 0, std::strlen (text), run.glyphs(), run.clusterIndexes(), [&] (int codePointCount, SkSpan<const char> utf1to1, SkSpan<const SkGlyphID> glyph1to1) {
            if (cluster == 0)
            {
...
DEF_TEST_DISABLED (SkParagraph_JSON2, reporter)
{
    ...
    {
        SkShaperJSONWriter::VisualizeClusters (
            impl->text().begin() + run.textRange().start, 0, run.textRange().width(), run.glyphs(), run.clusterIndexes(), [&] (int codePointCount, SkSpan<const char> utf1to1, SkSpan<const SkGlyphID> glyph1to1) {
                if (cluster == 0)
                {
                    std::string toCheckUtf8 { utf1to1.data(), utf1to1.size() };
                    SkASSERT (std::strcmp (text, utf1to1.data()) == 0);
...
DEF_TEST (SkParagraph_Ellipsis, reporter)
{
    ...
    auto relayout = [&] (size_t lines, bool ellipsis, SkScalar width, SkScalar height, SkScalar minWidth, SkScalar maxWidth, SkColor bg) {
        ParagraphStyle paragraph_style;
        SkPaint paint;
        paint.setColor (bg);
