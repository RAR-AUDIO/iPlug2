...
namespace internal
{
    namespace
    {
        ...
        public:
            FootageAnimator (sk_sp<ImageAsset> asset,
                             ...
                             float time_bias,
                             float time_scale)
                : fAsset (std::move (asset)), fImageNode (std::move (image_node)), fImageTransformNode (std::move (image_transform_node)), fAssetSize (asset_size), fTimeBias (time_bias), fTimeScale (time_scale), fIsMultiframe (fAsset->isMultiFrame()) {}
        ...
        AnimationBuilder::loadFootageAsset (const skjson::ObjectValue& jimage) const
    {
        ...
        if (! asset)
        {
            this->log (Logger::Level::kError, nullptr, "Could not load image asset: %s/%s (id: '%s').", path->begin(), name->begin(), id->begin());
            return nullptr;
        }
