...
static bool parse_encoding_data (const skjson::Value& jv, size_t data_len, float data[])
{
    ...
    auto parse_optional_point = [&parse_point] (const skjson::ArrayValue* ja, size_t i, float* x, float* y) {
        if (! ja || i >= ja->size())
        {
    ...
    for (size_t i = 0; i < jvs->size(); ++i)
    {
        ...
        if (! parse_point (jvs, i, dst + kX_Index, dst + kY_Index) || ! parse_optional_point (jis, i, dst + kInX_Index, dst + kInY_Index) || ! parse_optional_point (jos, i, dst + kOutX_Index, dst + kOutY_Index))
        {
            return false;
        }
    }
    ...
    auto addCubic = [&] (size_t from_vertex, size_t to_vertex) {
        ...
        const SkPoint p0 = SkPoint { (*this)[from_index + kX_Index],
                      ...
                      c0 = SkPoint { (*this)[from_index + kOutX_Index],
                                     (*this)[from_index + kOutY_Index] }
                           + p0,
                      c1 = SkPoint { (*this)[to_index + kInX_Index],
                                     (*this)[to_index + kInY_Index] }
                           + p1;
