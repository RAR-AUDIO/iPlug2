...
namespace internal
{
    namespace
    {
        ...
        public:
            VectorKeyframeAnimator (std::vector<Keyframe> kfs,
                ...
                : INHERITED (std::move (kfs), std::move (cms)), fStorage (std::move (storage)), fVecLen (vec_len), fTarget (target_value)
            {
                                                      ...
                                                      Keyframe::Value* kfv)
    {
        ...
        if (fCurrentVec > 0 && ! memcmp (fStorage.data() + offset, fStorage.data() + offset - fVecLen, fVecLen * sizeof (float)))
        {
