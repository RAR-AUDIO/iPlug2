...
namespace
{
    ...
    public:
                                             ...
                                             const SkSize& displ_size)
        {
            ...
            return sk_sp<DisplacementNode> (new DisplacementNode (std::move (child), child_size, std::move (displ), displ_size));
        }
    ...
    private:
        DisplacementNode (sk_sp<RenderNode> child, const SkSize& child_size, sk_sp<RenderNode> displ, const SkSize& displ_size)
            : INHERITED ({ std::move (child) }), fDisplSource (std::move (displ)), fDisplSize (displ_size), fChildSize (child_size)
        {
        ...
        sk_sp<SkShader> buildEffectShader (sksg::InvalidationController* ic, const SkMatrix& ctm)
        {
            ...
            if ((IsConst (fXSelector) && IsConst (fYSelector)) || (SkScalarNearlyZero (fScale.x) && SkScalarNearlyZero (fScale.y)))
            {
                return nullptr;
            }
            ...
            const float selector_m[] = {
                xc.dr * s.x, yc.dr * s.y, 0, 0, xc.dg * s.x, yc.dg * s.y, 0, 0, xc.db * s.x, yc.db * s.y, 0, 0, xc.da * s.x, yc.da * s.y, xc.c_scale, yc.c_scale,
        ...
        void onRender (SkCanvas* canvas, const RenderContext* ctx) const override
        {
            ...
            auto local_ctx = ScopedRenderContext (canvas, ctx).setIsolation (this->bounds(), canvas->getTotalMatrix(), true);
