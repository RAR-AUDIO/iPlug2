    ...
    namespace
    {
        ...
        private:
            TritoneAdapter (const skjson::ArrayValue& jprops,
                            sk_sp<sksg::RenderNode> layer,
                            const AnimationBuilder& abuilder)
                : fLoColorNode (sksg::Color::Make (SK_ColorBLACK)), fMiColorNode (sksg::Color::Make (SK_ColorBLACK)), fHiColorNode (sksg::Color::Make (SK_ColorBLACK)), fCF (sksg::GradientColorFilter::Make (std::move (layer), { fLoColorNode, fMiColorNode, fHiColorNode }))
            {
                enum : size_t
                {
                    kHiColor_Index = 0,
