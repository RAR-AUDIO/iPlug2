...
namespace internal
{
    ...
    MotionBlurEffect::MotionBlurEffect (sk_sp<Animator> animator,
                                        ...
                                        float phase,
                                        float dt)
        : INHERITED ({ std::move (child) }), fAnimator (std::move (animator)), fSampleCount (samples), fPhase (phase), fDT (dt) {}
                                                          ...
                                                          const RenderContext* ctx) const
    {
        ...
        SkASSERT (layer);
        SkASSERT (info.colorType() == kRGBA_8888_SkColorType || info.colorType() == kBGRA_8888_SkColorType);

        SkASSERT (! info.isEmpty());
        std::vector<uint64_t> accum (info.width() * info.height());

        SkDEBUGCODE (size_t frames_rendered = 0;) bool needs_clear = false; // Cleared initially by saveLayer().
        for (size_t i = 0; i < fSampleCount; ++i)
        {
    ...
    void MotionBlurEffect::onRender (SkCanvas* canvas, const RenderContext* ctx) const
    {
        ...
        SkPixmap pm;
        if (canvas->peekPixels (&pm) && (canvas->imageInfo().colorType() == kRGBA_8888_SkColorType || canvas->imageInfo().colorType() == kBGRA_8888_SkColorType)
