    ...
    namespace
    {
        ...
        private:
            TintAdapter (const skjson::ArrayValue& jprops,
                         sk_sp<sksg::RenderNode> layer,
                         const AnimationBuilder& abuilder)
                : fColorNode0 (sksg::Color::Make (SK_ColorBLACK)), fColorNode1 (sksg::Color::Make (SK_ColorBLACK)), fFilterNode (sksg::GradientColorFilter::Make (std::move (layer), fColorNode0, fColorNode1))
            {
                enum : size_t
                {
                    kMapBlackTo_Index = 0,
