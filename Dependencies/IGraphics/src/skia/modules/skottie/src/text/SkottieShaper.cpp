...
namespace
{
    ...
    public:
        BlobMaker (const Shaper::TextDesc& desc, const SkRect& box, const sk_sp<SkFontMgr>& fontmgr)
            : fDesc (desc), fBox (box), fHAlignFactor (HAlignFactor (fDesc.fHAlign)), fFont (fDesc.fTypeface, fDesc.fTextSize), fShaper (SkShaper::Make (fontmgr))
        {
        ...
        Shaper::Result finalize (SkSize* shaped_size)
        {
            ...
            auto extent_box = [&]() {
                ...
                const auto typographical_top = fBox.fTop + ascent,
                           typographical_bottom = fBox.fTop + fLastLineDescent + fDesc.fLineHeight * (fLineCount > 0 ? fLineCount - 1 : 0ul);
                                   ...
                                   uint32_t line_index)
        {
            ...
            for (size_t i = 0; i < rec.fGlyphCount; ++i)
            {
                ...
                fResult.fFragments.push_back ({ fBuilder.make(),
                                                ...
                                                ascent,
                                                line_index,
                                                is_whitespace (fUTF8[clusters[i]]) });
    ...
    Shaper::Result ShapeImpl (const SkString& txt, const Shaper::TextDesc& desc, const SkRect& box, const sk_sp<SkFontMgr>& fontmgr, SkSize* shaped_size = nullptr)
    {
    ...
    Shaper::Result ShapeToFit (const SkString& txt, const Shaper::TextDesc& orig_desc, const SkRect& box, const sk_sp<SkFontMgr>& fontmgr)
    {
...
Shaper::Result Shaper::Shape (const SkString& txt, const TextDesc& desc, const SkPoint& point, const sk_sp<SkFontMgr>& fontmgr)
{
    return (desc.fResize == ResizePolicy::kScaleToFit || desc.fResize == ResizePolicy::kDownscaleToFit) // makes no sense in point mode
               ? Result()
               : ShapeImpl (txt, desc, SkRect::MakeEmpty().makeOffset (point.x(), point.y()), fontmgr);
}

Shaper::Result Shaper::Shape (const SkString& txt, const TextDesc& desc, const SkRect& box, const sk_sp<SkFontMgr>& fontmgr)
{
    switch (desc.fResize)
    {
...
SkRect Shaper::Result::computeVisualBounds() const
{
    ...
    {
        bounds.join (ComputeBlobBounds (fragment.fBlob).makeOffset (fragment.fPos.x(), fragment.fPos.y()));
    }
