...
namespace internal
{
    namespace
    {
        ...
        T ParseEnum (const TArray& arr, const skjson::Value& jenum, const AnimationBuilder* abuilder, const char* warn_name)
        {
            ...
            if (idx != 0)
            {
                abuilder->log (Logger::Level::kWarning, nullptr, "Ignoring unknown range selector %s '%d'", warn_name, idx);
            }
...
*/
        ...
        struct ShapeGenerator
        {
            ...
            ShapeGenerator (const ShapeInfo& sinfo, float ease_lo, float ease_hi)
                : shape_mapper (sinfo.ctrl0, sinfo.ctrl1), ease_mapper (EaseVec (ease_lo), SkVector { 1, 1 } - EaseVec (ease_hi)), e0 (sinfo.e0), e1 (sinfo.e1), crs (sinfo.crs) {}
                                              ...
                                              AnimatablePropertyContainer* acontainer)
    {
...

        {
            ...
            {
                abuilder->log (Logger::Level::kWarning, nullptr, "Ignoring unsupported selector type '%d'", type);
                return nullptr;
            }
        }
    ...
    RangeSelector::RangeSelector (Units u, Domain d, Mode m, Shape sh)
        : fUnits (u), fDomain (d), fMode (m), fShape (sh)
    {
