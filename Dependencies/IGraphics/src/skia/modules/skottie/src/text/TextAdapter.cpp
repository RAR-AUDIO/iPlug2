...
namespace internal
{
    ...
    TextAdapter::TextAdapter (sk_sp<SkFontMgr> fontmgr, sk_sp<Logger> logger, AnchorPointGrouping apg)
        : fRoot (sksg::Group::Make()), fFontMgr (std::move (fontmgr)), fLogger (std::move (logger)), fAnchorPointGrouping (apg), fHasBlurAnimator (false), fRequiresAnchorPoint (false) {}
    ...
    void TextAdapter::onSync()
    {
        ...
        switch (fAnchorPointGrouping)
        {
            ...
            case AnchorPointGrouping::kWord:
                grouping_domain = &fMaps.fWordsMap;
                break;
            case AnchorPointGrouping::kLine:
                grouping_domain = &fMaps.fLinesMap;
                break;
        ...
        for (const auto& line_span : fMaps.fLinesMap)
        {
            ...
            for (size_t i = line_span.fOffset; i < line_span.fOffset + line_span.fCount; ++i)
            {
                ...
                if (grouping_domain && i >= (*grouping_domain)[grouping_span_index].fOffset + (*grouping_domain)[grouping_span_index].fCount)
                {
                    grouping_span_index += 1;
                    SkASSERT (i < (*grouping_domain)[grouping_span_index].fOffset + (*grouping_domain)[grouping_span_index].fCount);
                }
                ...
                this->pushPropsToFragment (props, frag, fGroupingAlignment * .01f, // percentage
                                           grouping_domain ? &(*grouping_domain)[grouping_span_index] : nullptr);
                                       ...
                                       float total_tracking) const
    {
        ...
        total_tracking -= 0.5f * (buf[line_span.fOffset].props.tracking + buf[line_span.fOffset + line_span.fCount - 1].props.tracking);
