...
SkRect SkSVGLengthContext::resolveRect (const SkSVGLength& x, const SkSVGLength& y, const SkSVGLength& w, const SkSVGLength& h) const
{
    ...
    : fInherited (SkSVGPresentationAttributes::MakeInitial())
{
    ...
    SkSVGRenderContext fake (&fakeCanvas, nullptr, SkSVGIDMapper(), SkSVGLengthContext (SkSize::Make (0, 0)), *this, nullptr);
...
SkSVGRenderContext::SkSVGRenderContext (SkCanvas* canvas,
    ...
    : fFontMgr (fmgr), fIDMapper (mapper), fLengthContext (lctx), fPresentationContext (pctx), fCanvas (canvas), fCanvasSaveCount (canvas->getSaveCount()), fNode (node) {}
...
void SkSVGRenderContext::updatePaintsWithCurrentColor (const SkSVGPresentationAttributes& attrs)
{
    ...
    {
        applySvgPaint (*this, *fPresentationContext->fInherited.fFill, &fPresentationContext.writable()->fFillPaint);
    }

    if (fPresentationContext->fInherited.fStroke->type() == SkSVGPaint::Type::kCurrentColor)
    {
        applySvgPaint (*this, *fPresentationContext->fInherited.fStroke, &fPresentationContext.writable()->fStrokePaint);
    }
}
