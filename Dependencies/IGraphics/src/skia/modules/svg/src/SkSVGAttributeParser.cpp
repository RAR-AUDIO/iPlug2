...
bool SkSVGAttributeParser::parseHexColorToken (SkColor* c)
{
    ...
    switch (fCurPos - initial)
    {
        ...
        case 4:
            // matched '#xxx;
            v = ((v << 12) & 0x00f00000) | ((v << 8) & 0x000ff000) | ((v << 4) & 0x00000ff0) | ((v << 0) & 0x0000000f);
            break;
        default:
            return false;
    }
...
bool SkSVGAttributeParser::parseRGBColorToken (SkColor* c)
{
    return this->parseParenthesized (
        "rgb", [this] (SkColor* c) -> bool {
            int32_t r, g, b;
            if (this->parseColorComponentToken (&r) && this->parseSepToken() && this->parseColorComponentToken (&g) && this->parseSepToken() && this->parseColorComponentToken (&b))
            {
...
{
    return this->parseParenthesized (
        "url", [this] (SkSVGStringType* iriString) -> bool {
            SkSVGIRI iri;
            if (this->parse (&iri))
            {
                *iriString = iri.fIRI;
...
bool SkSVGAttributeParser::parse (SkSVGLength* length)
{
    ...
    if (this->parseScalarToken (&s) && (this->parseLengthUnitToken (&u) || this->parseSepToken() || this->parseEOSToken()))
    {
...
bool SkSVGAttributeParser::parseViewBox (SkSVGViewBoxType* vb)
{
    ...
    bool parsedValue = false;
    if (this->parseScalarToken (&x) && this->parseSepToken() && this->parseScalarToken (&y) && this->parseSepToken() && this->parseScalarToken (&w) && this->parseSepToken() && this->parseScalarToken (&h))
    {
...
{
    return this->parseParenthesized (
        "translate", [this] (SkMatrix* m) -> bool {
