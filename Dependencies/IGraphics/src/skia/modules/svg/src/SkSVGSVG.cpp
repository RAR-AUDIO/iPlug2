...
static SkMatrix ViewboxMatrix (const SkRect& view_box, const SkRect& view_port, const SkSVGPreserveAspectRatio& par)
{
    ...
    auto compute_trans = [&] (const SkV2& scale) -> SkV2 {
        ...
        SkASSERT (x_coeff < SK_ARRAY_COUNT (gAlignCoeffs) && y_coeff < SK_ARRAY_COUNT (gAlignCoeffs));
...
SkSize SkSVGSVG::intrinsicSize (const SkSVGLengthContext& lctx) const
{
    ...
    if (fWidth.unit() == SkSVGLength::Unit::kPercentage || fHeight.unit() == SkSVGLength::Unit::kPercentage)
    {
