...
void SkBidiIterator::ReorderVisual (const Level runLevels[], int levelsCount, int32_t logicalFromVisual[])
{
...
public:
    ...
    static std::unique_ptr<SkBreakIterator> makeUtf8BreakIterator (const char locale[], SkUnicode::BreakType type)
    {
        UErrorCode status = U_ZERO_ERROR;
...
class SkUnicode_icu : public SkUnicode
{
    static UBreakIteratorType convertType (BreakType type)
    {
        switch (type)
        {
            case BreakType::kLines:
                return UBRK_LINE;
            case BreakType::kGraphemes:
                return UBRK_CHARACTER;
            case BreakType::kWords:
                return UBRK_WORD;
            default:
    ...
    static bool extractPositions (const char utf8[], int utf8Units, BreakType type, std::function<void (int, int)> setBreak)
    {
        UErrorCode status = U_ZERO_ERROR;
        UText sUtf8UText = UTEXT_INITIALIZER;
...
public:
                        ...
                        std::vector<LineBreakBefore>* results) override
    {
        return extractPositions (utf8, utf8Units, BreakType::kLines, [results] (int pos, int status) {
            results->emplace_back (pos, status == UBRK_LINE_HARD ? LineBreakType::kHardLineBreak : LineBreakType::kSoftLineBreak);
        });
    }
