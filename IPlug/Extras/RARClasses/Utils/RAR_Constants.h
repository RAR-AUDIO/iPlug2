...
//
// Category    : Utils
// Filename    : Utils/RAR_Constants.h
...
struct ShapeFromMiddle : public IParam::Shape {
  ShapeFromMiddle(double minVal = 0.0, double maxVal = 1.0,
                  double centeredVal = 0.5, double pos = 0.5)
  ...
  Shape* Clone() const override {
    return new ShapeFromMiddle(mMin, mMax, mCtr, mPos);
  };
  ...
  double NormalizedToValue(double value, const IParam& param) const override {
    return param.GetMin() +
           std::pow(value, mShape) * (param.GetMax() - param.GetMin());
  };
  double ValueToNormalized(double value, const IParam& param) const override {
    return std::pow(
        (value - param.GetMin()) / (param.GetMax() - param.GetMin()),
        1.0 / mShape);
  };
  double mShape;
...
struct RARParamProperties {
  ...
  const double defaultVal =
      0.0;  // Plugin loads in this state, return by double click
  ...
  const double centerVal = 0.5;    // Value that void setShapeCentered() will
                                   // center (knobs middle position)
  const double centerPoint =
      0.5;  // Value WHERE void setShapeCentered() will center centerVal (0.5
            // means: real middle position)
  const char* unit = "";   // Measuring unit of parameter
  const char* group = "";  // Parameter group, not supported by every host
  const IParam::EParamType Type =
      IParam::EParamType::kTypeNone;  // Data type of parameter
...
const double dcoff =
    1e-15;  // amount of DC offset added and subtracted respectivly at the
            // beginning and the end of DoubleReplacing loop
...
const double cascadedFilterQs[EFilterSlope::kNumOrders][10] = {
    ...
    {0.50431448, 0.54119610, 0.63023621, 0.82133982, 1.3065630,
     3.8306488},  // order 12
    {0.50154610, 0.51420760, 0.54119610, 0.58641385, 0.65754350, 0.76988452,
     0.95694043, 1.3065630, 2.1418288, 6.3727474}  // order 20
};
