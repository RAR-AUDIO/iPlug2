...
//
// Category    : Utils
// Filename    : RAR_Param.h
// Created by  : RAR-AUDIO, JUL/2020
// Author      : Roberto Ramirez
// Description : Parameter Smoothing
//
//---------------------------------------------------------------------------------
// LICENSE
...
class RAR_ParamSmooth {
 public:
  ...
  RAR_ParamSmooth(double value, int numSmoothSamples)
      : mTargetValue(value),
        mCurrentValue(value),
  ...
  void Set(double value) {
    if (mTargetValue != value) {
      ...
      mValueChangePerSample =
          (mTargetValue - mCurrentValue) / mNumSmoothSamples;
    }
  }
...
enum EType { kDouble = 0, kFloat, kInt, kBool, kNoninit, kNumTypes };
class RAR_Param {
 public:
  RAR_Param(int numSmoothSamples = 0)
      : mNumSmoothSamples(numSmoothSamples), mNumParams(0), mParams() {}
  ~RAR_Param() {
  ...
  }  // Add Parameter, use EParams loop. You have to set value and if it's
     // smoothing after that
  ...
  void SetDouble(int paramIdx, double value, bool smooth = true) {
    ...
    if (smooth)
      paramToSet->mSmoothSamplesLeft = mNumSmoothSamples;
    else
      paramToSet->mValue = (double)value;
    // paramToSet->mValue = value;

    // if (paramIdx == 0) DBGMSG("Set: i: %i; Tp: %i; v: %f; V: %f; TV: %f\n",
    // paramIdx, paramToSet->mType, value, paramToSet->mValue,
    // paramToSet->mTargetValue);
  ...
  float GetFloat(int paramIdx) {
    return std::get<float>(mParams.Get(paramIdx)->mValue);
  }  // Get floating value at index
  int GetInt(int paramIdx) {
    return std::get<int>(mParams.Get(paramIdx)->mValue);
  }  // Get integer value at index
  bool GetBool(int paramIdx) {
    return std::get<bool>(mParams.Get(paramIdx)->mValue);
  }  // Get boolean value at index

  void SetSmoothing(int paramIdx, bool smoothing) {
    mParams.Get(paramIdx)->mSmooth =
        (mNumSmoothSamples > 0) ? smoothing : false;
  }  // Set if parameter changes should be smoothed
  void SetGlobalNumSmoothSamples(int samples) { mNumSmoothSamples = samples; }
  ...
  void Process(int paramIdx) {
    ...
    if (paramToProcess->mSmoothSamplesLeft > 0) {
      switch (mParams.Get(paramIdx)->mType) {
        case EType::kDouble:
          // if (paramIdx == 7) DBGMSG("%f %f %i",
          // std::get<double>(paramToProcess->mTargetValue),
          // std::get<double>(paramToProcess->mValue),
          // paramToProcess->mIsCurrentlySmoothing); paramToProcess->mValue =
          // paramToProcess->mSmoothFilter.Process(std::get<double>(paramToProcess->mTargetValue));
          paramToProcess->mValue =
              double(std::get<double>(paramToProcess->mValue) +
                     (std::get<double>(paramToProcess->mTargetValue) -
                      std::get<double>(paramToProcess->mValue)) *
                         (1. / paramToProcess->mSmoothSamplesLeft));
        ...
        case EType::kFloat:
          paramToProcess->mValue =
              float(std::get<float>(paramToProcess->mValue) +
                    (std::get<float>(paramToProcess->mTargetValue) -
                     std::get<float>(paramToProcess->mValue)) *
                        (1.f / paramToProcess->mSmoothSamplesLeft));
          paramToProcess->mSmoothSamplesLeft--;
          if (paramToProcess->mSmoothSamplesLeft == 0)
            paramToProcess->mValue = paramToProcess->mTargetValue;
          break;
