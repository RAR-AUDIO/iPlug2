...
namespace RAR{
  namespace DSP{
    ...
    public:
      ...
      void SetGainDb(double pGainDb){ SetGain(RAR::Utils::DBToAmp(pGainDb)); /* don't update because SetGain does */ } // Set gain with decibel value

      double GetGain(){ return mGainLin; } // Get current gain value (linear voltage)
      double GetGainDb(){ return RAR::Utils::AmpToDB(mGainLin); } // Get current gain value (decibels)
