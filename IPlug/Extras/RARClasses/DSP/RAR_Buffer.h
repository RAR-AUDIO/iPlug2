...
#define BUFFERIMPL 3
#ifndef _RAR_DSP_BUFFER_H
#define _RAR_DSP_BUFFER_H
...
template <typename T = double, int MAXNUMCHANNELS = 1,
          int MAXNUMFRAMES = RAR::Utils::DEFAULT_BLOCK_SIZE>
class RAR_Buffer {
 public:
  // Construct class
  RAR_Buffer(int nChannels = MAXNUMCHANNELS, int nFrames = MAXNUMFRAMES)
  ...
  T AverageBufferAbs(int channel) {
    return SumBufferAbs(channel) / (T)mNumFrames;
  }
