...
class RAR_Saturation {
 public:
  ...
  RAR_Saturation(RAR_Saturation::ESaturationType pType, double pDriveDb,
                 double pAmountNormalized, double pHarmonicsNormalized,
                 bool pPositiveSide, double pSkewNormalized, double pWet,
                 double pSamplerate)
      : prev(0.0), dry(0.0) {
    SetSaturation(pType, pDriveDb, pAmountNormalized, pHarmonicsNormalized,
                  pPositiveSide, pSkewNormalized, pWet, pSamplerate);
  }

  ~RAR_Saturation() {}

  void SetType(RAR_Saturation::ESaturationType pType) {
    mType = pType;
    calcSaturation();
  }
  ...
  void SetSaturation(RAR_Saturation::ESaturationType pType, double pDriveDb,
                     double pAmountNormalized, double pHarmonicsNormalized,
                     bool pPositive, double pSkewNormalized,
                     double pWetNormalized, double pSamplerate) {
    mType = pType;
    mDriveNormalized = RAR::Utils::DBToAmp(pDriveDb);
    mAmountNormalized = pAmountNormalized;
    mHarmonicsNormalized = pHarmonicsNormalized;
 ...
 protected:
  ...
  bool mPositive;  // if aiming for even harmonics, the positive side of the
                   // envelope will be affected if true, otherwise the negative
                   // side
...
inline double RAR_Saturation::Process(double in) {
  ...
  if (mAmountNormalized == 0.0) return in;
  ...
  switch (mType) {
    case ESaturationType::kMusicDSP:
      in = processMusicDSP(in);
      break;
    case ESaturationType::kZoelzer:
      in = processZoelzer(in);
      break;
    case ESaturationType::kPirkle:
      in = processPirkle(in);
      break;
    case ESaturationType::kPirkleMod:
      in = processPirkleModified(in);
      break;
    case ESaturationType::kSoftSat:
      in = processSoftSat(in);
      break;
    case ESaturationType::kRectHalf:
      in = processRectHalf(in);
      break;
    case ESaturationType::kRectFull:
      in = processRectFull(in);
      break;
    default:
      break;
  }
  ...
  if (!mPositive && in < 0.)
    in = in * mHarmonicsNormalized + dry * (1. - mHarmonicsNormalized);
  if (mPositive && in > 0.)
    in = in * mHarmonicsNormalized + dry * (1. - mHarmonicsNormalized);
...
inline double RAR_Saturation::processMusicDSP(double in) {
  if (fabs(in) > mAmount) {
    in = (in > 0.)
             ? (mAmount +
                (fabs(in) - mAmount) /
                    (1. + pow((fabs(in) - mAmount) / (1. - mAmount), 2.))) *
                   (in / fabs(in))
             : (mAmount +
                (fabs(in) - mAmount) /
                    (1. + pow((fabs(in) - mAmount) / (1. - mAmount), 2.))) *
...
inline double RAR_Saturation::processPirkleModified(double in) {
  const double a =
      mAmount * (1. + (in - prev) * (1. / mDriveNormalized) * mSkewNormalized);
  in = tanh(a * in) / tanh(a);
  return in;
}
...
inline double RAR_Saturation::processSoftSat(double in) {
  in = (in > 1.)
           ...
           : (in < -1.) ? -mAmount * (1. + mAmount / (in - (mAmount - 1.))) - 1.
                        : in;
  return in;
}
