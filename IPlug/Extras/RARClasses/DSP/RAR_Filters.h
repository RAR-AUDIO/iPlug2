...
// Description : Filters
//               SVF (Based on IPlug2's SVF Filter
//               TwoPole (Nigel Redmon)
//
//---------------------------------------------------------------------------------
// LICENSE
...
namespace RAR {
namespace DSP {
#define SVFMODES_VALIST                                                       \
  "LowPass", "HighPass", "BandPass", "Notch", "Peak", "Bell", "LowPassShelf", \
      "HighPassShelf"
...
class SVF {
 public:
  ...
  void SetFreqCPS(double freqCPS) {
    mNewState.freq = Clip(freqCPS, 10, 20000.);
  }
 ...
 private:
  ...
  struct Settings {
    ...
    bool operator!=(const Settings& other) const {
      return !(mode == other.mode && freq == other.freq && q == other.q &&
               gain == other.gain && sampleRate == other.sampleRate);
    }
  };
...
class RAR_FilterIIR {
 public:
  ...
  RAR_FilterIIR(EFilterType type, double frequency, double q, double peakGainDB,
                double samplerate) {
  ...
  void SetFilter(EFilterType type, double frequency, double q,
                 double peakGainDb, double samplerate) {
    mType = type;
    mQ = q;
    mFreq = frequency;
  ...
  void GetFrequencyResponse(double* values, int points, double rangeDb,
                            bool returnPhase) {
    for (int i = 0; i < points; ++i) {
      values[i] = GetFrequencyResponse(0.5 * double(i) / double(points),
                                       rangeDb, returnPhase);
    }
  }  // Get frequency response or phase at specified points

  double GetFrequencyResponse(double atNormalizedFrequency, double rangeDb,
                              bool returnPhase) {
    ...
    double magnitude =
        sqrt(realHw * realHw + imagHw * imagHw);  // magnitude linear
...
class RAR_FilterOnePole {
 public:
  ...
  RAR_FilterOnePole(double frequency, double samplerate) {
...
class RAR_FilterParamSmooth {
 public:
  RAR_FilterParamSmooth(double smoothingTimeInMs = 10.0,
                        double samplerate = 44100.0) {
    Reset(smoothingTimeInMs, samplerate);
  }

  ~RAR_FilterParamSmooth() {}
