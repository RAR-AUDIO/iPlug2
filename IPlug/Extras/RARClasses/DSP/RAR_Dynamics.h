...
// Holds attack AND release parameters
    ...
    public:
      ...
      //-------------------------------------------------------------
      RAR_DynamicsDetector(double mAttackMs = 10.0, double mReleaseMs = 100.0, double sampleRate = 44100.0)
    ...
    public:
      ...
      // Constructor
      RAR_DynamicsBase(double threshDb = 0.0, double ratio = 1.0, bool autoMakeup = false)
        : mThreshDb(threshDb)
    ...
    protected:
      ...
      virtual double AutoMakeup(double threshDb, double ratio, double referenceDb, double attackMs, double releaseMs){
        return	1. + (1. / (RAR::Utils::DBToAmp(((ratio - 1.) * -threshDb) / 2.)) - 1.) * sqrt(threshDb / referenceDb) * (sqrt(30.) / sqrt(attackMs)) * (sqrt(releaseMs) / sqrt(5000.));
      }
    ...
    class RAR_Compressor
      : public RAR_DynamicsDetector
      , public RAR_DynamicsBase{
    public:
      //-------------------------------------------------------------
      ...
      virtual void InitSidechainFilter(double sidechainFC){
        ...
        fSidechainFilter.SetFilter(RAR::DSP::RAR_FilterIIR<double, 2>::EFilterType::BiquadHighpass, sidechainFC, 0.7071, 0., RAR_DynamicsDetector::GetSampleRate());
      }
    ...
    protected:
      RAR::DSP::RAR_FilterIIR<double, 2> fSidechainFilter; // Compressors stereo sidechain filter
    ...
    //-------------------------------------------------------------
    class RAR_CompressorRMS
      : public RAR_Compressor{
    public:
    ...
    inline void RAR_Compressor::Process(double& in1, double& in2){
      ...
      double rectifiedInputMaxed = max(rectifiedInput1, rectifiedInput2);	// link channels with greater of 2
      process(in1, in2, rectifiedInputMaxed);	// rest of process
    }

    // Compressor runtime process for external sidechain
    inline void RAR_Compressor::Process(double& in1, double& in2, double& extSC1, double& extSC2){
    ...
    inline void RAR_Compressor::process(double& in1, double& in2, double sidechain){
      sidechain = fabs(sidechain);                          // rect (just in case)
      sidechain += DC_OFFSET;                               // add DC offset to avoid log( 0 )
      double sidechainDb = RAR::Utils::AmpToDB(sidechain);   // linear -> dB conversion
      ...
      RAR_DynamicsDetector::process(sampleOvershootDb, currentOvershootDb);	// process attack/release envelope
      ...
      grRaw = mGrLin = RAR::Utils::DBToAmp(grRaw);// Logarithmic to linear conversion
    ...
    inline void RAR_Limiter::Process(double& in1, double& in2){
      ...
      double keyLink = max(rectifiedInput1, rectifiedInput2);	// link channels with greater of 2
    ...
    public:
      RAR_Gate()
        : RAR_DynamicsDetector(1.0, 100.0)
        , RAR_DynamicsBase(0.0, 1.0){}
      virtual ~RAR_Gate(){}
    ...
    inline void RAR_Gate::Process(double& in1, double& in2, double sidechain){
      ...
      RAR_DynamicsDetector::process(gateGainApply, currentOvershootLin);	// process attack/release
    ...
    public:
      RAR_Deesser()
        ...
        , mFilterQ(0.707)
        , mFilterGain(0.0)
        , fSidechainBandpass(RAR::DSP::RAR_FilterIIR<double, 2>(RAR::DSP::RAR_FilterIIR<double, 2>::EFilterType::BiquadBandpass, 0.5, 0.707, 0.0, 44100.0))
        , fDynamicEqFilter(RAR::DSP::RAR_FilterIIR<double, 2>(RAR::DSP::RAR_FilterIIR<double, 2>::EFilterType::BiquadPeak, 0.5, 0.707, 0.0, 44100.0)){}
      virtual ~RAR_Deesser(){}
      ...
      virtual void InitFilter(double freq, double q){
        mFilterFreq = freq;
        mFilterQ = q;
        fSidechainBandpass.SetFilter(RAR::DSP::RAR_FilterIIR<double, 2>::EFilterType::BiquadBandpass, mFilterFreq, mFilterQ, 0.0, GetSampleRate());
