...
class envFollower {
 public:
  enum kMode { kPeak, kRMS };

  envFollower() { init(kPeak, 5, 50, 0, 44100); }

  ~envFollower() {}

  envFollower(double attackMS, double releaseMS, double holdMS,
              double SampleRate) {
    init(kPeak, attackMS, releaseMS, holdMS, SampleRate);
  }

  virtual void init(int detectMode, double attackMS, double releaseMS,
                    double holdMS, double SampleRate) {
    mode = detectMode;
    sr = SampleRate;
  ...
  void setAttack(double attackMS) { attack = attackMS; }

  void setRelease(double releaseMS) { release = releaseMS; }

  void setHold(double holdMS) { hold = holdMS; }
...
class compressor : public envFollower {
 public:
  enum kMode { kCompressor, kLimiter };

  compressor() { init(5, 50, 0, 4, 0, 44100); }

  compressor(double attackMS, double releaseMS, double holdMS, double ratio,
             double knee, double SampleRate) {
    init(attackMS, releaseMS, holdMS, ratio, knee, SampleRate);
  }

  ~compressor(){};

  void init(double attackMS, double releaseMS, double holdMS, double ratio,
            double knee, double SampleRate) {
  ...
  void initRMS(double attackMS, double releaseMS, double holdMS, double ratio,
               double knee, double SampleRate) {
  ...
  void setHold(double holdMS) { hold = holdMS / 1000. * sr; }
  ...
  double process(double sample) {
    double e = RAR::Utils::AmpToDB(envFollower::process(sample));
 ...
 private:
  double gainReduction, mKnee, mRatio, mThreshold, kneeWidth, kneeBoundL,
      kneeBoundU, slope;
  int mMode;
