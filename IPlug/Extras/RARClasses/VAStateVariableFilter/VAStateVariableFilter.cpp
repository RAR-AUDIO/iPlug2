...
void VAStateVariableFilter::setFilter(const int& newType,
                                      const float& newCutoffFreq,
                                      const float& newResonance,
                                      const float& newShelfGain) {
  filterType = newType;
  cutoffFreq = newCutoffFreq;
  Q = static_cast<float>(resonanceToQ(newResonance));
...
/*void VAStateVariableFilter::setSmoothingTimeInMs(const float &
newSmoothingTimeMs)
{
    smoothTimeMs = newSmoothingTimeMs;
}*/

void VAStateVariableFilter::setIsActive(bool isActive) { active = isActive; }
...
float VAStateVariableFilter::processAudioSample(const float& input,
  ...
  if (active) {
    ...
    // Filter processing:
    const float HP = (input - (2.0f * RCoeff + gCoeff) * z1_A[channelIndex] -
                      z2_A[channelIndex]) /
...
void VAStateVariableFilter::processAudioBlock(float* const samples,
