...
class AWNoiseShaper {
 public:
  AWNoiseShaper() {}
  ~AWNoiseShaper() {}
  inline void ProcessBlock(double** inputs, double** outputs, int nChannels,
                           int nFrames) {
  ...
  inline void ProcessBlock(float** inputs, float** outputs, int nChannels,
                           int nFrames) {
    for (int s = 0; s < nFrames; s++) {
      for (int c = 0; c < nChannels; c++) {
        if (inputs[c][s] < 1.2e-38 && -inputs[c][s] < 1.2e-38) {
...
class AWBase {
 public:
  AWBase() : mSampleRate(44100.), mOverallScale(1.) {}
  ~AWBase() {}
...
template <int MAXNUMCHANNELS = 2>
class AWConsoleChannel : AWBase {
 public:
  enum EConsoleType { kConsole6 = 0, kConsole5, kNumConsoleTypes };

  AWConsoleChannel() : AWBase(), mInput(1.0), mType(EConsoleType::kConsole6) {
    Reset();
  }
      ...
      for (int c = 0; c < nChannels; c++) {
        ...
        switch (mType) {
          case EConsoleType::kConsole6:
            if (mInput != 1.0) outputs[c][s] *= mInput;
            if (outputs[c][s] > 1.0)
              outputs[c][s] = 1.0;
            else if (outputs[c][s] > 0.0)
              outputs[c][s] = 1.0 - pow(1.0 - outputs[c][s], 2.0);
          ...
          case EConsoleType::kConsole5:
            ...
            gainchase =
                (((gainchase * chasespeed) + mInput) / (chasespeed + 1.0));
...
template <int MAXNUMCHANNELS = 2>
class AWConsoleBuss : AWBase {
 public:
  enum EConsoleType { kConsole6 = 0, kConsole5, kNumConsoleTypes };
  AWConsoleBuss() : AWBase(), mInput(1.0), mType(EConsoleType::kConsole6) {
    Reset();
  }
  ~AWConsoleBuss() {}
        ...
        switch (mType) {
          case EConsoleType::kConsole6:
            ...
            if (outputs[c][s] > 1.0)
              ...
              outputs[c][s] = 1.0 - pow(1.0 - outputs[c][s], 0.5);
          ...
          case EConsoleType::kConsole5:
            ...
            if (1.0 != gainchase) outputs[c][s] *= gainchase;
