...
def  main():
  ...
  installer = "\installer\RARPluginFaust Installer.exe"
   
  if demo:
    installer = "\installer\RARPluginFaust Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\RARPluginFaust manual.pdf" 
  ]
  ...
  ZIPNAME = "RARPluginFaust-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "RARPluginFaust-v" + FULLVERSIONSTR + "-win-demo.zip"
