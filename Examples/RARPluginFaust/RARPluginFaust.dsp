...
pfilterq  = hslider("Resonance", 1.0, 1.0, 8, 0.1)      : si.smooth(0.995);

// Distortion parameters
pdrive    = hslider("Drive", 3.0, -10.0, 10.0, 0.1)     : si.smooth(0.995);
psat      = hslider("Saturation", 0.0, 0.0, 1.0, 0.1)   : si.smooth(0.995);
pcurve    = hslider("Curve", 1.0, 0.1, 4.0, 0.1)        : si.smooth(0.995);

// Output parameters
pfeedback = hslider("Feedback", -60, -60, -24, 1)       : ba.db2linear : si.smooth(0.995);
plevel    = hslider("Level", -12, -24, 24, 1)           : ba.db2linear : si.smooth(0.995);
